...
## Main
- observed_main_before_this_edit: `a85772d12313a097e962c9005569f47a13edb7fa`
- last_verified: `2026-08-17T16:28Z`
...
- Midi / station envelope — PR #679 and #674 currently overlap; treat Midi as owned and do not add a third implementation.
- Player / fallback identity badge — PR #665 — ACTIVE.
- Shared Environment / sidewalk edge articulation — PR #662 — ACTIVE.
- Jette / generic OSM environment runtime — PR #673 — ACTIVE.
...
- Maison des Brasseurs — raw photo mapped directly to one quad is blocked; photo is constraints/reference only unless a later independently licensed derivative pipeline proves otherwise.

## NOW / NEXT / LATER
- NOW visual: red-first Brasseurs runtime contract consuming the shipped photo plan. It must prove no runtime exists yet, then require the official wall identity/span, the 4 column axes/3-bay rhythm and the photo-plan provenance.
- NEXT visual: implement only a photo-constrained facade reconstruction whose horizontal proportions come from the shipped plan; vertical world dimensions must come from an independent source or remain explicitly unresolved. No invented decorative survey claims.
- Final visual gate: true player-spawn 1280x720 A/B, strict recursive UI/dynamics masking, predeclared thresholds, mandatory human full-frame PASS.
...
## Shift handoff
- What changed: #680 is shipped and makes the Brasseurs photo-plan a production contract; Jette finish chain #676 also shipped.
- What is proven: Commons source identity/license/hash, City `010` -> UrbIS `1639974`, wall `10945501`, span `8.7490357183 m`, image axes `[720,1125,1580,2035]`, 3 bays, horizontal registers/crowning semantics; vertical world scale is still unresolved.
- What is NOT proven: any photo-constrained runtime reconstruction or visual PASS.
