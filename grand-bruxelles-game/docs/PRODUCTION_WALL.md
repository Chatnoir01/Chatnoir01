...
## Main
- observed_main_before_this_edit: `1296b057b17c54465d9d5fec19f3b86163dcf71b`
- last_verified: `2026-08-18T04:16:00Z`
- latest_verified_publication: `deb1646138eae39a7a81083044f6b03ce588f35e` — Web publication after #750. #753 is QA-only and may receive a later automatic publication without changing player runtime content.
...
## Active ownership
- #752 — vehicle/Mission 01 only: `.github/workflows/grand-bruxelles-primary-physical-vehicle.yml`, `game/scripts/mission_drive_to_center.gd`, `game/tests/primary_physical_vehicle_test.gd`. It is draft and currently NOT merge-ready: Primary Physical Vehicle is green, but Mission Save Contract and Game CI have failed on its current head. Do not overlap its files.
...
## Recently shipped
- #753 / `1296b057...` — canonical Grand-Place camera contract is now production truth. Exact merged #711 witness = 1280x720, camera `[319.01, 1.72, -535.20]`, target `[321.91, 11.8, -485.66]`, FOV 62. The new witness PNG is bit-for-bit identical to historical #711: SHA-256 `d869abcbd1cbc500954b85e5de07ee25ab4f2ecf96f0d5a593e9762a762d0798`. Exact-current-main Brasseurs owner audit from this REAL camera reports 25/25 sampled rays with `hit_count=0`, `collider_counts={}`; artifact `9310657334`, zip digest `sha256:3bfc203be76c46a00dc9547448dfa3fef85426611ca580799318d30333eda770`. This proves collision-line clearance only, NOT absence of non-collidable visual occlusion.
...
- #743 / `b4973a2...` — Brasseurs generic selected-OSM owner hypothesis rejected; nearest selected OSM building remains ~90.031492 m from wall `10945501`. Its old ray result used a mislabeled camera and is superseded by #753 for camera provenance.
- #711 — clean Grand-Place player-eye witness pattern, now backed by the shared #753 camera contract.
...
- Generic selected OSM ownership is disproved for this wall; do not assign it to `Building_306856563` or another distant OSM block.
...
- #748 is closed stale without a valid candidate A/B. It repeated the wrong `324.9581...` camera and never reached a legitimate final render. Its already-frozen future visual thresholds remain useful and must not be weakened: `>3 RGB >= 2.00%`, `>8 RGB >= 1.00%`, changed bbox `>=300x260`, 1280x720.
...
- No camera rescue: never use `Vector3(324.9581,3.3,-512.8388)` as “#711”. The canonical shared contract is production truth.
...
## NOW / NEXT / LATER
- NOW: production HEAD observed `1296b057...`. #753 camera truth is shipped. #752 owns the vehicle lot and is not yet merge-ready. No active Brasseurs implementation exists.
- NEXT Brasseurs experiment: fresh branch from then-live `main`; exact official five-vertex/three-triangle wall skin only, `details=0`, no outward offset, no neighbor hide, no windows/bands/columns/arcades. A/B must read `data/qa/grand_place_clean_player_witness.json` instead of hardcoding a camera. Freeze and preserve thresholds `2.00% / 1.00% / 300x260` before the first candidate render.
...
- What is proven: real #711 witness is exactly `[319.01,1.72,-535.20]` / target `[321.91,11.8,-485.66]` / FOV 62; its image hash is unchanged; 25/25 Brasseurs wall ray samples have no collision hit on exact-current-main #753.
