...
1. Run a short-lived record workflow on a branch with `GB_VISUAL_BASELINE_RECORD=1`.
2. Inspect the PNG artifact visually. Ask: **does this look more like real Brussels, not merely different?**
3. Version only the emitted fingerprint JSON in `data/qa/rendered_main_baseline.json`.
4. Delete the one-shot recorder before integration.
5. Re-run the normal performance workflow with record mode disabled.
6. Merge only when the normal comparison passes together with Game CI, branch hygiene, tests and photo-match QA.
