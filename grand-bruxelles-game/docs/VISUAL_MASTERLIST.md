...
Production truth: exact GitHub `main` only. Phase: **VISUAL-FIRST VERTICAL SLICE**. Geographic expansion is frozen until **Bruxelles-Midi → Anneessens → Bourse → Grand-Place** looks and feels like a credible modern game.
...
## Current production checkpoint — 2026-08-15
...
**Now:** large source-backed facade/arrival/forecourt/hero corrections with legitimate player exposure. #359 Midi multimodal furniture is closed for missing exact placement truth + missing deterministic BEFORE/AFTER. #358 Bourse facade articulation is closed: CI was green, but the only artifact was one AFTER image with the camera clipped into a large wall and dark geometry crossing the frame, so no valid human player-facing verdict existed.
...
Owns PC/Desktop target, Web preview/QA, mobile readability/controls, LOD/material budgets and measured performance blockers.

**Now:** keep #355 crowd cost stable in sustained play, maintain the passing PC Desktop export gate, and preserve Web publication. No invisible optimization unless it removes a measured blocker or enables a visible lot.
...
- **#358 Bourse context facade articulation — CLOSED WITHOUT MERGE.** Runtime counts and CI passed, but evidence was invalid: only one AFTER image and a clipped/occluded camera. Rebuild only from a future fresh main with legitimate same-camera BEFORE/AFTER and positive full-frame human verdict.
...
2. Freeze commune/cell/hero-zone expansion. No Ixelles/Laeken/Jette expansion during this phase.
...
**Character + NPC: integrate one genuine rigged, textured, animated player model into `assets/characters/player/thandi/` and prove the production Player loads it instead of the procedural fallback.** This is the highest-value 3-second improvement currently available.

Parallel only if non-overlapping: Centre may prepare one large, source-backed, legitimate-player-view facade/arrival correction; Environment may prepare one broad reusable street/material asset; Runtime may validate/protect those lots and the PC/Web builds.
