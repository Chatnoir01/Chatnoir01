...
func _start_watching() -> void:
    if _tearing_down or not is_inside_tree() or _manual_binding or _watching_tree: return
    var tree := get_tree()
    if tree == null: return
    if not tree.node_added.is_connected(_on_node_added): tree.node_added.connect(_on_node_added)
    if not tree.node_removed.is_connected(_on_node_removed): tree.node_removed.connect(_on_node_removed)
    _watching_tree = true
func _stop_watching() -> void:
    var tree := get_tree()
    if tree != null:
        if tree.node_added.is_connected(_on_node_added): tree.node_added.disconnect(_on_node_added)
        if tree.node_removed.is_connected(_on_node_removed): tree.node_removed.disconnect(_on_node_removed)
    _watching_tree = false
func _release_owned_root() -> void:
    if is_instance_valid(_root):
        var parent := _root.get_parent()
        if parent != null and not _tearing_down: parent.remove_child(_root)
        _root.queue_free()
    _root = null
    _sidewalk_count = 0
    _collision_count = 0
...
func _on_node_added(_node: Node) -> void:
    if _tearing_down or _manual_binding or is_instance_valid(_scene): return
    _schedule_bind()
func _on_node_removed(node: Node) -> void:
    if _tearing_down or not is_inside_tree() or _manual_binding or not is_instance_valid(_scene) or node != _scene: return
    _reset_scene_binding()
    _start_watching()
    _schedule_bind()
func _schedule_bind() -> void:
    if _tearing_down or not is_inside_tree() or _bind_scheduled or _manual_binding or is_instance_valid(_scene): return
...
func _is_production_scene(candidate: Node3D) -> bool:
    if candidate == null or not _has_production_anchors(candidate): return false
    var tree := get_tree()
    if tree != null and tree.current_scene == candidate: return true
...
func _direct_viewport_main(viewport: Viewport) -> Node3D:
    ...
    if main is Node3D and main.get_parent() == viewport and _is_production_scene(main as Node3D): return main as Node3D
    return null
func _find_production_scene() -> Node3D:
    if _tearing_down or not is_inside_tree(): return null
    var tree := get_tree()
    if tree == null: return null
    var current := tree.current_scene
    if current is Node3D and _is_production_scene(current as Node3D): return current as Node3D
    var root := tree.root
    if root == null: return null
    for child: Node in root.get_children():
        if child is Node3D:
            var candidate := child as Node3D
            if _is_production_scene(candidate): return candidate
            continue
        if child is Viewport:
            var viewport_candidate := _direct_viewport_main(child as Viewport)
            if viewport_candidate != null: return viewport_candidate
    return null
func _try_bind() -> void:
    _bind_scheduled = false
    if _tearing_down or not is_inside_tree() or _manual_binding or is_instance_valid(_scene): return
    var candidate := _find_production_scene()
    if candidate == null or _tearing_down or not is_inside_tree(): return
    _bind_scene(candidate, false)
func bind_scene(scene: Node3D) -> void: _bind_scene(scene, true)
...
func _bind_scene(scene: Node3D, manual: bool) -> void:
    if scene == null or (_tearing_down and not manual): return
    if is_instance_valid(_root): _release_owned_root()
...
func _build_from_existing_osm_roads() -> bool:
    if not is_instance_valid(_scene) or not is_instance_valid(_root): return false
    ...
    for child: Node in roads.get_children():
        if not child is CSGBox3D or not child.name.begins_with("Road_"): continue
        var road := child as CSGBox3D
        if Vector2(road.global_position.x, road.global_position.z).distance_to(ANNEESSENS) > DETAIL_RADIUS_M: continue
        if road.size.z < 1.0 or road.size.x < 2.0: continue
...
func _add_sidewalk_pair(road: CSGBox3D, material: Material) -> void:
    ...
    if lateral.length_squared() < 0.5: lateral = Vector3.RIGHT
    for side: float in [-1.0, 1.0]:
        ...
        _sidewalk_count += 1
func diagnostic_sidewalk_count() -> int: return _sidewalk_count
func diagnostic_collision_count() -> int: return _collision_count
func set_sidewalks_enabled(enabled: bool) -> void:
    ...
    for child: Node in _root.get_children():
        if child is CSGBox3D: (child as CSGBox3D).use_collision = false
