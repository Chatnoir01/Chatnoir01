...
static func _decode(raw: PackedByteArray) -> Node3D:
    ...
    if group_count <= 0:
        return null
    var bounds_min := Vector3(raw.decode_float(offset), raw.decode_float(offset + 4), raw.decode_float(offset + 8))
    offset += 12
    var bounds_max := Vector3(raw.decode_float(offset), raw.decode_float(offset + 4), raw.decode_float(offset + 8))
    offset += 12
    ...
    for group_index: int in range(group_count):
        ...
        for vertex_index: int in range(vertex_count):
            ...
            offset += 6
            vertices[vertex_index] = bounds_min + Vector3(float(qx) / 65535.0 * span.x, float(qy) / 65535.0 * span.y, float(qz) / 65535.0 * span.z)
    ...
    root.set_meta("source_interior_retained", false)
    return root
...
static func _material_for(material_name: String) -> StandardMaterial3D:
    ...
    if "glass" in key:
        material.albedo_color = Color(0.055, 0.09, 0.12, 0.40)
        if "red" in key:
            ...
            material.albedo_color = Color(0.30, 0.38, 0.42, 0.30)
        material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
        material.cull_mode = BaseMaterial3D.CULL_DISABLED
        material.roughness = 0.14
    elif "paint" in key or "car-body" in key:
        material.albedo_color = Color(0.91, 0.92, 0.93, 1.0)
        ...
        material.albedo_color = Color(0.02, 0.035, 0.05, 1.0)
        material.emission_enabled = true
        material.emission = Color(0.03, 0.16, 0.26, 1.0)
