...
static func resolve_ranged_animation(names: PackedStringArray, hand_mode: String, state: String) -> String:
    ...
    # KayKit Adventurers 1.0 has no dedicated Ranged_Idle clips. Its authored
    # stationary weapon-ready pose is Ranged_Aiming, so use that only for idle.
    # Walk/run intentionally fall back to normal locomotion instead of gliding
    # a stationary aiming animation across the world.
