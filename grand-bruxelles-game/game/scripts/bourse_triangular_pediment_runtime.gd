...
func _read_candidate() -> Dictionary:
    ...
    if str(data.get("schema", "")) != "grand-bruxelles-bourse-triangular-pediment-candidate-v2":
...
func _make_pediment_mesh(center: Vector3, tangent: Vector3, toward_camera: Vector3, width: float, rise: float, depth: float, base_y: float) -> ArrayMesh:
    var half_width: float = width * 0.5
    var half_depth: float = depth * 0.5
    var front_center: Vector3 = center + toward_camera * half_depth
    var back_center: Vector3 = center - toward_camera * half_depth
...
func _build_candidate() -> void:
    var data: Dictionary = _read_candidate()
    if data.is_empty():
        return
    var source_contract: Dictionary = data.get("source_contract", {}) as Dictionary
    if not str(source_contract.get("heritage_fact", "")).contains("triangular pediment"):
        _fail("source contract does not prove triangular pediment identity")
        return
    var envelope: Dictionary = data.get("authoritative_front_envelope", {}) as Dictionary
    var visual: Dictionary = data.get("provisional_visualization", {}) as Dictionary
    var hard: Dictionary = data.get("hard_invariants", {}) as Dictionary
    if envelope.is_empty() or visual.is_empty() or hard.is_empty():
        _fail("candidate missing envelope, visualization or hard invariants")
        return
    for key: String in ["source_front_vertices_changed", "building_footprint_changed", "building_height_changed", "collision_changed", "survey_dimensions_claimed", "realism_complete"]:
        if bool(hard.get(key, true)):
            _fail("hard invariant violated: %s" % key)
            return
    ...
    if width <= 0.0 or width >= span or rise <= 0.0 or depth <= 0.0 or base_y <= 0.0 or base_y + rise >= y_max:
        _fail("pediment escapes authoritative envelope")
        return
    var plane := Vector3(plane_xz.x, 0.0, plane_xz.y)
    var center := plane + tangent * ((t_min + t_max) * 0.5) + toward_camera * forward_offset
    _material = WHITE_STONE.create(Color(0.72, 0.70, 0.65, 1.0), Color(0.84, 0.81, 0.74, 1.0), 0.84, "Bourse triangular pediment heritage identity; exact geometry provisional")
    ...
    print("Bourse triangular pediment candidate: width=%.3f rise=%.3f top_y=%.3f span=%.3f runtime_approved=false" % [width, rise, base_y + rise, span])
