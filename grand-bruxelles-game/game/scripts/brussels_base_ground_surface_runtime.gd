...
const PRESENTATION_REVISION := 8
const CALIBRATION_REVISION := 2
const VISUAL_RECIPE_PROFILE := "authored_near_field_isotropic_variation_v8"
...
uniform float tone_contrast_gain : hint_range(1.0, 2.0) = 1.42;
...
void fragment() {
    ...
    float broad_a = value_noise(p_a * 0.74 + vec2(19.0, 37.0));
    float broad_b = value_noise(p_b * 0.97 + vec2(71.0, 11.0));
    float broad_c = value_noise(p_c * 1.21 + vec2(43.0, 67.0));
    float broad = (broad_a + broad_b + broad_c) / 3.0;

    float fine_a = value_noise(p_a * 2.35 + vec2(31.0, 83.0));
    float fine_b = value_noise(p_b * 3.05 + vec2(97.0, 53.0));
    float fine_c = value_noise(p_c * 3.85 + vec2(59.0, 29.0));
    float fine = (fine_a + fine_b + fine_c) / 3.0;

    float camera_distance = distance(world_pos, CAMERA_POSITION_WORLD);
    float near_detail = 1.0 - smoothstep(12.0, 58.0, camera_distance);
    float distance_visibility = 1.0 - smoothstep(34.0, 102.0, camera_distance);
    float detail_weight = mix(0.58, 0.72, near_detail);
    float raw_ground_tone = mix(broad, fine, detail_weight);
    float centered_ground_tone = raw_ground_tone - 0.5;

    // Anti-banding v8: far-field authored contrast reaches exactly zero.
    // This prevents world-space noise from collapsing into horizon-aligned stripes.
    float distance_contrast = distance_visibility;
    ...
    ROUGHNESS = clamp(base_roughness + (0.5 - fine) * 0.018 * near_detail * distance_visibility, 0.90, 0.96);
    METALLIC = 0.0;
    SPECULAR = 0.08;
}
"""
    ...
    material.set_meta("anti_banding_revision", 2)
    material.set_meta("distance_contrast_fade", true)
    material.set_meta("far_field_contrast_zero", true)
...
func _apply_when_ready() -> void:
    ...
    if not _vectors_match(_ground.position, EXPECTED_POSITION) or not _vectors_match(_ground.size, EXPECTED_SIZE):
        push_error("Brussels base-ground surface runtime: Ground geometry drifted")
        _failed = true
        _ready_complete = true
        return
    ...
    _ready_complete = true
    print("BRUSSELS_BASE_GROUND_SURFACE_READY: family=%s revision=%d calibration=%d profile=%s anti_banding=2 far_field_contrast_zero=true material_only=true geometry_changed=false collision_changed=false" % [MATERIAL_FAMILY, PRESENTATION_REVISION, CALIBRATION_REVISION, VISUAL_RECIPE_PROFILE])
