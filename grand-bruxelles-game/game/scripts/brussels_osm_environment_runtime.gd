...
func _rebuild(anchor: Vector3) -> void:
    # Detach old batches immediately before deferred deletion. Without this,
    # rebuilding in the same frame leaves duplicate sibling names attached and
    # Godot auto-renames the replacement MultiMesh nodes, breaking deterministic
    # batch identity even though the rendered geometry is otherwise correct.
