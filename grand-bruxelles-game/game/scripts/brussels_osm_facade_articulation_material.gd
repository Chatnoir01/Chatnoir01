...
## Presentation-only articulation layered on the existing generic OSM facade family.
## OSM/Overpass supports generic building footprint/placement/kind only.
## The directional tonal response below is authored from existing mesh normals
## and world position. It does not claim windows, masonry units, facade material,
## weathering, measured RGB or photometric reflectance.
...
void fragment() {
    ...
    float authored_plane_tone = (orientation - 0.5) * 0.28;
    float tone = clamp(baseline_tone + authored_plane_tone, -0.24, 0.24);
    ALBEDO = clamp(base_color.rgb * (1.0 + tone), vec3(0.0), vec3(1.0));
    ROUGHNESS = clamp(base_roughness + (0.5 - broad) * 0.035 - authored_plane_tone * 0.08, 0.85, 0.97);
    METALLIC = 0.0;
    SPECULAR = 0.14;
}
"""
