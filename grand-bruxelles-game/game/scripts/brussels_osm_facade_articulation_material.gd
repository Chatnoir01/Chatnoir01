...
## Material-only articulation layer for the already-rendered generic OSM walls.
## OSM/Overpass supports the generic building footprint/placement/kind consumed
## by production. The directional tonal response below is authored presentation
## from the existing mesh normal and world position only: it does not claim
## windows, masonry units, facade material, weathering, measured RGB or
...
void fragment() {
    ...
    float existing_tone = clamp((broad - 0.5) * 0.22 + (medium - 0.5) * 0.07, -0.12, 0.12);

    vec2 horizontal_normal = world_normal.xz;
    float horizontal_length = length(horizontal_normal);
    float orientation = 0.5;
    if (horizontal_length > 0.001) {
        vec2 n = horizontal_normal / horizontal_length;
        orientation = abs(dot(n, normalize(vec2(0.78, 0.625))));
    }
    float authored_plane_tone = (orientation - 0.5) * 0.12;
    float tone = clamp(existing_tone + authored_plane_tone, -0.17, 0.17);

    ALBEDO = clamp(base_color.rgb * (1.0 + tone), vec3(0.0), vec3(1.0));
    ROUGHNESS = clamp(base_roughness + (0.5 - broad) * 0.035 - authored_plane_tone * 0.10, 0.85, 0.97);
    METALLIC = 0.0;
    SPECULAR = 0.14;
}
"""
