...
func geometry_unchanged() -> bool:
    for building: CSGPolygon3D in _buildings:
        ...
        if not building.global_transform.is_equal_approx(_original_transforms.get(id, Transform3D.IDENTITY)):
            return false
        if building.polygon != (_original_polygons.get(id, PackedVector2Array()) as PackedVector2Array):
            return false
        if not is_equal_approx(building.depth, float(_original_depths.get(id, -1.0))):
            return false
    return true
