...
uniform float micro_grain_strength : hint_range(0.0, 1.0) = 0.76;
...
static func _material(shader: Shader, dark: Color, light: Color, roughness: float, role: String) -> ShaderMaterial:
    ...
    material.set_shader_parameter("micro_grain_strength", 0.76)
...
static func create_materials() -> Dictionary:
    var shader := _shader()
    return {
        "regular": _material(shader, Color(0.082, 0.087, 0.092, 1.0), Color(0.135, 0.140, 0.145, 1.0), 0.96, "regular"),
        "major": _material(shader, Color(0.060, 0.065, 0.070, 1.0), Color(0.112, 0.117, 0.122, 1.0), 0.95, "major"),
    }
