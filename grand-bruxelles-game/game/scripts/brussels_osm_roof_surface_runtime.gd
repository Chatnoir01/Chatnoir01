...
func _apply_to_root(root: Node) -> void:
    ...
    if added > 0:
        print(
            "BRUSSELS_OSM_ROOF_SURFACE_READY: roofs=%d family=%s source=OSM license=ODbL-1.0 geometry_changed=false" %
            [_live_roof_count(), BrusselsOsmRoofSurfaceMaterial.MATERIAL_FAMILY]
        )
