...
# Shared presentation only for generic dashed lane markings already authored
# inside OSMCityBuilder/GeneratedRoads. No marking geometry is created, moved,
# resized or reinterpreted; Centre-owned exact-location markings are excluded.
...
func _is_generic_lane_dash(node: Node) -> bool:
    ...
    return absf(box.size.x - TARGET_WIDTH_M) <= 0.001 and absf(box.size.y - TARGET_THICKNESS_M) <= 0.001 and box.size.z > 0.0 and box.size.z <= TARGET_MAX_LENGTH_M + 0.001
...
func _make_paint_texture() -> ImageTexture:
    ...
    for tile_y: int in range(8):
        for tile_x: int in range(8):
            ...
            var patch := Color(clampf(base.r + delta, 0.0, 1.0), clampf(base.g + delta, 0.0, 1.0), clampf(base.b + delta * 0.75, 0.0, 1.0), 1.0)
