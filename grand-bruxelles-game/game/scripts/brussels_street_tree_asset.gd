...
## bounded deterministic per-tree crown scale variation. These are presentation-only
...
static func foliage_tone_variation(osm_id: int) -> float:
    ## Deterministic presentation hint only. It intentionally does not encode
    ## species, season, health or measured RGB.
