...
func _perform_ixelles_fast_travel(player: Node) -> void:
    ...
    for _frame_index: int in range(120):
        ...
        if _backend.has_active_instance(CELL_ID):
            ...
            if is_instance_valid(instance) and bool(instance.get("runtime_loaded")) and bool(instance.call("is_streamed_collision_enabled")):
                break
    if not is_instance_valid(instance) or not bool(instance.get("runtime_loaded")):
        ...
        return
    if not bool(instance.call("is_streamed_collision_enabled")):
...
func _ixelles_source_backed_spawn(instance: Node) -> Dictionary:
    ...
    return {"position": Vector3(camera_xz.x, ground + IXELLES_BODY_CLEARANCE_M, camera_xz.y), "yaw_degrees": yaw}
...
func get_metrics() -> Dictionary:
    return {"registered": _registered, "ixelles_center": _ixelles_world_center, "scheduler": _manager.get_metrics() if is_instance_valid(_manager) else {}, "backend": _backend.get_metrics() if is_instance_valid(_backend) else {}}
