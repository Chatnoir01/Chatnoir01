...
## compatibility contracts; it is not the regional streaming source of truth.
...
const SHIPPED_CELLS: Array[Dictionary] = [
    ...
    },
    {
        ...
        "script_path": IXELLES_NORTH_DTM_STREAMED_SCRIPT_PATH,
        "destination_collision_authorized": true,
        "metadata": {"build_collision": false},
    },
    {
...
func _descriptor_for_runtime_cell(root_path: String, cell_id: String) -> Dictionary:
    ...
    if not shipped.is_empty():
        script_path = str(shipped.get("script_path", SOURCE_PLAN_STREAMED_SCRIPT_PATH))
        var shipped_metadata: Variant = shipped.get("metadata", {})
        if shipped_metadata is Dictionary:
            metadata = (shipped_metadata as Dictionary).duplicate(true)
