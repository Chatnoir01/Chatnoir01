...
const CROSSBOW_CARRY_DEG := Vector3(-12.0, 24.0, 5.0)
const MAX_CROSSBOW_HAND_REGION_GAP_M := 0.12

var _bound_player_id := 0
var _bound_crossbow_id := 0
var _crossbow_hand_offset_player := Vector3.ZERO
...
func _present_crossbow(player: CharacterBody3D) -> void:
    ...
    if owner_changed or not _crossbow_offset_ready:
        ...
        _crossbow_hand_offset_player = player.global_transform.basis.inverse() * (crossbow.global_position - hand_transform.origin)
        _crossbow_offset_ready = true

    var transition := _transition_weight(player)
    var carry := CROSSBOW_CARRY_DEG + Vector3(22.0, 18.0, 12.0) * transition
    var recoil := _shot_recoil_weight(player)
    carry.x -= 5.0 * recoil
    crossbow.global_rotation = player.global_rotation + Vector3(
        deg_to_rad(carry.x),
        deg_to_rad(carry.y),
        deg_to_rad(carry.z)
    )
    var transition_offset := Vector3(0.15, -0.31, 0.22) * transition
    crossbow.global_position = hand_transform.origin + player.global_transform.basis * (_crossbow_hand_offset_player + transition_offset)
    ...
    crossbow.set_meta("combat_native_crossbow_carry_rotation_deg", carry)
    player.set_meta("combat_native_weapon_presentation_signature", SIGNATURE)
    player.set_meta("combat_native_crossbow_carry_rotation_deg", carry)
...
func _clear_binding() -> void:
    ...
    _crossbow_hand_offset_player = Vector3.ZERO
