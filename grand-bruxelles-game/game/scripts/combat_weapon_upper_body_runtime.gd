...
# LOT 2: authored-player upper-body weapon stance.
# This layer never owns weapon grip geometry, damage, firing or camera logic.
# It only drives the real authored arm chains toward carry/support targets.

const SIGNATURE := "combat_weapon_upper_body_v2"
...
func _resolve_forearm_bone(skeleton: Skeleton3D, hand_index: int) -> int:
    ...
    # Common authored rigs insert a wrist bone between hand and lower arm.
    # Skip that wrist in the fallback so the IK root can be the upper arm.
    if fallback >= 0:
...
func _update_pose(delta: float) -> void:
    ...
    var right_influence := 0.98 if aiming else 0.92
    var left_influence := 0.99 if aiming else 0.96
    if reloading:
        right_influence = 0.68
        left_influence = 0.28
...
func _resolve_right_target(weapon_id: StringName, aiming: bool) -> Vector3:
    var carry_world := _player.global_transform * carry_target_local(weapon_id, aiming)
    var holder := _player.get_node_or_null("CombatWeaponVisual") as Node3D
    if holder == null:
        return carry_world

    # When LOT 1 is present the weapon already follows hand.r, so the body pose
    # owns the carry position and the weapon will follow it. On production main
    # without LOT 1, blend toward the existing visible grip so LOT 2 remains
    # independently reviewable and does not teleport the hand away from the gun.
    if bool(_player.get_meta("combat_weapon_grip_locked", false)):
        return carry_world
    var visible_grip := holder.global_transform * right_hand_grip_local(weapon_id)
    return visible_grip.lerp(carry_world, 0.72 if aiming else 0.62)
...
func _resolve_support_target(weapon_id: StringName, right_world: Vector3) -> Vector3:
    ...
    var local_offset := support_offset_from_right(weapon_id)
    return right_world + _player.global_transform.basis * local_offset
...
func _update_magnets(aiming: bool) -> void:
    if not is_instance_valid(_right_ik) or not is_instance_valid(_left_ik):
        return
    var forward_z := -0.30 if aiming else -0.16
    var right_world := _player.global_transform * Vector3(0.54, 1.17, forward_z)
    var left_world := _player.global_transform * Vector3(-0.54, 1.17, forward_z - 0.10)
...
static func carry_target_local(weapon_id: StringName, aiming: bool) -> Vector3:
    if aiming:
        match weapon_id:
            &"bx9":
                return Vector3(0.23, 1.48, -0.43)
            &"cbr4":
                return Vector3(0.27, 1.45, -0.35)
            &"sct8":
                return Vector3(0.28, 1.43, -0.33)
            _:
                return Vector3(0.27, 1.44, -0.34)
    match weapon_id:
        &"bx9":
            return Vector3(0.29, 1.34, -0.30)
        &"cbr4":
            return Vector3(0.31, 1.36, -0.27)
        &"sct8":
            return Vector3(0.32, 1.34, -0.25)
        _:
            return Vector3(0.31, 1.35, -0.27)
...
static func support_grip_local(weapon_id: StringName) -> Vector3:
    match weapon_id:
        &"bx9":
            return Vector3(-0.07, -0.08, -0.16)
        &"cbr4":
            return Vector3(-0.04, -0.01, -0.52)
        &"sct8":
            return Vector3(-0.04, -0.03, -0.50)
        _:
            return Vector3(-0.05, -0.03, -0.42)

static func support_offset_from_right(weapon_id: StringName) -> Vector3:
    match weapon_id:
        &"bx9":
            return Vector3(-0.18, 0.01, -0.12)
        &"cbr4":
            return Vector3(-0.22, 0.02, -0.40)
        &"sct8":
            return Vector3(-0.23, 0.00, -0.38)
        _:
