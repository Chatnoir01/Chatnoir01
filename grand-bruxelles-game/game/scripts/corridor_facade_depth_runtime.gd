...
## lintels, sills, jambs, shop canopies and shopfront glass exclusively from
## existing visible window/shop transforms generated by osm_city_builder.gd.
...
func _shop_glass_index(instance_index: int) -> int:
    # A different cycle from the canopy palette prevents shop glass and canopy
    # families from marching in lockstep while remaining reproducible.
...
func _build_shopfront_glass(details: Node3D, shop_node: MultiMeshInstance3D) -> void:
    ...
    # Keep the source MultiMesh as the canonical transform reference for tests
    # and future styling, but hide its uniform material to avoid double draw.
...
func _build() -> void:
    ...
    if articulation_ready:
        print("CORRIDOR_FACADE_DEPTH_READY: windows=%d lintels=%d sills=%d jambs=%d canopies=%d canopy_groups=%d glass=%d glass_groups=%d" % [windows.instance_count, lintel_count, sill_count, jamb_count, canopy_count, canopy_material_group_count, glass_count, glass_material_group_count])
    else:
