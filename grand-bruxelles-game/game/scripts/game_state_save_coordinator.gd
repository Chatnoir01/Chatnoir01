...
static func load_domains(path: String, providers: Dictionary) -> Dictionary:
    ...
    # Phase 1: validate every requested domain before mutating any live state.
    for key_value in keys:
        var key := str(key_value)
    ...
    # Phase 2: apply in deterministic order. If a provider unexpectedly fails,
    # restore every domain already touched from the snapshots captured above.
