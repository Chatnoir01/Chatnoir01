...
func _build_for_scene(scene: Node) -> void:
    ...
    for owner_id: String in EXPECTED_OWNER_IDS:
        ...
        var bounds := _horizontal_bounds(faces)
        _mask_replaced_osm(owner_id, bounds, scene)
    ...
    print("GRAND_PLACE_COMPLETE_CONTOUR_READY: owners=%d source_faces=%d source_triangles=%d render_triangles=%d collisions=%d masked_osm=%d neutral=true" % [loaded_owner_ids.size(), source_face_count, source_triangle_count, render_triangle_count, collision_body_count, masked_osm_count])
...
func _read_owner(owner_id: String) -> Dictionary:
    ...
    if str(data.get("schema", "")) != SOURCE_SCHEMA:
        push_error("Grand-Place contour schema mismatch: %s" % owner_id)
        return {}
    if bool(data.get("runtime_approved", true)):
        push_error("Grand-Place contour source unexpectedly runtime-approved: %s" % owner_id)
        return {}
    ...
    if str(source.get("package_sha256", "")) != PACKAGE_SHA256:
        push_error("Grand-Place contour package digest mismatch: %s" % owner_id)
        return {}
    var evidence: Dictionary = data.get("evidence", {})
    var faces: Array = data.get("faces", [])
    if int(evidence.get("face_count", 0)) <= 0 or int(evidence.get("triangle_count", 0)) <= 0:
        push_error("Grand-Place contour empty evidence: %s" % owner_id)
        return {}
    if faces.size() != int(evidence.get("face_count", 0)):
    ...
    for raw_face: Variant in faces:
        if typeof(raw_face) != TYPE_DICTIONARY:
            push_error("Grand-Place contour malformed face: %s" % owner_id)
            return {}
