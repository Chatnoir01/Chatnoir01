...
func _build_when_ready() -> void:
    ...
    set_meta("three_second_goal","broad full-owner frontage mass and skyline readability")
    _sync_visibility(true)
    set_process(true)
    print("GRAND_PLACE_FACADE_FRONTAGE_MASS_V6_READY: cornet_renard=%d brasseurs=%d rose=%d mont_thabor=%d features=%d collisions=0 lateral_endpoint_extension=false" % [cornet_renard_mass_count,brasseurs_mass_count,rose_mass_count,mont_thabor_mass_count,feature_count])
...
func _build_brasseurs_mass() -> bool:
    ...
    if left == null or right == null or entablature == null:
    ...
    _add_box("BrasseursFullWidthLowerOrder",Vector3(center.x,base_y,center.z)+normal*0.20,tangent,normal,Vector3(span,0.46,0.38),stone,"full-width lower classical order")
    _add_box("BrasseursFullWidthEntablature",Vector3(center.x,top_y,center.z)+normal*0.24,tangent,normal,Vector3(span,0.58,0.42),stone,"powerful full-width entablature")
    _add_box("BrasseursAtticMass",Vector3(center.x,top_y+0.72,center.z)+normal*0.18,tangent,normal,Vector3(span*0.86,0.72,0.34),stone,"attic mass within native owner span")
    _add_box("BrasseursPedimentShoulder",Vector3(center.x,top_y+1.46,center.z)+normal*0.20,tangent,normal,Vector3(span*0.68,0.42,0.36),stone,"fronton shoulder mass")
    _add_box("BrasseursPedimentCrown",Vector3(center.x,top_y+2.02,center.z)+normal*0.22,tangent,normal,Vector3(span*0.38,0.48,0.38),stone,"fronton crown mass")
...
func _build_rose_mass() -> bool:
    ...
    if owner == null:
        _fail("Rose owner missing"); return false
    var left := owner.get_node_or_null("Pilaster_0") as MeshInstance3D
    ...
    for level: int in range(3):
        ...
        _add_box("RoseFullWidthOrderBand_%d"%level,Vector3(center.x,y,center.z)+normal*(0.18+0.03*float(level)),tangent,normal,Vector3(span,0.34+0.04*float(level),0.34),stone,"superposed order register band")
        rose_mass_count += 1
    for i: int in range(3):
        var y := mid_y-h*0.15+h*0.30*float(i)
        _add_box("RoseCentralOrderProjection_%d"%i,Vector3(center.x,y,center.z)+normal*0.36,tangent,normal,Vector3(span*0.44,h*0.23,0.42),stone,"central order projection within native owner span")
        rose_mass_count += 1
    return true
...
func _build_mont_thabor_mass() -> bool:
    ...
    if owner == null:
        _fail("Mont Thabor owner missing"); return false
    var left := owner.get_node_or_null("Pilaster_0") as MeshInstance3D
    var right := owner.get_node_or_null("Pilaster_3") as MeshInstance3D
    ...
    var normal := frame.normal as Vector3
    var center := frame.center as Vector3
    var stone := _stone("Urban Brussels 30907 / exact owner 1646728: sober classical frontage and documented gable cue; all mass stays inside native facade span")
    var base_y := upper.global_position.y+_box_world_size(upper).y*0.62
    var widths := [1.0,0.82,0.64,0.46,0.28]
    for i: int in range(5):
        _add_box("MontThaborFullGableMass_%d"%i,Vector3(center.x,base_y+0.34*float(i),center.z)+normal*(0.17+0.02*float(i)),tangent,normal,Vector3(span*float(widths[i]),0.28,0.34),stone,"stepped native-span gable mass")
