...
func _build_when_ready() -> void:
    ...
    set_process(true)
    print("GRAND_PLACE_FACADE_INTEGRATED_V5_READY: balcony_depth=%.2f balusters=%d maison_glazing=%d maison_piers=%d brasseurs=%d rose_orders=%d mont_thabor=%d collisions=0 lateral_stretch=false" % [RENARD_BALCONY_DEPTH_M,renard_baluster_count,maison_glazing_panel_count,maison_structural_pier_count,brasseurs_signature_count,rose_order_signature_count,mont_thabor_signature_count])
