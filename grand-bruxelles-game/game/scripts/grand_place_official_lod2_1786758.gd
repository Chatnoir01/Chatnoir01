...
func _read_geometry() -> Dictionary:
    if not FileAccess.file_exists(GEOMETRY_PATH):
        push_error("Grand-Place LoD2 1786758 geometry missing")
        return {}
    var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(GEOMETRY_PATH))
    if typeof(parsed) != TYPE_DICTIONARY:
        push_error("Grand-Place LoD2 1786758 JSON invalid")
        return {}
    ...
    if str(data.get("schema", "")) != "grand-bruxelles-urbis-context-mesh-v1":
        push_error("Grand-Place LoD2 1786758 schema mismatch")
        return {}
    if str(source.get("building_2d_id", "")) != BUILDING_ID:
        push_error("Grand-Place LoD2 1786758 building identity mismatch")
        return {}
    if str(source.get("crs", "")) != "EPSG:31370" or str(source.get("license", "")) != "CC0-1.0":
        push_error("Grand-Place LoD2 1786758 provenance drifted")
        return {}
    if str(source.get("package_sha256", "")) != PACKAGE_SHA256:
        push_error("Grand-Place LoD2 1786758 package digest drifted")
        return {}
    if int(evidence.get("face_count", 0)) != 82 or int(evidence.get("triangle_count", 0)) != 196:
        push_error("Grand-Place LoD2 1786758 geometry contract drifted")
        return {}
    var types: Dictionary = evidence.get("face_type_counts", {})
    if int(types.get("WALLSURFACE", 0)) != 19 or int(types.get("ROOFSURFACE", 0)) != 62 or int(types.get("GROUNDSURFACE", 0)) != 1:
        push_error("Grand-Place LoD2 1786758 face-type contract drifted")
        return {}
    if bool(data.get("runtime_approved", true)):
        push_error("Grand-Place LoD2 1786758 evidence must remain non-approved")
        return {}
    return data
    ...
    for raw_face: Variant in faces:
        ...
        for raw_triangle: Variant in raw_face.get("triangles", []):
            ...
            for vertex: Vector3 in [a, b, c]:
                tool.set_normal(normal); tool.add_vertex(vertex)
            count += 1
    return count
