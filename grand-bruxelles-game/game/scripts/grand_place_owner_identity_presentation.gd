...
const TWO_SIDED_SOURCE_OWNER_IDS := ["1654360"]
...
func _apply_when_ready() -> void:
    ...
    set_meta("source_winding_mitigation_owner_ids", TWO_SIDED_SOURCE_OWNER_IDS.duplicate())
    set_meta("source_winding_mitigation_enabled", true)
    set_meta("finished_perfect", false)
    print("GRAND_PLACE_OWNER_IDENTITY_PRESENTATION_READY owners=%d surfaces=%d geometry_changed=false collisions_changed=false focus_contrast_guarded=true focus_visibility_guarded=true two_sided_source_owners=%d" % [OWNER_PRESENTATION.size(), owner_surface_count, TWO_SIDED_SOURCE_OWNER_IDS.size()])
...
func _apply_material(surface: MeshInstance3D, owner_id: String, is_roof: bool) -> void:
    ...
    mat.cull_mode = BaseMaterial3D.CULL_DISABLED if mitigation_enabled else BaseMaterial3D.CULL_BACK
...
func set_source_winding_mitigation_enabled(enabled: bool) -> bool:
    if not built or _contour == null:
        _fail("source winding mitigation toggle requested before presentation readiness")
        return false
    for owner_id: String in TWO_SIDED_SOURCE_OWNER_IDS:
        for suffix: String in ["WALLSURFACE", "ROOFSURFACE"]:
            ...
            if surface == null:
                _fail("source winding mitigation surface missing: %s %s" % [owner_id, suffix])
                return false
            var mat := surface.material_override as StandardMaterial3D
            if mat == null or str(mat.get_meta("urbis_owner_id", "")) != owner_id or not bool(mat.get_meta("source_winding_mitigation", false)):
                _fail("source winding mitigation material ownership drifted: %s %s" % [owner_id, suffix])
                return false
