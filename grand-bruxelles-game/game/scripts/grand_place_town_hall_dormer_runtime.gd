...
func _build_materials() -> void:
    ...
    _slate = BrusselsSlateRoofMaterial.create(
        Color(0.045, 0.055, 0.068, 1.0),
        Color(0.145, 0.148, 0.155, 1.0),
        0.74,
        source_label
    )
    _stone = BrusselsWhiteStoneMaterial.create(
        Color(0.70, 0.68, 0.62, 1.0),
        Color(0.84, 0.81, 0.73, 1.0),
        0.82,
        source_label
    )
...
func _add_hipped_dormer(anchor: Vector3, row_index: int, index_in_row: int) -> void:
    ...
    # Four-piece pale-stone surround keeps the opening readable at gameplay distance.
    _push_quad(stone_vertices,
        front_bottom_left,
        front_bottom_right,
        front_bottom_right + UP * frame,
        front_bottom_left + UP * frame)
    _push_quad(stone_vertices,
        front_top_left - UP * frame,
        front_top_right - UP * frame,
        front_top_right,
        front_top_left)
    _push_quad(stone_vertices,
        front_bottom_left + UP * frame,
        front_bottom_left + EAVE_TANGENT * frame + UP * frame,
        front_top_left + EAVE_TANGENT * frame - UP * frame,
        front_top_left - UP * frame)
    _push_quad(stone_vertices,
        front_bottom_right - EAVE_TANGENT * frame + UP * frame,
        front_bottom_right + UP * frame,
        front_top_right - UP * frame,
        front_top_right - EAVE_TANGENT * frame - UP * frame)

    # Shallow cheeks connect the projecting vertical front back into the official roof plane.
    ...
    _push_quad(
        glass_vertices,
        front_bottom_left + EAVE_TANGENT * frame + UP * frame + glass_offset,
        front_bottom_right - EAVE_TANGENT * frame + UP * frame + glass_offset,
        front_top_right - EAVE_TANGENT * frame - UP * frame + glass_offset,
        front_top_left + EAVE_TANGENT * frame - UP * frame + glass_offset
    )
    _add_mesh(dormer_root, "Glazing", glass_vertices, _glass)

    # A true four-sided hipped cap: front/back roof slopes plus triangular side hips.
...
func source_truth() -> Dictionary:
    return {
        "urbis_roof_face_id": TARGET_FACE_ID,
