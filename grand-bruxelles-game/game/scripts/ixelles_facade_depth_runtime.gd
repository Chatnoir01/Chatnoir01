...
static func _point_in_ring(point: Vector2, ring: PackedVector2Array) -> bool:
    if ring.size() < 3:
        return false
    var inside := false
    var previous := ring.size() - 1
    for index: int in range(ring.size()):
        var current_point := ring[index]
        var previous_point := ring[previous]
        if (current_point.y > point.y) != (previous_point.y > point.y):
            var x_cross := (previous_point.x - current_point.x) * (point.y - current_point.y) / (previous_point.y - current_point.y) + current_point.x
            if point.x < x_cross:
                inside = not inside
        previous = index
    return inside
...
static func _outward_for_edge(a: Vector2, b: Vector2, ring: PackedVector2Array) -> Vector2:
    var edge := b - a
    if edge.length_squared() <= 0.000001:
        return Vector2.ZERO
    var left_normal := Vector2(-edge.y, edge.x).normalized()
    var midpoint := (a + b) * 0.5
    var left_inside := _point_in_ring(midpoint + left_normal * OUTSIDE_PROBE_M, ring)
    var right_inside := _point_in_ring(midpoint - left_normal * OUTSIDE_PROBE_M, ring)
    if left_inside and not right_inside:
        return -left_normal
    if right_inside and not left_inside:
        return left_normal
    # Degenerate/touching probe fallback: polygon winding resolves the exterior
    # without assuming that a simple-average centroid lies inside a concave footprint.
    return -left_normal if _signed_area(ring) > 0.0 else left_normal
...
static func _build_feature_transforms(
    ...
    for edge_index: int in range(ring.size()):
        ...
        var outward := _outward_for_edge(a, b, ring)
...
static func build_from_contract(
    ...
    root.set_meta("edge_outward_resolution", "polygon_side_test")
