...
func _on_node_added(node: Node) -> void:
    ...
    if node == null or not is_instance_valid(node) or node.name != TARGET_NAME:
        return
    # Do not transport a live Node through MessageQueue: node_added can fire
    # before another owner removes/frees the target. Defer only its stable ID.
    call_deferred("_apply_candidate", node.get_instance_id())

func _apply_candidate(instance_id: int) -> void:
    if _tearing_down or not is_inside_tree():
        return
    # Godot documents instance IDs as the safe way to retain non-owned Objects.
    # Validate the ID before resolving it so a freed target cannot make
    # instance_from_id() touch an invalid object during deferred execution.
