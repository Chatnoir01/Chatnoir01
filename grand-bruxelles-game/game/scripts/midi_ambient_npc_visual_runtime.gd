...
const LOD_SWITCH_DISTANCE_M := 170.0
const WEB_LOD_SWITCH_DISTANCE_M := 120.0
...
func bridge_scene(scene: Node) -> Dictionary:
	...
	print("Midi ambient NPC visual bridge: bridged=%d already=%d legacy_hidden=%d materials_reused=%d cache_entries=%d lod_detailed=%d legacy_nodes=%d legacy_visible=%d profiled_end_m=%.1f" % [bridged, already, legacy_hidden, materials_reused, _shared_materials.size(), int(lod.get("detailed_meshes", 0)), int(lod.get("legacy_visuals", 0)), int(lod.get("legacy_visible", 0)), _profiled_visual_distance_m()])
	return {
		"bridged": bridged,
		"already": already,
...
func _configure_distance_lod(person: Node3D, profiled_enabled: bool) -> void:
	var profiled_end := _profiled_visual_distance_m()
	var proxy := person.get_node_or_null("ProfiledNpcProxy") as Node3D
	if proxy != null:
		...
		if visual != null:
			for node: Node in visual.find_children("*", "MeshInstance3D", true, false):
				...
				if profiled_enabled:
					...
					mesh_instance.visibility_range_end = profiled_end
	...
	# Never expose the old box/sphere pedestrian body as a distance LOD. Past the
	# profiled range the pedestrian is culled instead of visibly reverting to a cuboid.
...
func _profiled_visual_distance_m() -> float:
	if OS.has_feature("web"):
		return WEB_LOD_SWITCH_DISTANCE_M
	return LOD_SWITCH_DISTANCE_M
...
func _set_legacy_visuals(person: Node3D, visible_value: bool) -> int:
	var changed := 0
	# `visible_value` is intentionally ignored when true: cuboid bodies are retired
	# from the production visual path and must never become a fallback again.
	var safe_visible := false
	for legacy_name: String in LEGACY_VISUAL_NAMES:
		var legacy := person.get_node_or_null(legacy_name)
		if legacy is VisualInstance3D:
			var visual := legacy as VisualInstance3D
			if visual.visible != safe_visible:
				visual.visible = safe_visible
				changed += 1
	return changed
...
func lod_stats() -> Dictionary:
	...
	counts["switch_distance_m"] = _profiled_visual_distance_m()
	counts["transition_margin_m"] = LOD_TRANSITION_MARGIN_M
	counts["detailed_near_legacy_far"] = false
	counts["profiled_only_distance_culled"] = true
	return counts
...
func truth_contract() -> Dictionary:
	return {
		...
		"distance_lod": "profiled humanoid only; culled beyond platform budget; legacy cuboid primitives never rendered",
