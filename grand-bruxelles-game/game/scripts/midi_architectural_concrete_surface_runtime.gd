...
const FONSNY_CANOPY_RUNTIME := preload("res://game/scripts/midi_fonsny_perforated_canopy_runtime.gd")
...
var _fonsny_canopy_runtime: Node

func _ready() -> void:
    _fonsny_canopy_runtime = FONSNY_CANOPY_RUNTIME.new()
    _fonsny_canopy_runtime.name = "MidiFonsnyPerforatedCanopyRuntime"
    add_child(_fonsny_canopy_runtime)
...
func _read_identity() -> Dictionary:
    if not FileAccess.file_exists(IDENTITY_PATH):
        push_error("Midi concrete runtime: material identity missing")
        _identity_failure = true
        return {}
    var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(IDENTITY_PATH))
    if typeof(parsed) != TYPE_DICTIONARY:
        push_error("Midi concrete runtime: material identity invalid")
        _identity_failure = true
        return {}
...
func _runtime_identity_allowed(identity: Dictionary) -> bool:
    ...
    if str(contract.get("material_identity", "")) != "architectural_concrete":
        return false
    if bool(contract.get("geometry_changed", true)) or bool(contract.get("new_location_specific_placement", true)):
        return false
    if bool(contract.get("formwork_pattern_authored", true)) or bool(contract.get("aggregate_scale_claimed", true)):
        return false
...
func fonsny_canopy_runtime() -> Node:
    return _fonsny_canopy_runtime
