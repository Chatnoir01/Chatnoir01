...
func _ready() -> void: call_deferred("_apply_when_ready")

func _apply_when_ready() -> void:
    await get_tree().process_frame
    _identity = _read_identity()
    if _identity.is_empty(): _ready_complete = true; return
    ...
    if midi == null:
        push_error("Midi glazing runtime: MidiHeroZone missing"); _identity_failure = true; _ready_complete = true; return
    if bool(midi.get_meta("midi_urbis_envelope_owner", false)):
        _urbis_owner_skip = true
        _ready_complete = true
        print("Midi glazing runtime: SKIP legacy station-mass surfaces; UrbISMidiExact owns station masses")
        return
...
func _read_identity() -> Dictionary:
    if not FileAccess.file_exists(IDENTITY_PATH): push_error("Midi glazing runtime: material identity missing"); _identity_failure = true; return {}
    var parsed: Variant = JSON.parse_string(FileAccess.get_file_as_string(IDENTITY_PATH))
    if typeof(parsed) != TYPE_DICTIONARY: push_error("Midi glazing runtime: material identity invalid"); _identity_failure = true; return {}
...
func _runtime_identity_allowed(identity: Dictionary) -> bool:
    ...
    if int(target.get("expected_surface_count", -1)) != EXPECTED_SURFACES: return false
    var contract := identity.get("presentation_contract", {}) as Dictionary
    if str(contract.get("material_identity", "")) != "architectural_glazing": return false
    if bool(contract.get("geometry_changed", true)) or bool(contract.get("new_location_specific_placement", true)): return false
    if bool(contract.get("pane_layout_authored", true)) or bool(contract.get("interior_authored", true)): return false
    if bool(contract.get("exact_reflectance_claimed", true)): return false
