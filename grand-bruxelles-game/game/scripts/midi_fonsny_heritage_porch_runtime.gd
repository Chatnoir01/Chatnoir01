...
func _build_fonsny_heritage_porch(entrance: Node3D) -> void:
    # Heritage identity: broad access porch supported by polygonal columns,
    # composed in three registers; middle register has three long bays with
    # concrete crosswork and glass-block infill; upper register is blind.
    # The existing production entrance anchor is reused. Dimensions are an
    # authored player-readable reconstruction pending metric facade capture.
    ...
    # Register 1: deep, dark access plane with a continuous blue-stone base.
    ...
    # Register 2: exactly three source-described long glass-block bays.
    var bay_span := 5.35
    ...
    # Register 3: source-described blind upper register; yellow facing brick is
    # already documented for this unified Blomme/Petit Fonsny ensemble.
    ...
    # Broad perforated/glass-block canopy: modelled as a concrete perimeter and
    # ribs with translucent infill rather than a false solid slab.
    ...
    # Polygonal concrete supports are an explicit heritage feature.
