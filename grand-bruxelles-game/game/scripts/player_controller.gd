...
# Exact presentation framing used by the deterministic ground-oblique witness.
# This is a showcase camera, not a surveyed/player walking position.
const ATOMIUM_SHOWCASE_CAMERA_OFFSET := Vector3(-185.0, 86.0, 235.0)
const ATOMIUM_SHOWCASE_TARGET_OFFSET := Vector3(0.0, 50.0, 0.0)
const ATOMIUM_SHOWCASE_FOV_DEGREES := 48.0
...
func _apply_direct_spawn_from_user_args(args: PackedStringArray) -> void:
    for arg: String in args:
        ...
        if normalized == "spawn=atomium":
            call_deferred("_activate_atomium_direct_spawn", false)
            return
        if normalized == "spawn=atomium-showcase":
            call_deferred("_activate_atomium_direct_spawn", true)
            return

func _activate_atomium_direct_spawn(showcase: bool = false) -> void:
    ...
    terrain.build_collision = not showcase
    ...
    _configure_atomium_hud(world)

    if showcase:
        var showcase_camera := Camera3D.new()
        showcase_camera.name = "AtomiumShowcaseCamera"
        showcase_camera.position = atomium_anchor + ATOMIUM_SHOWCASE_CAMERA_OFFSET
        showcase_camera.fov = ATOMIUM_SHOWCASE_FOV_DEGREES
        world.add_child(showcase_camera)
        showcase_camera.look_at(atomium_anchor + ATOMIUM_SHOWCASE_TARGET_OFFSET, Vector3.UP)
        ...
        print("Direct showcase: Atomium ground-oblique witness")
        return
...
func _configure_atomium_hud(world: Node3D) -> void:
    ...
    for node_name: String in ["MissionLabel", "SaveStatusLabel", "WalletLabel", "MiniMap"]:
        var item := world.get_node_or_null(node_name)
...
func _unhandled_input(event: InputEvent) -> void:
    if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
        ...
        camera_pivot.rotation.x = clampf(camera_pivot.rotation.x, deg_to_rad(-60.0), deg_to_rad(35.0))
...
func _physics_process(delta: float) -> void:
    ...
    var input_2d := Vector2(float(right) - float(left), float(backward) - float(forward)).limit_length(1.0)
    var move_direction := Vector3(input_2d.x, 0.0, input_2d.y).rotated(Vector3.UP, rotation.y).normalized()
