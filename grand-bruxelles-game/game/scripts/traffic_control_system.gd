...
func rebuild(raw_controls: Array) -> void:
    ...
    for raw_control in raw_controls:
        if typeof(raw_control) != TYPE_DICTIONARY: continue
        var source: Dictionary = raw_control
        var raw_point: Variant = source.get("point", null)
        if raw_point == null or not raw_point is Array or raw_point.size() < 2: continue
...
func _cluster_signals() -> void:
    for control_index in range(_controls.size()):
        var control: Dictionary = _controls[control_index]
        if str(control.get("kind", "")) != "traffic_signals": continue
        var position: Vector3 = control["position"]
        var selected_cluster := -1
        for cluster_index in range(_signal_clusters.size()):
            var cluster: Dictionary = _signal_clusters[cluster_index]
            if (cluster["center"] as Vector3).distance_to(position) <= SIGNAL_CLUSTER_RADIUS_M:
...
func controls_for_route(route: PackedVector3Array) -> Array[Dictionary]:
    ...
    for control: Dictionary in _controls:
        ...
        if float(nearest.get("distance_m", INF)) > ROUTE_CONTROL_SNAP_M: continue
        var segment_index := int(nearest.get("segment_index", -1))
        if segment_index < 0 or segment_index >= route.size() - 1: continue
        ...
        route_control["route_distance_m"] = float(nearest.get("distance_m", INF))
...
func signal_state_for(control: Dictionary, approach_direction: Vector3, now_seconds: float = -1.0) -> String:
    if str(control.get("kind", "")) != "traffic_signals": return "green"
    ...
    if x_axis:
        if phase < 28.0: return "green"
        if phase < 32.0: return "amber"
        return "red"
    if phase >= 35.0 and phase < 63.0: return "green"
    if phase >= 63.0 and phase < 67.0: return "amber"
    return "red"

func get_control_count() -> int: return _controls.size()
func get_signal_count() -> int:
    var count := 0
    for control: Dictionary in _controls:
        if str(control.get("kind", "")) == "traffic_signals": count += 1
    return count
func get_signal_cluster_count() -> int: return _signal_clusters.size()
