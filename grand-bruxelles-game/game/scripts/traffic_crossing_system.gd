...
func get_crossing_state(crossing_id: int) -> Dictionary:
    ...
    var waiting: Dictionary = state.get("waiting", {})
    var crossing: Dictionary = state.get("crossing", {})
    return {"waiting": waiting.size(), "crossing": crossing.size(), "requires_stop": not waiting.is_empty() or not crossing.is_empty()}
    ...
    for road: Dictionary in roads:
        ...
        for index: int in range(points.size() - 1):
            ...
            if distance < best_distance:
                ...
                best = {"distance_m": distance, "start": start, "finish": finish, "road": road}
    return best
