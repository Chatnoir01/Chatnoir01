...
var _intersection_system: RefCounted
...
func _ready() -> void:
    ...
    _replenish_traffic()
    print(
        "Grand Bruxelles traffic graph: %d OSM ways, %d nodes, %d directed edges, %d intersections, %d right-priority, %d controls (%d signals), %d AI vehicles" %
        [
            _roads.size(),
...
func _spawn_one_vehicle() -> bool:
    ...
    for _attempt: int in range(MAX_SPAWN_ATTEMPTS):
        ...
        if _intersection_system != null:
            route_intersections = _intersection_system.call("intersections_for_route", route)
...
func get_intersection_count() -> int:
    if _intersection_system != null:
        return int(_intersection_system.call("get_intersection_count"))
    if _road_graph == null:
        return 0
...
func get_right_priority_count() -> int:
    if _intersection_system == null:
        return 0
    return int(_intersection_system.call("get_right_priority_count"))
