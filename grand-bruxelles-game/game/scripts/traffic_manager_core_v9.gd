extends "res://game/scripts/traffic_manager_core_v8.gd"
...
func _on_traffic_vehicle_disabled(vehicle: Node) -> void:
    ...
    if not is_instance_valid(vehicle) or _tow_root == null:
        return
...
func _create_tow_truck(wreck: Node) -> StaticBody3D:
    ...
    var body := _box_mesh(
        Vector3(2.18, 1.18, 5.45),
        Color(0.86, 0.70, 0.16, 1.0),
    ...
    var cab := _box_mesh(
        Vector3(2.02, 0.82, 1.72),
        Color(0.90, 0.75, 0.18, 1.0),
        Vector3(0.0, 0.76, -1.65)
    )
    cab.name = "TowCab"
    tow.add_child(cab)

    var bed := _box_mesh(
        Vector3(1.94, 0.20, 2.72),
        Color(0.20, 0.22, 0.24, 1.0),
        Vector3(0.0, 0.66, 0.92)
    )
    bed.name = "TowBed"
    tow.add_child(bed)
