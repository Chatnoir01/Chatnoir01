...
func _append_edge(
    ...
    var edge := {
        ...
        "from": start,
        "to": finish,
        "length_m": start.distance_to(finish),
        "road": road,
...
func find_candidate_edge_ids(anchor: Vector3, radius_m: float) -> Array[int]:
    ...
    for edge: Dictionary in _edges:
        ...
        midpoint.y = anchor.y
        var distance_squared := midpoint.distance_squared_to(anchor)
        if distance_squared <= radius_squared:
            result.append(int(edge["id"]))
    return result


func build_random_walk(
    start_edge_id: int,
    ...
    while route.size() < max_edges:
        ...
        for raw_id: Variant in outgoing_raw:
            var candidate_id := int(raw_id)
            if candidate_id < 0 or candidate_id >= _edges.size():
                continue
            var candidate: Dictionary = _edges[candidate_id]
            if str(candidate["to_key"]) == str(current["from_key"]):
                continue
            candidates.append(candidate_id)
            if not visited.has(candidate_id):
                unvisited.append(candidate_id)
...
func _choose_next_edge(
    ...
    for candidate_id: int in candidates:
        var candidate: Dictionary = _edges[candidate_id]
        ...
        if int(candidate.get("osm_id", 0)) == int(current.get("osm_id", 0)):
            alignment += 0.18
...
func get_edge(edge_id: int) -> Dictionary:
    if edge_id < 0 or edge_id >= _edges.size():
        return {}
    return _edges[edge_id]


func get_node_count() -> int:
    return _node_positions.size()
...
func get_intersection_count() -> int:
    var count := 0
    for raw_key: Variant in _node_positions.keys():
        var key := str(raw_key)
        var neighbors: Dictionary = {}
        var outgoing_raw: Array = _outgoing.get(key, [])
        var incoming_raw: Array = _incoming.get(key, [])

        for raw_id: Variant in outgoing_raw:
            var edge: Dictionary = get_edge(int(raw_id))
            if not edge.is_empty():
                neighbors[str(edge["to_key"])] = true
        for raw_id: Variant in incoming_raw:
            var edge: Dictionary = get_edge(int(raw_id))
            if not edge.is_empty():
                neighbors[str(edge["from_key"])] = true
