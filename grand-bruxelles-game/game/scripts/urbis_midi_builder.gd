...
func _build_from_runtime() -> void:
    ...
    print("Grand Bruxelles UrbIS Midi exact-plan: %d surfaces, %d hidden-level surfaces, %d buildings" % [surface_count, int(_surface_family_counts.get("hidden_level", 0)), building_count])
...
func surface_family(surface_type: String, level: float) -> String:
    if not is_zero_approx(level):
        return "hidden_level"
    match surface_type:
        "S", "A", "AC", "B", "C", "I", "IC", "IL", "K", "SC": return "road"
        "SW", "G": return "sidewalk"
        "M": return "island"
        "P": return "paved"
        "MS", "MT", "RS", "RT": return "transit"
        _: return "other"
...
func _build_street_surfaces(features: Array) -> int:
    ...
    var count: int = 0
    _surface_family_counts = {"road": 0, "sidewalk": 0, "island": 0, "paved": 0, "transit": 0, "other": 0, "hidden_level": 0}
    for feature: Dictionary in features:
        var polygon: PackedVector2Array = _ring(feature.get("polygon", []))
        var family: String = surface_family(str(feature.get("type", "")), float(feature.get("level", 0.0)))
        ...
        match family:
            "road": target = road_tool
            "sidewalk": target = sidewalk_tool
            "island": target = island_tool
            "paved": target = paved_tool
...
func _build_buildings(features: Array) -> int:
    ...
    for feature: Dictionary in features:
        ...
        if polygon.size() < 3:
            continue
        var bucket: int = _building_bucket(str(feature.get("id", "")))
        if _append_building(tools[bucket], polygon, float(feature.get("height", 10.0))):
            count += 1
