...
func register_impact(speed_kmh: float, alignment: float = 1.0) -> Dictionary:
    var impact_speed := maxf(0.0, speed_kmh)
    var hit_alignment := clampf(alignment, 0.0, 1.0)
    ...
    var excess := impact_speed - MIN_DAMAGE_SPEED_KMH
    var severity := pow(excess, 1.18) * 0.72
    var alignment_factor := lerpf(0.42, 1.0, hit_alignment)
    var damage := clampf(severity * alignment_factor, 0.0, 55.0)
