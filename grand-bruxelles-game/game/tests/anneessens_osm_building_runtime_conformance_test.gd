...
func _run() -> void:
    ...
    if typeof(parsed) != TYPE_DICTIONARY:
        ...
        return
    var source_building: Dictionary = {}
    ...
    if source_building.is_empty():
        _fail("source building 256376389 missing")
        return
    ...
    if candidates.is_empty():
        _fail("runtime building 256376389 missing")
        return
    ...
    print("ANNEESSENS_OSM_BUILDING_RUNTIME_CONFORMANCE_OK: osm_id=256376389 source_height=14.0 runtime_depth=14.0 source_footprint_preserved=true runtime_center=(-285.4966,-185.4604) collision=true geometry_change_authorized=false camera_rescue_authorized=false visual_acceptance=false")
    quit(0)
