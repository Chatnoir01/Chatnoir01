...
func _run() -> void:
    ...
    await process_frame
    _assert_false(
        runtime._is_authoritative_production_scene(root_decoy),
    ...
    viewport.name = "AuthorityDecoyViewport"
    root.add_child(viewport)
    var viewport_decoy := _make_main_fixture()
    viewport.add_child(viewport_decoy)
    await process_frame
    _assert_false(
        runtime._is_authoritative_production_scene(viewport_decoy),
    ...
    await process_frame
    _assert_true(
        runtime._is_authoritative_production_scene(explicit_scene),
    ...
    if packed != null:
        ...
        if canonical != null:
            ...
            await process_frame
            _assert_true(
                runtime._is_authoritative_production_scene(canonical),
    ...
    if _failures.is_empty():
        print("ANNEESSENS_OSM_FURNITURE_AUTHORITATIVE_ROOT_BIND_OK")
        quit(0)
    else:
        push_error("Anneessens furniture authoritative-root witness failed with %d assertion(s)" % _failures.size())
        quit(1)
