...
func _run() -> void:
    ...
    for _attempt: int in range(480):
        ...
        if not bool(terrain_probe.get("terrain_loaded")) or not bool(hero_probe.get("hero_built")):
            continue
        if not bool(hero_probe.get("base_sphere_window_band_semantics_applied")):
            continue
    ...
    if not spawn_complete:
        _fail("production Atomium arrival with window-band hero was not observed")
        return
    ...
    var band := hero.get("base_sphere_window_band") as Node3D
    if band == null or not bool(band.get("band_built")):
        _fail("base-sphere window-band component missing")
        return
    ...
    await process_frame
    if bool(hero.call("base_sphere_window_band_enabled")) or overlay.visible:
    ...
    await process_frame
    if not bool(hero.call("base_sphere_window_band_enabled")) or not overlay.visible:
    ...
    print("ATOMIUM_BASE_SPHERE_WINDOW_BAND_PLAYER_AB_OK: player_eye=true camera_rescue=false fov=%.1f pitch=%.1f spring=%.1f xz_error=%.6f spheres=9 tubes=20 exact_layout=false geometry_moved=false collision_changed=false" % [camera.fov, pivot.rotation_degrees.x, arm.spring_length, xz_error])
    quit(0)
