...
const WINDOW_RUNTIME_NAME := "AtomiumBaseSphereWindowBandRuntime"
const ARRIVAL_RUNTIME_NAME := "AtomiumArrivalPresentationRuntime"
...
func _run() -> void:
    ...
    var window_runtime: Node = null
    var arrival_runtime: Node = null
    for _attempt: int in range(240):
        await process_frame
        main = current_scene
        player = main.get_node_or_null("Player") as CharacterBody3D if main != null else null
        window_runtime = root.get_node_or_null(WINDOW_RUNTIME_NAME)
        arrival_runtime = root.get_node_or_null(ARRIVAL_RUNTIME_NAME)
        if main != null and player != null and window_runtime != null and arrival_runtime != null:
            break
    if main == null or player == null:
        _fail("main/player unavailable")
        return
    if window_runtime == null:
    ...
    if arrival_runtime == null:
        ...
        return
    if bool(window_runtime.call("runtime_approved")) or bool(window_runtime.call("realism_complete")):
        _fail("glazing cue was accidentally promoted")
        return
    if bool(window_runtime.call("source_geometry_moved")) or bool(window_runtime.call("collision_changed")) or bool(window_runtime.call("hero_core_modified")):
        _fail("registry isolation/source rails drifted")
        return

    window_runtime.call("set_enhanced_enabled", false)
    ...
    for _attempt: int in range(600):
        ...
        if not bool(terrain_probe.get("terrain_loaded")) or not bool(hero_probe.get("hero_built")):
            continue
        if not bool(window_runtime.call("ready_complete")) or bool(window_runtime.call("failed")):
            continue
        if window_runtime.call("band_node") == null:
            continue
    ...
    if not bool(player.get_meta("atomium_arrival_avatar_hidden", false)):
        _fail("#881 avatar-occluder suppression was not active")
        return
    if int(arrival_runtime.call("current_visible_visual_count")) != 0:
        _fail("player visual still occludes the normal Atomium arrival")
        return
    if bool(arrival_runtime.call("camera_changed")) or bool(arrival_runtime.call("source_position_changed")) or bool(arrival_runtime.call("collision_changed")):
        _fail("#881 arrival presentation changed protected world/camera truth")
        return

    var band := window_runtime.call("band_node") as Node3D
    if band == null or band.get_parent() != hero or not bool(band.get("band_built")):
        _fail("registry-mounted base-sphere window band missing or mis-parented")
        return
    if not bool(band.get_meta("registry_mounted", false)):
        _fail("window band lost registry identity")
        return
    ...
    if bool(band.call("source_geometry_moved")) or bool(band.call("collision_changed")):
        ...
        return
    var sphere := hero.get_node_or_null("Sphere_00") as MeshInstance3D
    if sphere == null or not sphere.mesh is SphereMesh:
        _fail("production Sphere_00 missing")
        return
    if band.position.distance_to(sphere.position) > 0.001:
        _fail("window band moved away from the existing Sphere_00 centre")
        return
    ...
    window_runtime.call("set_enhanced_enabled", false)
    await process_frame
    if bool(window_runtime.call("enhanced_enabled")) or bool(band.call("enabled")) or overlay.visible:
        _fail("BEFORE toggle did not hide registry window band")
        return
    ...
    window_runtime.call("set_enhanced_enabled", true)
    await process_frame
    if not bool(window_runtime.call("enhanced_enabled")) or not bool(band.call("enabled")) or not overlay.visible:
        _fail("AFTER toggle did not show registry window band")
        return
    if not await _capture(after_path):
        _fail("AFTER capture failed")
        return

    print("ATOMIUM_BASE_SPHERE_WINDOW_BAND_PLAYER_AB_OK: player_eye=true avatar_hidden=true registry_mounted=true hero_core_modified=false camera_rescue=false fov=%.1f pitch=%.1f spring=%.1f xz_error=%.6f spheres=9 tubes=20 exact_layout=false geometry_moved=false collision_changed=false" % [camera.fov, pivot.rotation_degrees.x, arm.spring_length, xz_error])
    quit(0)
