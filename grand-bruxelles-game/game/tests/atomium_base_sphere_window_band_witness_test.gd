...
func _find_sphere_by_id(hero: Node, sphere_id: int) -> MeshInstance3D:
    var found: MeshInstance3D = null
    for child: Node in hero.get_children():
        if child is MeshInstance3D and child.has_meta("atomium_sphere_id") and int(child.get_meta("atomium_sphere_id")) == sphere_id:
            if found != null:
                return null
            found = child as MeshInstance3D
    return found
...
func _run() -> void:
    ...
    for _attempt: int in range(480):
        ...
        if main == null:
            continue
        if _location_text(main.get_node_or_null("LocationLabel")) != EXPECTED_LOCATION:
            continue
    ...
    var target_sphere := _find_sphere_by_id(hero, EXPECTED_TARGET_SPHERE_ID)
    if target_sphere == null:
        _fail("target sphere_id=0 missing or duplicated")
        return

    var band := hero.get("base_sphere_window_band") as Node
    if band == null or not bool(band.get("band_built")):
        _fail("id-bound window band did not build")
        return
    if int(band.call("target_sphere_id")) != EXPECTED_TARGET_SPHERE_ID:
        _fail("window band bound to wrong sphere id")
        return
    if int(band.call("source_monument_relation_id")) != EXPECTED_MONUMENT_RELATION_ID:
        _fail("monument relation id drifted")
        return
    if int(band.call("source_heritage_record_id")) != EXPECTED_HERITAGE_RECORD_ID:
        _fail("heritage record id drifted")
        return
    if bool(band.get_meta("selection_by_display_name", true)):
        _fail("display-name selection was reintroduced")
        return
    ...
    print("ATOMIUM_BASE_SPHERE_WINDOW_BAND_WITNESS_OK: relation_id=%d heritage_id=%d sphere_id=%d player_arrival=true camera_rescue=false fov=%.1f pitch=%.1f spring=%.1f xz_error=%.6f avatar_hidden_for_clean_witness=true exact_layout=false source_geometry_moved=false collision_changed=false" % [EXPECTED_MONUMENT_RELATION_ID, EXPECTED_HERITAGE_RECORD_ID, EXPECTED_TARGET_SPHERE_ID, camera.fov, pivot.rotation_degrees.x, arm.spring_length, xz_error])
    quit(0)
