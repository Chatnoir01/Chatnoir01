...
func _run() -> void:
    ...
    var basin := runtime.call("basin_node") as Node3D
    if terrain == null or hero == null or reflection == null or basin == null:
        _fail("Atomium production context / registry basin incomplete after spawn signal")
        return
    if not bool(terrain.get("terrain_loaded")) or not bool(runtime.call("ready_complete")) or bool(runtime.call("failed")):
        _fail("terrain or registry runtime not ready after spawn signal")
        return
    ...
    if not is_finite(ground_y) or not is_finite(player.global_position.y):
        _fail("production arrival vertical state is non-finite")
        return
    var settled_clearance := player.global_position.y - ground_y
    print("ATOMIUM_CURRENT_BASIN_ARRIVAL_OBSERVED: player=%s anchor=%s xz_error=%.6f settled_clearance=%.6f fov=%.1f pitch=%.1f spring=%.1f" % [player.global_position, anchor, xz_error, settled_clearance, camera.fov, pivot.rotation_degrees.x, arm.spring_length])
