...
func _fail(message: String) -> void:
    push_error("ATOMIUM_ROADAREA_CONTEXT_FAIL: %s" % message)
    quit(1)
...
func _validate_context(context: Node) -> bool:
    return context != null \
        ...
        and str(context.get("source_layer")) == "TN.RoadTransportNetwork.RoadArea" \
        and str(context.get("source_response_sha256")) == "4c6b139d43e8fa5e56b7a0f2d2f01b24e579a436338c9e129970cc649a1c3a77" \
        and int(context.get("source_feature_count")) == 24 \
        and int(context.get("selected_feature_count")) == 5 \
        and float(context.get("source_coverage_fraction")) > 0.24 \
        and int(context.get("triangle_count")) > 50 \
        and bool(context.get("geometry_exact_from_wfs")) \
        and not bool(context.get("material_photometry_resolved")) \
        and not bool(context.get("collision_resolved"))
...
func _run() -> void:
    ...
    if not bool(hero.call("build_on_terrain", terrain)):
        _fail("hero failed")
        return
    var context := hero_world.get_node_or_null("AtomiumRoadAreaContext")
    if not _validate_context(context):
        _fail("hero RoadArea source contract failed")
        return
    ...
    if not await _save_capture(hero_viewport, "res://artifacts/atomium/roadarea_hero_before.png"):
        _fail("hero before capture failed")
        return
    context.visible = true
    if not await _save_capture(hero_viewport, "res://artifacts/atomium/roadarea_hero_after.png"):
    ...
    for _frame: int in range(12):
        await process_frame
    var direct_context := main.get_node_or_null("AtomiumRoadAreaContext")
    if not _validate_context(direct_context):
        _fail("direct-player RoadArea source contract failed")
        return
    direct_context.visible = false
    if not await _save_capture(direct_viewport, "res://artifacts/atomium/roadarea_player_before.png"):
        _fail("player before capture failed")
        return
    direct_context.visible = true
    if not await _save_capture(direct_viewport, "res://artifacts/atomium/roadarea_player_after.png"):
        _fail("player after capture failed")
        return

    print("ATOMIUM_ROADAREA_CONTEXT_OK: source_features=24 selected=5 coverage=0.242798 hero=1280x960 player=1280x720")
    quit(0)
