...
func _run() -> void:
	...
	# Lifecycle regression: a bound NPC may disappear between frames (despawn, scene
	# teardown, vehicle smoke-test cleanup). The runtime must purge that binding
	# without ever casting a freed Object.
	var lifecycle_count := runtime.binding_count()
	civilian.queue_free()
	await process_frame
	await process_frame
	if not _expect(runtime.binding_count() == lifecycle_count - 1, "freed NPC binding should be purged safely on the next frame"):
		return

	print("AUTHORED_NPC_VISUAL_RUNTIME_OK: source=%s bindings=%d ambient_clip=%s lifecycle_purge=true" % [runtime.resolved_source_path(), runtime.binding_count(), ambient_animation_player.current_animation])
