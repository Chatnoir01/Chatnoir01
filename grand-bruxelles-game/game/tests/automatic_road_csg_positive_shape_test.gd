...
func _run() -> void:
    ...
    print("AUTOMATIC_ROAD_CSG_POSITIVE_SHAPE_GREEN: road_id=%d subtraction_only_rejected=true intersection_only_rejected=true union_accepted=true destination_advertisable=false jouable=false" % ROAD_ID)
    quit(0)
