...
func _write_witness_metadata(player: CharacterBody3D, witness_road_id: int, witness_road_name: String, ground_y: float) -> Error:
    var metadata := {
        "schema": "grand_bruxelles_automatic_road_player_witness_runtime_v1",
...
func _run() -> void:
    ...
    if not FileAccess.file_exists(WITNESS_PATH):
        ...
        return
    var metadata_error := _write_witness_metadata(player, second_id, second_name, ground_y)
    ...
    print("AUTOMATIC_ROAD_DIRECT_SPAWN_GREEN: indexed_roads=%d source_documents=%d first=%d witness=%d witness_name=%s source=%s source_sha=%s ground_y=%.3f lookup=deterministic_runtime_index png=%s metadata=%s" % [expected_road_count, expected_document_count, LEMONNIER_ID, second_id, second_name, source_path, expected_source_sha, ground_y, WITNESS_PATH, WITNESS_METADATA_PATH])
    quit(0)
