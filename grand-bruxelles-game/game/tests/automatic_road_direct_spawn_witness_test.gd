...
func _empty_animation_driver_state(using_authored_character: bool, authored_character_found: bool) -> Dictionary:
    return {
        "using_authored_character": using_authored_character,
        "authored_character_found": authored_character_found,
        "animation_player_count": 0,
        "animation_tree_count": 0,
        "active_driver_count": 0,
        "active_driver": "",
        "active_animation": "",
        "animation_player_diagnostics": []
    }
...
func _authored_animation_driver_state(player: CharacterBody3D) -> Dictionary:
    ...
    while not stack.is_empty():
        ...
        if node is AnimationPlayer:
            ...
            animation_player_diagnostics.append({
                "path": str(authored.get_path_to(animation_player)),
                "animation_count": animation_names.size(),
                "animation_names": animation_names,
                "autoplay": str(animation_player.get("autoplay")),
                "current_animation": str(animation_player.current_animation),
                "assigned_animation": str(animation_player.assigned_animation),
                "is_playing": animation_player.is_playing(),
                "speed_scale": animation_player.speed_scale,
                "process_mode": animation_player.process_mode
            })
    ...
    return {
        ...
        "animation_player_diagnostics": animation_player_diagnostics
    }
...
func _write_animation_diagnostic(player: CharacterBody3D, animation_state: Dictionary, status: String, reason: String) -> bool:
    var payload := {
        ...
        "animation_player_diagnostics": animation_state.get("animation_player_diagnostics", []),
        "status": status,
        "reason": reason,
...
func _run() -> void:
    ...
    var visual := player.get_node_or_null("VisualUpgrade")
    if visual == null: _fail("production VisualUpgrade missing"); return
    var locomotion_runtime := root.get_node_or_null("AuthoredPlayerLocomotionRuntime")
    if locomotion_runtime == null or not locomotion_runtime.has_method("bind_target"):
        _fail("production AuthoredPlayerLocomotionRuntime autoload missing")
        return
    if not bool(locomotion_runtime.call("bind_target", player, visual)):
        _fail("production authored locomotion runtime refused isolated Player/VisualUpgrade binding")
        return
    if not bool(locomotion_runtime.get_meta("authored_locomotion_ready", false)):
        _fail("production authored locomotion runtime did not reach readiness")
        return
    for _frame: int in range(4): await process_frame; await physics_frame
