...
const BEFORE_PATH := "res://artifacts/visual/bourse_limestone_base_before.png"
const AFTER_PATH := "res://artifacts/visual/bourse_limestone_base_after.png"
const MIN_CHANGED_3 := 0.010
...
func _fail(message: String) -> void:
    push_error("BOURSE_LIMESTONE_BASE_WITNESS_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if bool(contract.get("geometry_changed", true)) or bool(contract.get("candidate_geometry_promoted", true)):
        _fail("material pass must not promote provisional geometry")
        return
    if bool(contract.get("masonry_joints_authored", true)) or bool(contract.get("tooling_marks_authored", true)):
        _fail("material contract overclaims limestone detail")
        return
    ...
    var runtime := root.get_node_or_null("BourseLimestoneBaseSurfaceRuntime")
    ...
    if runtime == null or portico == null:
        ...
        return
    if int(runtime.call("diagnostic_target_count")) != 16:
        runtime.call("bind_portico", portico)
    if bool(runtime.call("diagnostic_identity_failure")) or int(runtime.call("diagnostic_target_count")) != 16:
        _fail("expected 16 safe limestone stair/base surfaces")
        return
    ...
    print("BOURSE_LIMESTONE_BASE_METRICS: changed_gt3=%.6f changed_gt8=%.6f targets=16" % [changed_3, changed_8])
    ...
    print("BOURSE_LIMESTONE_BASE_WITNESS_OK: changed_gt3=%.4f%% changed_gt8=%.4f%%" % [changed_3 * 100.0, changed_8 * 100.0])
    viewport.queue_free()
    quit(0)
