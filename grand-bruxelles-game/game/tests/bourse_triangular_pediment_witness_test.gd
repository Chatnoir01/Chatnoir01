...
func _metrics(before: Image, after: Image) -> Dictionary:
    ...
    var max_x: int = -1
    var max_y: int = -1
    var limit_3: float = 3.0 / 255.0
    var limit_8: float = 8.0 / 255.0
    ...
    var bbox_width: int = 0
    var bbox_height: int = 0
...
func _write_metrics(metrics: Dictionary) -> void:
    var absolute: String = ProjectSettings.globalize_path(METRICS_PATH)
    DirAccess.make_dir_recursive_absolute(absolute.get_base_dir())
...
func _run() -> void:
    ...
    var position: Vector3 = _vector3(candidate.get("position", []))
    var rotation: Vector3 = _vector3(candidate.get("rotation_degrees", []))
    var horizontal_fov: float = float(candidate.get("horizontal_fov_degrees", 0.0))
    ...
    var existing_camera: Camera3D = scene.get_viewport().get_camera_3d()
    ...
    for _frame: int in range(WARMUP_FRAMES):
        _freeze_dynamic(scene)
        await process_frame
    var before: Image = await _capture(viewport, scene, BEFORE_PATH)
    if before == null:
    ...
    if runtime.diagnostic_pediment_count() != 1:
        ...
        return
    var after: Image = await _capture(viewport, scene, AFTER_PATH)
    if after == null:
        _fail("after capture failed")
        return
    var metrics: Dictionary = _metrics(before, after)
