...
const BEFORE_PATH := "res://artifacts/visual/bourse_white_stone_before.png"
const AFTER_PATH := "res://artifacts/visual/bourse_white_stone_after.png"
...
func _run() -> void:
    ...
    if bool(contract.get("geometry_changed", true)) or bool(contract.get("candidate_geometry_promoted", true)):
        _fail("material pass must not promote or change provisional geometry")
        return
    if bool(contract.get("masonry_joints_authored", true)) or bool(contract.get("exact_stone_species_claimed", true)):
        _fail("material contract overclaims stone detail")
        return

    var evidence := _read_json(EVIDENCE_PATH)
    if evidence.is_empty():
        _fail("Bourse camera evidence missing")
        return
    ...
    var runtime := root.get_node_or_null("BourseWhiteStoneSurfaceRuntime")
    ...
    if int(runtime.call("diagnostic_target_count")) != 24:
        _fail("expected 24 white-stone surfaces")
        return
    ...
    print("BOURSE_WHITE_STONE_METRICS: changed_gt3=%.6f changed_gt8=%.6f targets=24" % [changed_3, changed_8])
    ...
    print("BOURSE_WHITE_STONE_WITNESS_OK: changed_gt3=%.4f%% changed_gt8=%.4f%%" % [changed_3 * 100.0, changed_8 * 100.0])
    viewport.queue_free()
    quit(0)
