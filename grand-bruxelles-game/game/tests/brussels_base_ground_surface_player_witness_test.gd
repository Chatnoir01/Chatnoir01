...
func _run() -> void:
    ...
    var report := {
        "schema": "grand-bruxelles-base-ground-surface-player-witness-v1",
        ...
        "legacy_material_frozen_from_main_9fec981": true,
    ...
    print("BRUSSELS_BASE_GROUND_SURFACE_PLAYER_WITNESS_OK: changed_fraction=%.6f mean_delta=%.6f bbox=%dx%d fov=%.2f human_review=pending" % [fraction, float(metrics["mean_changed_delta"]), int(metrics["bbox_width"]), int(metrics["bbox_height"]), review_camera.fov])
    quit(0)
