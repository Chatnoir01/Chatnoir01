...
func _run() -> void:
    ...
    if str(shader_material.get_meta("visual_recipe_profile", "")) != EXPECTED_PROFILE:
        ...
        return
    if bool(shader_material.get_meta("surface_composition_claimed", true)) or bool(shader_material.get_meta("surface_identity_claimed", true)):
        _fail("authored Ground presentation manufactured source semantics")
        return
    if bool(shader_material.get_meta("microtexture_scale_source_measured", true)):
        _fail("authored Ground microtexture scale claimed as measured")
        return
