...
func _run() -> void:
    ...
    # Match the performance harness: a real production scene may exist under root
    # while current_scene stays null. Manual bind_scene() would hide this defect.
    ...
    print("BRUSSELS_BOLLARD_OK: points=%d collisions=%d batches=%d family=%s source=OSM license=ODbL-1.0 root_auto_bind=true" % [EXPECTED_COUNT, int(runtime.call("collision_count")), int(runtime.call("visual_batch_count")), EXPECTED_FAMILY])
    quit(0)
