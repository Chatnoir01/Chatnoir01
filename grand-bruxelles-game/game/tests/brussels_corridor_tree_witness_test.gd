...
func _diff_stats(before: Image, after: Image, threshold: int) -> Dictionary:
    ...
    var changed := 0
    var min_x := before.get_width(); var min_y := before.get_height(); var max_x := -1; var max_y := -1
...
func _densest_center(positions: Array) -> Vector3:
    var best := Vector3.ZERO; var best_count := -1
    for raw_a: Variant in positions:
        var a: Vector3 = raw_a as Vector3; var count := 0; var sum := Vector3.ZERO
        for raw_b: Variant in positions:
            var b: Vector3 = raw_b as Vector3; var dx := a.x - b.x; var dz := a.z - b.z
            if dx * dx + dz * dz <= CLUSTER_RADIUS * CLUSTER_RADIUS: count += 1; sum += b
        if count > best_count: best_count = count; best = sum / float(maxi(count, 1))
...
func _run() -> void:
    ...
    if runtime == null: _fail("production corridor tree autoload missing"); return
    runtime.call("bind_scene", scene)
    for _frame: int in range(12): await process_frame
    if not bool(runtime.call("ready_complete")) or bool(runtime.call("failed")): _fail("production corridor tree runtime did not bind cleanly"); return
    if int(runtime.call("tree_count")) != 266 or int(runtime.call("preowned_tree_count")) != 7: _fail("runtime ownership counts changed"); return
    if not bool(runtime.call("source_positions_unchanged")): _fail("source positions changed before visual witness"); return
    var positions: Array = runtime.call("source_positions") as Array; var center := _densest_center(positions)
    var camera := Camera3D.new(); camera.current = true; camera.fov = 67.0; scene.add_child(camera)
    var candidates := _camera_candidates(center); var best_before: Image = null; var best_after: Image = null; var best_changed := -1.0; var best_index := -1
    for index: int in range(candidates.size()):
        var candidate := candidates[index] as Array; camera.look_at_from_position(candidate[0] as Vector3, candidate[1] as Vector3, Vector3.UP)
        runtime.call("set_visual_enabled", false); for _frame: int in range(4): await process_frame
        var before := await _grab(viewport); runtime.call("set_visual_enabled", true); for _frame: int in range(4): await process_frame
        var after := await _grab(viewport); var fraction := float(_diff_stats(before, after, 3).get("fraction", -1.0))
        if fraction > best_changed: best_changed = fraction; best_before = before; best_after = after; best_index = index
    if best_before == null or best_after == null or best_index < 0: _fail("failed to obtain deterministic player-eye A/B"); return
    ...
    if changed3 < MIN_CHANGED_3 or changed8 < MIN_CHANGED_8: _fail("full-frame corridor-tree impact too weak: gt3=%.4f%% gt8=%.4f%%" % [changed3 * 100.0, changed8 * 100.0]); return
    if bbox.size.x < MIN_BBOX_WIDTH or bbox.size.y < MIN_BBOX_HEIGHT: _fail("corridor-tree change bbox too small: %dx%d" % [bbox.size.x, bbox.size.y]); return
    if not bool(runtime.call("source_positions_unchanged")): _fail("source positions changed during A/B"); return
