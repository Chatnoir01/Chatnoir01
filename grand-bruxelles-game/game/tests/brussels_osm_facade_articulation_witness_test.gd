...
func _run() -> void:
    ...
    if base_runtime == null or runtime == null: _fail("facade runtimes missing"); return
    ...
    camera.look_at_from_position(camera.position, Vector3(building_xz.x, clampf(building.depth * 0.42, 4.0, 8.5), building_xz.y), Vector3.UP)
    camera.fov = 67.0
    camera.current = true
    ...
    if before == null or after == null: _fail("1280x720 current-vs-candidate capture failed"); return
