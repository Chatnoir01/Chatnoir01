...
const WIDTH := 1280
const HEIGHT := 720
const BEFORE_PATH := "res://artifacts/visual/facade_readability_before.png"
const AFTER_PATH := "res://artifacts/visual/facade_readability_after.png"
const CONTROL_PATH := "res://artifacts/visual/facade_readability_control.png"
const CORRIDOR_ANCHOR := Vector3(-272.04, 0.0, -217.07)
const FAMILY := "brussels_osm_facade_surface_v1"
const MIN_CHANGED_3 := 0.0005
const MIN_CHANGED_8 := 0.0001
const MAX_CHANGED_3 := 0.1500
const MIN_BBOX_W := 320
const MIN_BBOX_H := 120
const MIN_CONTROL_CHANGED_8 := 0.0200
...
func _fail(message: String) -> void:
    push_error("BRUSSELS_OSM_FACADE_READABILITY_VISUAL_FAIL: %s" % message)
    quit(1)
...
func _collect_facade_materials(node: Node, output: Array[ShaderMaterial], ids: Dictionary) -> void:
    # Production generic buildings are CSGPolygon3D nodes. The predecessor witness
    # tested CSGPrimitive3D and therefore collected zero materials even after the
    # facade runtime had successfully rebound all 139 buildings.
    if node is CSGPolygon3D:
        var building := node as CSGPolygon3D
        if str(building.get_meta("material_family", "")) == FAMILY:
            var material := building.material as ShaderMaterial
            if material != null and str(material.get_meta("material_family", "")) == FAMILY and not ids.has(material.get_instance_id()):
                ids[material.get_instance_id()] = true
                output.append(material)
    for child: Node in node.get_children():
        _collect_facade_materials(child, output, ids)
...
func _run() -> void:
    ...
    if roads == null or buildings == null:
        _fail("generic OSM road/building anchors missing")
        return
    var road := _find_witness_road(roads)
    if road == null:
        _fail("no stable corridor road available for player witness")
        return

    var materials: Array[ShaderMaterial] = []
    _collect_facade_materials(buildings, materials, {})
    if materials.is_empty() or materials.size() > 6:
        _fail("generic facade material set invalid: %d" % materials.size())
        return

    var strengths: Array[float] = []
    var base_colors: Array[Color] = []
    for material: ShaderMaterial in materials:
        var strength_variant: Variant = material.get_shader_parameter("surface_readability_strength")
        var color_variant: Variant = material.get_shader_parameter("base_color")
        if strength_variant == null or color_variant == null:
            _fail("generic facade readability parameters missing")
            return
        strengths.append(float(strength_variant))
        base_colors.append(color_variant as Color)
        if strengths.back() <= 0.0:
            _fail("authored facade readability strength must be positive")
            return
    ...
    if before == null:
        _fail("1280x720 facade baseline capture failed")
        return

    for index: int in range(materials.size()):
        materials[index].set_shader_parameter("base_color", Color(1.0, 0.0, 1.0, 1.0))
    for _frame: int in range(8):
        await process_frame
    var control := await _capture(viewport, CONTROL_PATH)
    if control == null:
        _fail("1280x720 facade control capture failed")
        return
    var control_gt8 := _metrics(before, control, 8)
    print("BRUSSELS_OSM_FACADE_READABILITY_CONTROL: changed_gt8=%.6f bbox=%dx%d materials=%d" % [float(control_gt8["fraction"]), int(control_gt8["bbox_w"]), int(control_gt8["bbox_h"]), materials.size()])
    if float(control_gt8["fraction"]) < MIN_CONTROL_CHANGED_8:
        _fail("generic facade materials do not reach enough player-frame pixels")
        return

    for index: int in range(materials.size()):
        materials[index].set_shader_parameter("base_color", base_colors[index])
        materials[index].set_shader_parameter("surface_readability_strength", strengths[index])
    for _frame: int in range(8):
        await process_frame
    var after := await _capture(viewport, AFTER_PATH)
    if after == null:
        _fail("1280x720 facade readability capture failed")
        return
    ...
    print("BRUSSELS_OSM_FACADE_READABILITY_VISUAL: changed_gt3=%.6f changed_gt8=%.6f bbox=%dx%d strength=%.3f eye=1.65 fov=67" % [changed_3, changed_8, bbox_w, bbox_h, strengths[0]])
    if changed_3 < MIN_CHANGED_3 or changed_8 < MIN_CHANGED_8:
        _fail("generic facade readability change is not player-visible enough")
        return
    if changed_3 > MAX_CHANGED_3:
        _fail("generic facade readability change overpowers the player frame")
        return
    if bbox_w < MIN_BBOX_W or bbox_h < MIN_BBOX_H:
        _fail("generic facade readability lacks broad screen coverage")
        return

    print("BRUSSELS_OSM_FACADE_READABILITY_VISUAL_OK")
    quit(0)
