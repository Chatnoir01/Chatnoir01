...
const WIDTH := 1280
const HEIGHT := 720
const BEFORE_PATH := "res://artifacts/visual/osm_facade_roughness_before.png"
const CONTROL_PATH := "res://artifacts/visual/osm_facade_roughness_control.png"
const AFTER_PATH := "res://artifacts/visual/osm_facade_roughness_after.png"
...
func _fail(message: String) -> void: push_error("BRUSSELS_OSM_FACADE_ROUGHNESS_VISUAL_FAIL: %s" % message); quit(1)
...
func _run() -> void:
    ...
    if material_factory == null or not "ROUGHNESS_MICRO_VARIATION" in material_factory:
        _fail("authored facade roughness response unavailable"); return
    var authored_strength := float(material_factory.ROUGHNESS_MICRO_VARIATION)
    if authored_strength <= 0.0 or authored_strength > 0.04:
        _fail("authored facade roughness response outside frozen bounds"); return
    ...
    for material: ShaderMaterial in materials:
        ...
        material.set_shader_parameter("roughness_micro_variation", 0.0)
    ...
    for i: int in range(materials.size()):
        ...
        materials[i].set_shader_parameter("roughness_micro_variation", authored_strength)
    for _frame: int in range(8): await process_frame
    var after := await _capture(viewport, AFTER_PATH)
    if before == null or control == null or after == null: _fail("1280x720 roughness capture failed"); return
    if not building.global_transform.is_equal_approx(original_transform) or building.polygon != original_polygon or not is_equal_approx(building.depth, original_depth): _fail("building geometry changed during roughness A/B"); return
    if not bool(runtime.call("geometry_unchanged")): _fail("runtime geometry invariant failed"); return
    print("BRUSSELS_OSM_FACADE_ROUGHNESS_VISUAL_CAPTURED: building=%s distance=%.2f materials=%d player_eye_height=1.65m fov=67 strength=%.3f same_camera=true geometry_unchanged=true" % [building.name, camera_distance, materials.size(), authored_strength])
    quit(0)
