...
func _run() -> void:
    ...
    viewport.name = "DormantMountViewport"
    ...
    if bool(surface.call("failed")) or not bool(surface.call("ready_complete")):
        _fail("surface runtime did not bind after legitimate nested generic city mount")
        return
    if bool(articulation.call("failed")) or not bool(articulation.call("ready_complete")):
        _fail("articulation runtime did not bind after legitimate nested generic city mount")
        return
    ...
    print("BRUSSELS_OSM_FACADE_DORMANT_MOUNT_OK: buildings=1 off_zone_errors=0 nested_mount=true event_driven=true geometry_changed=false source=OSM license=ODbL-1.0")
    quit(0)
