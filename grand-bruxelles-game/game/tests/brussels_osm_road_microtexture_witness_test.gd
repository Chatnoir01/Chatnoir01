...
const BUILDING_CONTROL_PATH := "res://artifacts/visual/road_microtexture_building_control.png"
...
const MIN_FRUSTUM_BUILDINGS := 3
const MIN_RENDERED_BUILDING_CHANGED_8 := 0.0200
const MIN_RENDERED_BUILDING_BBOX_W := 320
const MIN_RENDERED_BUILDING_BBOX_H := 120
...
func _aabb_corner(aabb: AABB, index: int) -> Vector3:
    return aabb.position + Vector3(
        aabb.size.x if (index & 1) != 0 else 0.0,
        aabb.size.y if (index & 2) != 0 else 0.0,
        aabb.size.z if (index & 4) != 0 else 0.0
    )

func _node_intersects_frustum(node: Node, camera: Camera3D) -> bool:
    if node is Node3D:
        var spatial := node as Node3D
        if spatial.visible and camera.is_position_in_frustum(spatial.global_position):
            return true
        if spatial.visible and spatial.has_method("get_aabb"):
            var local_bounds_variant: Variant = spatial.call("get_aabb")
            if local_bounds_variant is AABB:
                var local_bounds := local_bounds_variant as AABB
                if local_bounds.size.length_squared() > 0.0:
                    for index: int in range(8):
                        var world_corner := spatial.global_transform * _aabb_corner(local_bounds, index)
                        if camera.is_position_in_frustum(world_corner):
                            return true
    for child: Node in node.get_children():
        if _node_intersects_frustum(child, camera):
            return true
    return false

func _count_frustum_buildings(scene: Node, camera: Camera3D) -> int:
    var buildings := scene.get_node_or_null("BrusselsOSM/GeneratedBuildings")
    if buildings == null:
        return 0
    var count := 0
...
func _collect_building_geometry(node: Node, output: Array[GeometryInstance3D]) -> void:
    if node is GeometryInstance3D:
        var geometry := node as GeometryInstance3D
        if geometry.visible:
            output.append(geometry)
    for child: Node in node.get_children():
        _collect_building_geometry(child, output)

func _building_control_material() -> StandardMaterial3D:
...
func _run() -> void:
    ...
    var frustum_buildings := _count_frustum_buildings(scene, camera)
    print("BRUSSELS_ROAD_MICROTEXTURE_SCENE_RICHNESS: frustum_buildings=%d minimum=%d method=center_or_aabb_corners" % [frustum_buildings, MIN_FRUSTUM_BUILDINGS])
    if frustum_buildings < MIN_FRUSTUM_BUILDINGS:
        _fail("normal-player road witness is visually empty; insufficient static building context")
        return
    ...
    var buildings := scene.get_node_or_null("BrusselsOSM/GeneratedBuildings")
    if buildings == null:
        _fail("GeneratedBuildings missing for rendered richness control")
        return
    var building_geometry: Array[GeometryInstance3D] = []
    _collect_building_geometry(buildings, building_geometry)
    if building_geometry.is_empty():
        _fail("no static building geometry found for rendered richness control")
        return
    var original_overrides: Array[Material] = []
    var debug_building_material := _building_control_material()
    for geometry: GeometryInstance3D in building_geometry:
        original_overrides.append(geometry.material_override)
        geometry.material_override = debug_building_material
    for _frame: int in range(8):
        await process_frame
    var building_control := await _capture(viewport, BUILDING_CONTROL_PATH)
    for index: int in range(building_geometry.size()):
        building_geometry[index].material_override = original_overrides[index]
    if building_control == null:
        _fail("1280x720 rendered-building control capture failed")
        return
    var building_gt8 := _metrics(before, building_control, 8)
    var building_changed_8 := float(building_gt8["fraction"])
    var building_bbox_w := int(building_gt8["bbox_w"])
    var building_bbox_h := int(building_gt8["bbox_h"])
    print("BRUSSELS_ROAD_MICROTEXTURE_RENDERED_RICHNESS: changed_gt8=%.6f bbox=%dx%d geometries=%d" % [building_changed_8, building_bbox_w, building_bbox_h, building_geometry.size()])
    if building_changed_8 < MIN_RENDERED_BUILDING_CHANGED_8 or building_bbox_w < MIN_RENDERED_BUILDING_BBOX_W or building_bbox_h < MIN_RENDERED_BUILDING_BBOX_H:
        _fail("normal-player road witness lacks rendered static-building coverage despite frustum occupancy")
        return
    ...
    if float(bbox_w) / float(control_bbox_w) < MIN_BBOX_CONTROL_COVERAGE or float(bbox_h) / float(control_bbox_h) < MIN_BBOX_CONTROL_COVERAGE:
        ...
        return
    print("BRUSSELS_ROAD_MICROTEXTURE_VISUAL_OK: eye=1.65m fov=67 changed_gt3=%.4f%% changed_gt8=%.4f%% bbox=%dx%d control_bbox=%dx%d frustum_buildings=%d rendered_building_gt8=%.4f%% rendered_building_bbox=%dx%d geometry_changed=false composition_claimed=false" % [changed_3 * 100.0, changed_8 * 100.0, bbox_w, bbox_h, control_bbox_w, control_bbox_h, frustum_buildings, building_changed_8 * 100.0, building_bbox_w, building_bbox_h])
    quit(0)
