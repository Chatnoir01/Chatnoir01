...
func _run() -> void:
    ...
    if float(bbox_w) / float(control_bbox_w) < MIN_BBOX_CONTROL_COVERAGE or float(bbox_h) / float(control_bbox_h) < MIN_BBOX_CONTROL_COVERAGE:
        ...
        return
    print("BRUSSELS_ROAD_MICROTEXTURE_VISUAL_OK: eye=1.65m fov=67 changed_gt3=%.4f%% changed_gt8=%.4f%% bbox=%dx%d control_bbox=%dx%d frustum_buildings=%d geometry_changed=false composition_claimed=false" % [changed_3 * 100.0, changed_8 * 100.0, bbox_w, bbox_h, control_bbox_w, control_bbox_h, frustum_buildings])
    quit(0)
