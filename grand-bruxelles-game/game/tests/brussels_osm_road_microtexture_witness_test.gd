...
func _count_frustum_buildings(scene: Node, camera: Camera3D) -> int:
    ...
    for child: Node in buildings.get_children():
        ...
        if not spatial.visible:
            continue
        if camera.is_position_in_frustum(spatial.global_position):
            count += 1
    return count
...
func _run() -> void:
    ...
    print("BRUSSELS_ROAD_MICROTEXTURE_SCENE_RICHNESS: frustum_buildings=%d minimum=%d" % [frustum_buildings, MIN_FRUSTUM_BUILDINGS])
