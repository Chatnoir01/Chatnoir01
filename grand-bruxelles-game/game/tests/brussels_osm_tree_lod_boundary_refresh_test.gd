...
func _run() -> void:
    ...
    # authored 140m tree LOD boundary. Visual detail must follow that boundary.
    ...
    print("BRUSSELS_OSM_TREE_LOD_BOUNDARY_REFRESH_OK: far_at_150m=true near_at_130m=true movement_m=20 refresh_distance_m=80 source=%s license=%s" % [str(runtime.get_meta("source", "")), str(runtime.get_meta("license", ""))])
    quit(0)
