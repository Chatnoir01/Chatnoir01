...
func _run() -> void:
    ...
    if not _expect(_write_complete(shipped_compat_id,149000.0,169500.0,_manifest(shipped_compat_id,149000.0,169500.0)),"write shipped"): return
    ...
    for cell_id in [second_id,first_id,incomplete_id,mismatched_id,sealed_candidate_id,unsealed_candidate_id,shipped_compat_id]:
    ...
    if not _expect(descriptors.size()==3,"only three production-safe indexed fixtures expected, got %d"%descriptors.size()): return
    ...
    if not _expect(str((by_id[shipped_compat_id] as Dictionary).get("script_path",""))==IXELLES_NORTH_DTM_SCRIPT,"shipped specialized renderer preserved"): return
    print("BRUSSELS_RUNTIME_CELL_AUTO_DISCOVERY_OK: deterministic index drives discovery; incomplete, mismatched and candidate bundles fail closed; shipped renderer stays compatible")
    quit(0)
