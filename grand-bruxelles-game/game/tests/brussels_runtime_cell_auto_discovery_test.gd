...
func _run() -> void:
    ...
    var shipped_compat_id := "bxl-e149000-n169500-s500"
    ...
    if not _expect(_write_complete_cell(first_id, 100000.0, 200000.0), "could not write first complete cell fixture"):
        return
    if not _expect(_write_complete_cell(shipped_compat_id, 149000.0, 169500.0), "could not write shipped compatibility fixture"):
        return
    ...
    if not _expect(descriptors.size() == 3, "automatic discovery should accept only the three production-safe fixtures, got %d" % descriptors.size()):
        return

    var discovered_ids := PackedStringArray()
    var by_id: Dictionary = {}
    for descriptor: Dictionary in descriptors:
        var cell_id := str(descriptor.get("cell_id", ""))
        discovered_ids.append(cell_id)
        by_id[cell_id] = descriptor
    ...
    var sorted_ids := discovered_ids.duplicate()
    sorted_ids.sort()
    if not _expect(discovered_ids == sorted_ids, "automatic discovery should be deterministic and sorted by cell id"):
        return
    if not _expect(str((by_id[first_id] as Dictionary).get("script_path", "")) == GENERIC_SOURCE_PLAN_SCRIPT, "new regional cells must use the generic source-plan renderer"):
        return
    if not _expect(str((by_id[second_id] as Dictionary).get("script_path", "")) == GENERIC_SOURCE_PLAN_SCRIPT, "new regional cells must use the generic source-plan renderer"):
        return
    if not _expect(str((by_id[shipped_compat_id] as Dictionary).get("script_path", "")) == IXELLES_NORTH_DTM_SCRIPT, "existing shipped Ixelles North DTM cell must preserve its specialized renderer"):
        return

    print("BRUSSELS_RUNTIME_CELL_AUTO_DISCOVERY_OK: production bundles discover automatically; incomplete, mismatched and candidate bundles fail closed; shipped specialized renderers stay compatible")
    quit(0)
