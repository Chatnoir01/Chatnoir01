...
func _run() -> void:
    ...
    # The autoload must discover GeneratedRoads from the live tree instead of relying on current_scene.
    var runtime := runtime_script.new() as Node; root.add_child(runtime)
    for _frame: int in range(8):
        await process_frame
        if bool(runtime.call("ready_complete")):
            break
    if not bool(runtime.call("ready_complete")): _fail("autoload did not discover GeneratedRoads from SceneTree.root"); return
    ...
    print("BRUSSELS_SIDEWALK_EDGE_RUNTIME_OK: auto_bind=root sidewalks=2 edges=2 batches=1 collisions=0 road_facing_only=true geometry_unchanged=true source_height_claimed=false")
    quit(0)
