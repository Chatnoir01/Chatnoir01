...
func _run() -> void:
    ...
    var sidewalk_count := int(runtime.call("sidewalk_count"))
    var edge_count := int(runtime.call("edge_count"))
    var clip_count := int(runtime.call("intersection_clip_count"))
    if sidewalk_count != 430: _fail("production sidewalk reuse count changed"); return
    if edge_count <= 0 or clip_count <= 0: _fail("intersection-safe fascia segmentation missing"); return
    ...
    print("BRUSSELS_SIDEWALK_EDGE_VISUAL_METRICS: length=%.2f width=%.2f bourse_distance=%.2f changed_gt3=%.6f changed_gt8=%.6f bbox=%dx%d sidewalks=%d edges=%d intersection_clips=%d batches=1 collisions=0" % [sidewalk.size.z, sidewalk.size.x, Vector2(sidewalk.global_position.x,sidewalk.global_position.z).distance_to(BOURSE), changed3, changed8, bbox.size.x, bbox.size.y, sidewalk_count, edge_count, clip_count])
    ...
    print("BRUSSELS_SIDEWALK_EDGE_VISUAL_OK: player_eye_height=1.65m fov=67 changed_gt3=%.4f%% changed_gt8=%.4f%% bbox=%dx%d intersection_clipped=true geometry_unchanged=true" % [changed3*100.0,changed8*100.0,bbox.size.x,bbox.size.y])
    quit(0)
