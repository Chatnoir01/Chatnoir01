...
func _run() -> void:
    ...
    # Regression: an OFF request while the runtime is dormant must be inherited
    # by both visuals and owned collisions when the legitimate scene later mounts.
    ...
    # Foreign children may coexist under the owned collision body. The runtime
    # must never mutate or count a collider that it did not create.
