...
func _run() -> void:
    ...
    # Production/performance harnesses can instantiate main.tscn under SceneTree.root
    # without assigning SceneTree.current_scene. The autoload must discover that real
    # scene itself; manual bind_scene() is intentionally forbidden in this regression.
