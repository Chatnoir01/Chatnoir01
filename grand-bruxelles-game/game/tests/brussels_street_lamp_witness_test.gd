...
const WIDTH := 1280
const HEIGHT := 720
const BEFORE_PATH := "res://artifacts/visual/street_lamps_before.png"
const AFTER_PATH := "res://artifacts/visual/street_lamps_after.png"
...
func _fail(message: String) -> void:
    push_error("BRUSSELS_STREET_LAMP_VISUAL_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if runtime == null:
        _fail("production street lamp autoload missing")
        return
    runtime.call("bind_scene", scene)
    for _frame: int in range(12):
        await process_frame
    if not bool(runtime.call("ready_complete")) or bool(runtime.call("failed")):
        _fail("production street lamp runtime did not bind cleanly")
        return
    ...
    print("BRUSSELS_STREET_LAMP_VISUAL_METRICS: camera=%d changed_gt3=%.6f changed_gt8=%.6f bbox=%dx%d points=%d batches=%d" % [best_index, changed3, changed8, bbox.size.x, bbox.size.y, int(runtime.call("point_count")), int(runtime.call("visual_batch_count"))])
    if changed3 < MIN_CHANGED_3 or changed8 < MIN_CHANGED_8:
        _fail("full-frame street lamp impact too weak: gt3=%.4f%% gt8=%.4f%%" % [changed3 * 100.0, changed8 * 100.0])
        return
    if bbox.size.x < MIN_BBOX_WIDTH or bbox.size.y < MIN_BBOX_HEIGHT:
        _fail("street lamp change bbox too small: %dx%d" % [bbox.size.x, bbox.size.y])
        return
    if not bool(runtime.call("source_positions_unchanged")):
        _fail("source positions changed during A/B")
        return
    print("BRUSSELS_STREET_LAMP_VISUAL_OK: player_eye_height=1.65m fov=67 camera=%d source_positions_unchanged=true changed_gt3=%.4f%% changed_gt8=%.4f%% bbox=%dx%d" % [best_index, changed3 * 100.0, changed8 * 100.0, bbox.size.x, bbox.size.y])
    quit(0)
