...
func _run() -> void:
    ...
    if changed_gt3 < MIN_CHANGED_GT3:
        ...
        return
    if changed_gt3 > MAX_CHANGED_GT3 or changed_gt8 > MAX_CHANGED_GT8:
        _fail("tree LOD changes too much of the player frame: gt3=%.4f%% gt8=%.4f%%" % [changed_gt3 * 100.0, changed_gt8 * 100.0])
        return

    print("BRUSSELS_TREE_DISTANCE_LOD_VISUAL_METRICS: trees=%d near=%d far=%d foliage=%d->%d reduction=%.2f%% changed_gt3=%.4f%% changed_gt8=%.4f%% bbox=%dx%d" % [EXPECTED_RENDERED_TREES, near_count, far_count, baseline_foliage, optimized_foliage, reduction * 100.0, changed_gt3 * 100.0, changed_gt8 * 100.0, int(metrics["bbox_width"]), int(metrics["bbox_height"])])
    print("BRUSSELS_TREE_DISTANCE_LOD_VISUAL_OK: camera_eye=1.65m fov=70 source_positions_unchanged=true batches=3")
    quit(0)
