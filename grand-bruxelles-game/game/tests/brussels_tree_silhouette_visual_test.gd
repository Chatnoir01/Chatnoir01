...
func _run() -> void:
    ...
    if runtime == null:
        ...
        return
    # SceneTree.current_scene is not populated when a SceneTree test instantiates
    # the production scene manually. Use the runtime's explicit test binding so
    # this witness exercises the same build path without inventing a fake scene.
