...
func _run() -> void:
    ...
    if not _expect(target.material_override is ShaderMaterial, "candidate presentation material was not applied"):
        return
    if not _expect(bool(target.get_meta("unclassified_massing_presentation", false)), "presentation metadata missing"):
        return
    if not _expect(not bool(target.get_meta("material_identity_claimed", true)), "runtime claimed a real material identity"):
        return

    runtime.call("set_presentation_enabled", false)
    await process_frame
    if not _expect(target.material_override is StandardMaterial3D, "baseline material toggle failed"):
        return
    var baseline := target.material_override as StandardMaterial3D
    if not _expect(baseline.albedo_color.is_equal_approx(Color(0.60, 0.53, 0.45, 1.0)), "baseline color drifted"):
        return
    if not _expect(is_equal_approx(baseline.roughness, 0.90), "baseline roughness drifted"):
        return

    runtime.call("set_presentation_enabled", true)
    await process_frame
    if not _expect(target.material_override is ShaderMaterial, "candidate material restore failed"):
        return
    ...
    for _frame: int in range(2):
        await process_frame
    if not _expect(unsafe.material_override == null, "runtime-approved geometry was unexpectedly restyled"):
        return
