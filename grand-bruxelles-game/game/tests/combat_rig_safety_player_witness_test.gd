...
func _run() -> void:
    if root.get_node_or_null("CombatAuthoredPoseRuntime") != null:
        _fail("unsafe CombatAuthoredPoseRuntime is active in production autoloads")
        return
    ...
    print("COMBAT_RIG_SAFETY_WITNESS_OK: authored_skeleton=true unsafe_pose_autoload=false max_bone_shift_m=%.3f report=%s" % [MAX_BONE_SHIFT_M, JSON.stringify(report)])
    quit(0)
