...
func _run() -> void:
    ...
    if runtime_script == null:
        _fail("upper-body runtime script failed to load")
        return
    var runtime_variant: Variant = runtime_script.new()
    if not runtime_variant is Node:
        _fail("upper-body runtime did not instantiate as Node")
        return
    var runtime := runtime_variant as Node
    runtime.name = "CombatWeaponUpperBodyRuntimeMetrics"
    root.add_child(runtime)
    if not runtime.has_method("set_pose_enabled"):
        _fail("upper-body runtime missing set_pose_enabled")
        return

    runtime.call("set_pose_enabled", false)
    for _frame: int in range(45):
        await process_frame
    # SkeletonIK3D is a SkeletonModifier3D. Read bone poses only after
    # skeleton_updated so all modifiers for that frame are already applied.
    ...
    print("COMBAT_WEAPON_UPPER_BODY_METRICS: weapon=%s right_gap=%.4f support_gap=%.4f right_raise=%.4f left_motion=%.4f right_root=%s right_forearm=%s left_root=%s left_forearm=%s right_chain=%s left_chain=%s" % [
        String(WEAPON_ID),
        right_gap,
        support_gap,
        right_raise,
    ...
    print("COMBAT_WEAPON_UPPER_BODY_METRICS_OK: right_gap=%.3f support_gap=%.3f raise=%.3f left_motion=%.3f root=%s forearm=%s" % [right_gap, support_gap, right_raise, left_motion, right_root, right_forearm])
    quit(0)