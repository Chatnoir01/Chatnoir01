...
func _weight_rows(w: Dictionary, skin: Skin, skeleton: Skeleton3D) -> Array:
    ...
    for key in keys:
        ...
        var skeleton_bone_name := ""
        var skeleton_rest_ok := false
        if skin != null and bind_index >= 0 and bind_index < skin.get_bind_count():
            bind_name = str(skin.get_bind_name(bind_index))
            bind_bone = skin.get_bind_bone(bind_index)
            bind_pose_ok = _finite_transform(skin.get_bind_pose(bind_index))
        if skeleton != null and bind_bone >= 0 and bind_bone < skeleton.get_bone_count():
            skeleton_bone_name = skeleton.get_bone_name(bind_bone)
            skeleton_rest_ok = _finite_transform(skeleton.get_bone_rest(bind_bone))
        rows.append({
            "bind_index": bind_index,
            "weight": float(w[key]),
            "bind_name": bind_name,
            "bind_bone": bind_bone,
            "skeleton_bone_name": skeleton_bone_name,
            "bind_pose_finite": bind_pose_ok,
            "skeleton_rest_finite": skeleton_rest_ok
        })
    return rows
...
func _initialize() -> void:
    ...
    for raw_vi in contract.get("probe_vertices", []):
        ...
        probe_rows[str(vi)] = {
            "weight_sum": sum,
            "influences": _weight_rows(w, skin, skeleton)
        }
    ...
    for row in probe_rows.values():
        for influence in row.get("influences", []):
            if not bool(influence.get("bind_pose_finite", false)) or not bool(influence.get("skeleton_rest_finite", false)):
                rig_invalid = true
    ...
    _write({
        "format": "grand-bruxelles-gate8-variant01-export-skin-origin-diagnostic-result-v1",
