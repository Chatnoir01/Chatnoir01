...
RESULT_PATH = Path(os.environ["GATE8_LOCAL601_BLEND_RESULT"]).resolve()
...
def main() -> None:
    ...
    def wrapped_create_copy(root, *args, **kwargs):
        ...
        for alpha in ALPHAS:
            variants.append((alpha, duplicate_mesh(sportsuit, f"gate8_probe_local601_blend_{int(alpha * 1000):03d}")))
        ...
        for vertex_index in FOCUS_VERTICES:
            ...
            for _, variant in variants:
                set_vertex_weights(variant, vertex_index, stored_weights)

        record601 = records["601"]
        stored601 = record601["stored_mpfb_deform_weights"]
        rematched601 = record601["rematched_mpfb_deform_weights"]
        for alpha, variant in variants:
            set_vertex_weights(variant, 601, blend_weights(stored601, rematched601, alpha))
        ...
        for alpha, variant in variants:
            ...
            candidates.append({
                ...
                "vertex_601_stored_vs_candidate_delta_m": float((posed[601] - stored_posed[601]).length),
                "inversion_count": inversion_count(sportsuit, rest, posed),
                "local_edges": edges,
            })

        measured = {
            "format": "grand-bruxelles-gate8-variant01-local601-blend-sweep-v1-red-first",
            "candidate_strategy": "LINEAR_BLEND_STORED_TO_REMATCHED_WEIGHTS_AT_VERTEX_601_ONLY",
            "alphas": list(ALPHAS),
            "source_evidence_artifact_id": 9720026708,
            "source_evidence_head_sha": "9defe931b0b51b3582c1e555ff80992f64f311fc",
            "rejected_full_rematch_artifact_id": 9723546302,
            "rejected_full_rematch_digest": "sha256:99da25f988c5c472fab825c669146f7d0ac5281110fdad1eda76c6771b6eb277",
            "mpfb_release": "2.0.17",
            ...
            "diagnostic_state": "RED_FIRST_LOCAL601_BLEND_SWEEP_MEASURED_UNREVIEWED",
            "next_safe_axis": "REVIEW_PARETO_FRONTIER_CRITICAL_CONTROL_DISPLACEMENT_AND_INVERSION",
        ...
        raise StopAfterMeasurement("local601 blend sweep measured")
    ...
    if measured is None or not RESULT_PATH.is_file():
        raise RuntimeError("local601 blend sweep did not produce a result")
