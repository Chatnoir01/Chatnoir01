...
RESULT_PATH = Path(os.environ["GATE8_LOCAL601_RESULT"]).resolve()
...
def edge_record(a: int, b: int, rest: list[Vector], stored: list[Vector], candidate: list[Vector]) -> dict[str, Any]:
    rest_len = float((rest[a] - rest[b]).length)
    stored_len = float((stored[a] - stored[b]).length)
    candidate_len = float((candidate[a] - candidate[b]).length)
    if rest_len <= 1e-9:
        raise RuntimeError(f"degenerate rest edge {a}<->{b}")
    stored_ratio = stored_len / rest_len
    candidate_ratio = candidate_len / rest_len
    return {
        "edge": [a, b],
        "critical": (a, b) in CRITICAL_EDGES,
        "touches_candidate_vertex": 601 in (a, b),
        "rest_length_m": rest_len,
        "stored_posed_length_m": stored_len,
        "candidate_posed_length_m": candidate_len,
        "stored_stretch_ratio": stored_ratio,
        "candidate_stretch_ratio": candidate_ratio,
        "stored_abs_strain": abs(stored_ratio - 1.0),
        "candidate_abs_strain": abs(candidate_ratio - 1.0),
    }
...
def incident_polygons(obj: bpy.types.Object) -> list[bpy.types.MeshPolygon]:
    ...
    for polygon in obj.data.polygons:
        ...
        polygon_edges = {frozenset((verts[i], verts[(i + 1) % len(verts)])) for i in range(len(verts))}
...
def inversion_records(obj: bpy.types.Object, rest: list[Vector], stored: list[Vector], candidate: list[Vector]) -> list[dict[str, Any]]:
    ...
    for polygon in incident_polygons(obj):
        ...
        candidate_n = polygon_normal([candidate[v] for v in verts])
        if min(rest_n.length, stored_n.length, candidate_n.length) <= 1e-12:
        ...
        candidate_dot = float(rest_n.normalized().dot(candidate_n.normalized()))
        records.append({
            "polygon_index": int(polygon.index),
            "vertices": verts,
            "stored_rest_normal_dot": stored_dot,
            "candidate_rest_normal_dot": candidate_dot,
            "stored_inverted": stored_dot < 0.0,
            "candidate_inverted": candidate_dot < 0.0,
        })
    return records


def displacement_records(rest: list[Vector], stored: list[Vector], candidate: list[Vector]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for vertex_index in FOCUS_VERTICES:
        out[str(vertex_index)] = {
            "stored_displacement_m": float((stored[vertex_index] - rest[vertex_index]).length),
            "candidate_displacement_m": float((candidate[vertex_index] - rest[vertex_index]).length),
            "stored_vs_candidate_delta_m": float((candidate[vertex_index] - stored[vertex_index]).length),
        }
    return out
...
def main() -> None:
    ...
    def wrapped_create_copy(root, *args, **kwargs):
        ...
        candidate_variant = duplicate_mesh(sportsuit, "gate8_probe_local601_weights")
        for vertex_index in FOCUS_VERTICES:
            ...
            set_vertex_weights(candidate_variant, vertex_index, stored_weights)
        record601 = records["601"]
        set_vertex_weights(candidate_variant, 601, record601["rematched_mpfb_deform_weights"])

        rest = rest_world_positions(sportsuit)
        apply_pose(rig)
        stored_posed = evaluated_world_positions(stored_variant)
        candidate_posed = evaluated_world_positions(candidate_variant)
        edges = [edge_record(a, b, rest, stored_posed, candidate_posed) for a, b in LOCAL_EDGES]
        inversions = inversion_records(sportsuit, rest, stored_posed, candidate_posed)
        displacements = displacement_records(rest, stored_posed, candidate_posed)
        critical = [edge for edge in edges if edge["critical"]]
        controls = [edge for edge in edges if not edge["critical"]]

        for point in [*rest, *stored_posed, *candidate_posed]:
            if not all(math.isfinite(float(value)) for value in point):
                raise RuntimeError("non-finite posed geometry")

        measured = {
            "format": "grand-bruxelles-gate8-variant01-local601-real-posed-geometry-v1-red-first",
            "candidate_strategy": "REMATCH_ONLY_SPORTSUIT_VERTEX_601_KEEP_OTHER_FOCUS_VERTICES_STORED",
            ...
            "candidate_vertices": list(CANDIDATE_VERTICES),
            "local_edges": edges,
            "critical_edge_mean_stored_abs_strain": sum(x["stored_abs_strain"] for x in critical) / len(critical),
            "critical_edge_mean_candidate_abs_strain": sum(x["candidate_abs_strain"] for x in critical) / len(critical),
            "control_edge_mean_stored_abs_strain": sum(x["stored_abs_strain"] for x in controls) / len(controls),
            "control_edge_mean_candidate_abs_strain": sum(x["candidate_abs_strain"] for x in controls) / len(controls),
            ...
            "candidate_inversion_count": sum(1 for x in inversions if x["candidate_inverted"]),
            "diagnostic_state": "RED_FIRST_LOCAL601_REAL_POSED_GEOMETRY_MEASURED_UNREVIEWED",
            "next_safe_axis": "REVIEW_LOCAL601_CRITICAL_CONTROL_STRAIN_DISPLACEMENT_AND_INVERSION",
        ...
        raise StopAfterMeasurement("local601 real posed geometry measured")
    ...
    if measured is None or not RESULT_PATH.is_file():
        raise RuntimeError("local601 real posed geometry probe did not produce a result")
