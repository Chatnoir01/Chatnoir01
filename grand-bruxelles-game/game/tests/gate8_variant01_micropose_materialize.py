...
REPLACEMENTS = {
    "npc_gate_06.glb": "npc_gate_01.glb",
    "gate8_variant06_target_micropose_skin_result.json": "gate8_variant01_target_micropose_skin_result.json",
    "EXPECTED_VERTICES := 24073": "EXPECTED_VERTICES := 21044",
    "variant06": "variant01",
    "Variant06": "Variant01",
    "VARIANT06": "VARIANT01",
    "candidate_variant\":6": "candidate_variant\":1",
    "REJECT_VARIANT06_DYNAMIC_TARGET_SKIN_OR_REWEIGHT_REQUIRED": "REJECT_VARIANT01_DYNAMIC_TARGET_SKIN_OR_REWEIGHT_REQUIRED",
}
...
REQUIRED_OUTPUT_TOKENS = (
    'res://assets/npc_gate_01.glb',
    'gate8_variant01_target_micropose_skin_result.json',
    'const EXPECTED_VERTICES := 21044',
    '"candidate_variant":1',
    'grand-bruxelles-gate8-variant01-target-micropose-skin-v1',
    'REJECT_VARIANT01_DYNAMIC_TARGET_SKIN_OR_REWEIGHT_REQUIRED',
)
...
def materialize(source: str) -> str:
    ...
    for token in REQUIRED_OUTPUT_TOKENS:
        if token not in output:
            raise AssertionError(f"variant01 materialization incomplete: missing {token!r}")
    for token in FORBIDDEN_OUTPUT_TOKENS:
        if token in output:
            raise AssertionError(f"variant01 materialization leaked forbidden token {token!r}")
...
def main() -> None:
    ...
    print("VARIANT01_MICROPOSE_HARNESS_MATERIALIZED_OK")
