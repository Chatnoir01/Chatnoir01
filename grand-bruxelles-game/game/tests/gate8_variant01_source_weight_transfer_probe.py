...
def find_sportsuit(root: bpy.types.Object) -> bpy.types.Object:
    matches = [obj for obj in ready.base.descendants(root) if obj.type == "MESH" and OBJECT_FRAGMENT in obj.name.lower()]
    if len(matches) != 1:
...
def snapshot(root: bpy.types.Object, stage: str) -> dict[str, Any]:
    ...
    for vertex in obj.data.vertices:
        ...
        vertices.append({"index": int(vertex.index), "local_xyz": local, "world_xyz": tuple(float(v) for v in world_v), "weights": weights(obj, vertex)})
    return {"stage": stage, "object": obj.name, "vertex_count": len(vertices), "edge_count": len(edges), "edges": edges, "vertices": vertices}
...
def map_endpoint(target: dict[str, Any], earlier: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"vertex": int(best["index"]), "position_distance_m": float(best_distance), "weights": best["weights"], "weight_delta_to_prepared_l1": l1(best["weights"], target["weights"])}
...
def stage_transfer(prepared: dict[str, Any], earlier: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"stage": earlier["stage"], "mapped_vertices": [ma["vertex"], mb["vertex"]], "mapped_pair_is_native_edge": pair in earlier["edges"], "endpoint_a": ma, "endpoint_b": mb, "mapped_edge_weight_l1": cliff, "mapped_edge_is_cliff": cliff >= CLIFF_L1, "positions_resolved": ma["position_distance_m"] <= POSITION_TOL and mb["position_distance_m"] <= POSITION_TOL}
...
def main() -> None:
    ...
    def wrapped_export(prepared_root: bpy.types.Object, output_path: Path) -> dict:
        ...
        stage_order = ["source_before_copy", "copy_after_create", "copy_after_target_bake", "copy_before_helper_bake", "copy_after_helper_bake"]
        ...
        if not transfers or not all(t["positions_resolved"] for t in transfers):
            state, next_axis = "SOURCE_WEIGHT_TRANSFER_POSITION_MAPPING_UNRESOLVED", "TRACE_SOURCE_TO_EXPORT_COPY_VERTEX_MAPPING"
        else:
            cliff_stages = [t["stage"] for t in transfers if t["mapped_edge_is_cliff"]]
            if "source_before_copy" in cliff_stages:
                state, next_axis = "SOURCE_SPORTSUIT_WEIGHT_CLIFF_ALREADY_PRESENT", "TRACE_SOURCE_ASSET_WEIGHT_ORIGIN"
            else:
                first = next((t["stage"] for t in transfers if t["mapped_edge_is_cliff"]), None)
                mapping = {
                    "copy_after_create": ("WEIGHT_CLIFF_INTRODUCED_BY_EXPORT_COPY", "ISOLATE_CREATE_CHARACTER_COPY_WEIGHT_MUTATION"),
                    "copy_after_target_bake": ("WEIGHT_CLIFF_INTRODUCED_DURING_TARGET_BAKE", "ISOLATE_TARGET_BAKE_PROXY_WEIGHT_MUTATION"),
                    "copy_before_helper_bake": ("WEIGHT_CLIFF_INTRODUCED_DURING_TARGET_BAKE", "ISOLATE_TARGET_BAKE_PROXY_WEIGHT_MUTATION"),
                    "copy_after_helper_bake": ("WEIGHT_CLIFF_INTRODUCED_DURING_HELPER_BAKE", "ISOLATE_BAKE_MODIFIERS_PROXY_WEIGHT_MUTATION"),
                }
                state, next_axis = mapping.get(first, ("SOURCE_WEIGHT_TRANSFER_STAGE_NOT_IDENTIFIED", "STOP_AND_INSPECT_WEIGHT_TRANSFER"))
        result = {"format": "grand-bruxelles-gate8-variant01-source-weight-transfer-v1", "diagnostic_state": state, "next_safe_axis": next_axis, "generated_glb_sha256": digest, "generated_glb_size_bytes": size, "generated_seed": int(record["seed"]), "source_head_sha": "afcb7b352ed054d98fdf83eae3333ec82c814b3e", "prepared_native_edge": [486, 601], "prepared_edge_weight_l1": prepared_cliff, "position_tolerance_m": POSITION_TOL, "weight_tolerance_l1": WEIGHT_TOL, "cliff_threshold_l1": CLIFF_L1, "transfers": transfers, "canonical_asset_mutation": False, "canonical_generator_mutation": False, "runtime_npc_mutation": False, "production_activation_allowed": False, "visual_approval_allowed": False}
