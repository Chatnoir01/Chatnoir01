...
func _finish() -> void:
    ...
    var result := {
        "format": "grand-bruxelles-gate8-variant01-target-integrity-v1",
        "variant": 1,
        ...
        "failures": failures,
        "state": "TARGET01_EXACT_INTEGRITY_READY" if failures.is_empty() else "TARGET01_EXACT_INTEGRITY_BLOCKED",
    }
    var file := FileAccess.open("res://gate8_variant01_target_integrity.json", FileAccess.WRITE)
    file.store_string(JSON.stringify(result, "  ") + "\n")
    file.close()
    print("GATE8_VARIANT01_TARGET_INTEGRITY state=%s bones=%d meshes=%d surfaces=%d materials=%d vertices=%d failures=%d" % [result.state, bone_count, skinned_meshes, skinned_surfaces, material_surfaces, vertex_count, failures.size()])
