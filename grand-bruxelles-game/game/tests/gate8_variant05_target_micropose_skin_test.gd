...
const TARGET_SCENE := "res://assets/npc_gate_05.glb"
const OUTPUT_PATH := "res://gate8_variant05_target_micropose_skin_result.json"
...
func _run() -> void:
    ...
    for role_value in MICROPOSE_ROLES:
        ...
        for axis_idx in range(AXIS_VECTORS.size()):
            ...
            for sign_f in [-1.0, 1.0]:
                ...
                var foot_drift := maxf(left_foot_rest.distance_to(target.get_bone_global_pose(left_foot_idx).origin), right_foot_rest.distance_to(target.get_bone_global_pose(right_foot_idx).origin))
                ...
                rows.append({"case": case_name, "role": role, "bone": String(target.get_bone_name(bone_idx)), "axis": axis_name, "degrees": MICROPOSE_DEG * float(sign_f), "max_edge_absolute_change_m": edge_change, "max_edge_stretch_ratio": stretch, "min_edge_compression_ratio": compression, "max_foot_drift_m": foot_drift, "within_gates": within})
    ...
    result["next_safe_axis"] = "SOURCE_TO_TARGET_POSE_AMPLITUDE_OR_REST_FRAME" if microposes_green else "REJECT_VARIANT05_DYNAMIC_TARGET_SKIN_OR_REWEIGHT_REQUIRED"
    _finish(result)

func _base_result(state: String) -> Dictionary:
    return {"format":"grand-bruxelles-gate8-variant05-target-micropose-skin-v1","candidate_variant":5,"measurement":"target_only_controlled_micropose_cpu_skin_space","source_animation_used":false,"retarget_applied":false,"target_skin_modified":false,"target_rest_modified":false,"threshold_changed":false,"max_edge_absolute_change_allowed_m":MAX_EDGE_ABSOLUTE_CHANGE_M,"max_edge_stretch_allowed_ratio":MAX_EDGE_STRETCH_RATIO,"min_edge_compression_allowed_ratio":MIN_EDGE_COMPRESSION_RATIO,"max_foot_drift_allowed_m":MAX_FOOT_DRIFT_M,"diagnostic_state":state,"walk_alias_selected":"","run_alias_selected":"","production_authorized":false,"activation_ready":false,"adoption_ready":false,"runtime_population_changed":false,"visual_witness_authorized":false,"visual_approval_claimed":false,"player_character_reuse_allowed":false,"mixamo_payload_allowed":false,"failures":failures}
...
func _capture_rest_snapshot() -> Array[Transform3D]:
    ...
    for i in range(target.get_bone_count()): out.append(target.get_bone_rest(i))
    return out
...
func _capture_skinned_positions() -> Dictionary:
    ...
    for mi in meshes:
        ...
        for s in range(mi.mesh.get_surface_count()):
            if mi.mesh.surface_get_primitive_type(s) != Mesh.PRIMITIVE_TRIANGLES: continue
            var arrays := mi.mesh.surface_get_arrays(s)
            var vv: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
            var bb = arrays[Mesh.ARRAY_BONES]
            var ww = arrays[Mesh.ARRAY_WEIGHTS]
            if vv.is_empty() or bb.size() != ww.size() or bb.size() % vv.size() != 0:
                failures.append("skin_array_shape_invalid=%s:%d" % [mi.name, s]); continue
            var n := int(bb.size() / vv.size())
            var posed := PackedVector3Array(); posed.resize(vv.size())
            for vi in range(vv.size()): posed[vi] = common_inverse * _skin_vertex(skin, vv[vi], bb, ww, vi, n)
...
func _skin_vertex(skin: Skin, vertex: Vector3, bones, weights, vi: int, n: int) -> Vector3:
    ...
    for j in range(n):
        ...
        if bind_idx < 0 or bind_idx >= skin.get_bind_count(): failures.append("bind_index_out_of_range=%d" % bind_idx); continue
        var bone_idx := _resolve_skin_bone(skin, bind_idx)
        if bone_idx < 0: failures.append("skin_bind_unresolved=%d" % bind_idx); continue
...
func _measure_edge_distortion(rest_positions: Dictionary, posed_positions: Dictionary) -> Dictionary:
    ...
    for mi in meshes:
        for s in range(mi.mesh.get_surface_count()):
            ...
            for tri in range(count):
                var ids := [indices[tri*3] if not indices.is_empty() else tri*3, indices[tri*3+1] if not indices.is_empty() else tri*3+1, indices[tri*3+2] if not indices.is_empty() else tri*3+2]
                triangles += 1
                for pair in [[0,1,"01"],[1,2,"12"],[2,0,"20"]]:
                    var a := int(ids[int(pair[0])]); var b := int(ids[int(pair[1])])
                    var rl := rest[a].distance_to(rest[b])
                    if rl <= 0.000001: continue
                    var pl := posed[a].distance_to(posed[b])
                    var ratio := pl/rl
                    var change := absf(pl-rl)
                    max_ratio = maxf(max_ratio, ratio); min_ratio = minf(min_ratio, ratio)
                    if change > max_abs:
                        max_abs = change
    ...
    if triangles <= 0: failures.append("no_triangles_measured")
...
func _ensure_common_bind_origin() -> bool:
    ...
    for mi in meshes:
        ...
        for bind_idx in range(skin.get_bind_count()):
            var bone_idx := _resolve_skin_bone(skin, bind_idx)
            if bone_idx < 0: failures.append("normalized_bind_unresolved=%d" % bind_idx); continue
            var product := target.get_bone_global_rest(bone_idx) * skin.get_bind_pose(bind_idx)
            if not have:
                reference = product; have = true
            else:
            ...
            bind_observations += 1
    if not have: failures.append("common_bind_origin_missing"); return false
    ...
    if max_bind_translation_delta_m > COMMON_TRANSLATION_EPS_M: failures.append("common_bind_translation_spread=%.9f" % max_bind_translation_delta_m)
    if max_bind_rotation_delta_deg > COMMON_ROTATION_EPS_DEG: failures.append("common_bind_rotation_spread=%.9f" % max_bind_rotation_delta_deg)
...
func _collect_skinned_meshes(node: Node, out: Array[MeshInstance3D]) -> void:
    if node is MeshInstance3D:
        ...
        if mi.mesh != null and mi.skin != null: out.append(mi)
    for child in node.get_children(): _collect_skinned_meshes(child, out)
...
func _synthetic_rigid_edge_regression() -> bool:
    ...
    for pair in [[0,1],[1,2],[2,0]]:
        if absf(p[int(pair[0])].distance_to(p[int(pair[1])]) - (t*p[int(pair[0])]).distance_to(t*p[int(pair[1])])) > 0.00001: return false
    return true
...
func _finish(result: Dictionary) -> void:
    ...
    if file == null:
        push_error("result_file_open_failed"); quit(1); return
    file.store_string(JSON.stringify(result, "\t")); file.close()
    if not failures.is_empty():
        for failure in failures: push_error(failure)
        print("GATE8_VARIANT05_TARGET_MICROPOSE_BLOCKED state=%s failures=%d" % [String(result.get("diagnostic_state","unknown")), failures.size()])
        quit(1); return
    print("GATE8_VARIANT05_TARGET_MICROPOSE state=%s cases=%d blocked=%d edge=%.9f stretch=%.9f compression=%.9f" % [String(result.get("diagnostic_state","unknown")),int(result.get("case_count",0)),int(result.get("blocked_case_count",0)),float(result.get("max_edge_absolute_change_m",0.0)),float(result.get("max_edge_stretch_ratio",1.0)),float(result.get("min_edge_compression_ratio",1.0))])
    quit(0)
