...
const RUNTIME_NAME := "GrandPlace1786758ThreeBayRuntime"
const WIDTH := 1280
const HEIGHT := 720
const CAMERA_POSITION := Vector3(319.01, 1.72, -535.20)
const CAMERA_TARGET := Vector3(315.3, 12.5, -490.7)
const CAMERA_FOV := 62.0
const MIN_CHANGED_3 := 1.0
const MIN_CHANGED_8 := 0.6

func _initialize() -> void:
    call_deferred("_run")

func _fail(message: String) -> void:
    push_error("GRAND_PLACE_1786758_THREE_BAY_AB_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    for _frame: int in range(6):
        await process_frame
    var before := await _capture("/tmp/grand-place-1786758-three-bay-before.png")
    runtime.call("set_articulation_visible", true)
    for _frame: int in range(6):
        await process_frame
    var after := await _capture("/tmp/grand-place-1786758-three-bay-after.png")
    ...
    if p3 < MIN_CHANGED_3 or p8 < MIN_CHANGED_8:
        _fail("anti-micro failed changed3=%.4f%% changed8=%.4f%% gates=%.2f/%.2f" % [p3, p8, MIN_CHANGED_3, MIN_CHANGED_8])
        return
    print("GRAND_PLACE_1786758_THREE_BAY_AB_OK: changed3=%.4f%% changed8=%.4f%% player_spawn=true dynamics_masked=true" % [p3, p8])
    quit(0)
