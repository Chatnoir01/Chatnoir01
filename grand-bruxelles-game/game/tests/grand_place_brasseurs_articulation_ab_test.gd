...
const RUNTIME_NAME := "GrandPlaceBrasseursArticulationRuntime"
const WIDTH := 1280
const HEIGHT := 720
...
func _fail(message: String) -> void:
    push_error("GRAND_PLACE_BRASSEURS_ARTICULATION_AB_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    var before := await _capture("/tmp/grand-place-brasseurs-articulation-before.png")
    runtime.call("set_articulation_visible", true)
    for _frame: int in range(6): await process_frame
    var after := await _capture("/tmp/grand-place-brasseurs-articulation-after.png")
    ...
    if max_x < min_x or max_y < min_y:
        ...
        return
    print("GRAND_PLACE_BRASSEURS_ARTICULATION_AB_OK: changed3=%.4f%% changed8=%.4f%% bbox=(%d,%d)-(%d,%d) player_spawn=true ui_masked=true dynamics_masked=true thresholds=1.20/0.70" % [p3,p8,min_x,min_y,max_x,max_y])
    quit(0)
