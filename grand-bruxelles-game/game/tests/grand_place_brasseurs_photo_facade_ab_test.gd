...
func _run() -> void:
    ...
    camera.name = "BrasseursEvidenceCamera"
    ...
    var before := await _capture("/tmp/grand-place-brasseurs-photo-before.png")
    runtime.call("set_facade_visible", true)
    for _frame: int in range(8): await process_frame
    var after := await _capture("/tmp/grand-place-brasseurs-photo-after.png")
    ...
    if max_x < min_x or max_y < min_y:
        ...
        return
    print("GRAND_PLACE_BRASSEURS_PHOTO_FACADE_AB_OK: changed3=%.4f%% changed8=%.4f%% bbox=(%d,%d)-(%d,%d) player_eye=true ui_masked=true dynamics_masked=true thresholds=1.20/0.70" % [p3, p8, min_x, min_y, max_x, max_y])
    quit(0)
