...
const MIN_CHANGED3_PERCENT := 0.35
const MIN_CHANGED8_PERCENT := 0.20

func _initialize() -> void:
    call_deferred("_run")

func _fail(message: String) -> void:
    push_error("BRASSEURS_PHOTO_AB_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if runtime == null: _fail("runtime missing"); return
    ...
    if not bool(runtime.get("facade_ready")): _fail("runtime not ready"); return
    _freeze(main)
    _mask_ui()
    ...
    if p3 < MIN_CHANGED3_PERCENT or p8 < MIN_CHANGED8_PERCENT:
        _fail("anti-micro changed3=%.4f%% changed8=%.4f%% thresholds=%.2f/%.2f" % [p3, p8, MIN_CHANGED3_PERCENT, MIN_CHANGED8_PERCENT]); return
    print("BRASSEURS_PHOTO_AB_OK: changed3=%.4f%% changed8=%.4f%% bbox=(%d,%d)-(%d,%d) player_spawn=true" % [p3, p8, min_x, min_y, max_x, max_y])
    quit(0)
