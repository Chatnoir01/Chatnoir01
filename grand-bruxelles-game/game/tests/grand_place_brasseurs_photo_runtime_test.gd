...
func _run() -> void:
    ...
    if str(placement.get("building_id", "")) != "1639974":
        _fail("plan building identity drifted")
        return
    if str(placement.get("front_wall_id", "")) != "10945501":
        _fail("plan wall identity drifted")
        return
    if abs(float(placement.get("front_wall_span_m", 0.0)) - EXPECTED_SPAN_M) > 0.000001:
        _fail("plan official wall span drifted")
        return
    if str(placement.get("vertical_world_scale", "")) != "unresolved":
        _fail("photo plan must not silently claim vertical world scale")
        return
    if str(source.get("download_sha256", "")) != EXPECTED_SOURCE_SHA256:
        _fail("photo source hash drifted")
        return
    var stored_offsets: Array = derived.get("column_center_offsets_from_left_m", [])
    if stored_offsets.size() != EXPECTED_OFFSETS.size():
        _fail("photo plan must expose four column offsets")
        return
    for i: int in range(EXPECTED_OFFSETS.size()):
        if abs(float(stored_offsets[i]) - float(EXPECTED_OFFSETS[i])) > 0.001:
            _fail("photo-plan column offset %d drifted" % i)
            return

    var runtime := root.get_node_or_null(AUTOLOAD)
    if runtime == null:
        _fail("red-first witness: photo-constrained Brasseurs runtime missing")
        return
    ...
    if str(runtime.get("building_id")) != BUILDING_ID:
        _fail("runtime building identity drifted")
        return
    if str(runtime.get("source_wall_id")) != WALL_ID:
        _fail("runtime must remain anchored to official wall 10945501")
        return
    if abs(float(runtime.get("source_facade_span_m")) - EXPECTED_SPAN_M) > 0.002:
        _fail("runtime official facade span drifted")
        return
    if int(runtime.get("bay_count")) != 3:
        _fail("runtime must preserve three-bay photo plan")
        return
    var runtime_offsets = runtime.get("column_offsets_m")
    if not (runtime_offsets is Array) or runtime_offsets.size() != EXPECTED_OFFSETS.size():
        _fail("runtime must expose four photo-constrained column offsets")
        return
    for i: int in range(EXPECTED_OFFSETS.size()):
        if abs(float(runtime_offsets[i]) - float(EXPECTED_OFFSETS[i])) > 0.001:
            _fail("runtime column offset %d diverges from shipped plan" % i)
            return
    if str(runtime.get("photo_source_sha256")) != EXPECTED_SOURCE_SHA256:
        _fail("runtime provenance must point to shipped photo source hash")
        return
    if bool(runtime.get("raw_photo_pixels_shipped")):
        _fail("raw Commons photo quad/pixels are forbidden in this runtime")
        return
    if bool(runtime.get("geometry_claimed_surveyed")):
        _fail("photo-constrained geometry must not claim survey accuracy")
        return
    if str(runtime.get("vertical_world_scale_source")) == "photo":
        _fail("vertical world scale may never be sourced from the photo")
        return
    if str(runtime.get("vertical_world_scale_source")) != "official_lod2_wall":
        _fail("runtime requires independent official LoD2 wall vertical source before implementation")
        return
