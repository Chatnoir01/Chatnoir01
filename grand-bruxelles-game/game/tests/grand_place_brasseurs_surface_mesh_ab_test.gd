...
const RUNTIME_NAME := "GrandPlaceBrasseursSurfaceMeshRuntime"
const WIDTH := 1280
const HEIGHT := 720
const CAMERA_POSITION := Vector3(319.01, 1.72, -535.20)
const CAMERA_TARGET := Vector3(321.9103, 11.8, -485.6594)
const CAMERA_FOV := 62.0
const MIN_CHANGED3_PERCENT := 1.20
const MIN_CHANGED8_PERCENT := 0.70

func _initialize() -> void:
    call_deferred("_run")

func _fail(message: String) -> void:
    push_error("GRAND_PLACE_BRASSEURS_SURFACE_MESH_AB_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if not bool(runtime.get("facade_ready")):
        _fail("facade did not become ready")
        return
    ...
    camera.name = "BrasseursSurfaceMeshEvidenceCamera"
    ...
    for _frame: int in range(12):
        ...
        await process_frame
    if not _ui_is_hidden():
        _fail("recursive UI mask invariant failed")
        return
    runtime.call("set_facade_visible", false)
    for _frame: int in range(8):
        await process_frame
    var before := await _capture("/tmp/grand-place-brasseurs-surface-before.png")
    runtime.call("set_facade_visible", true)
    for _frame: int in range(8):
        await process_frame
    var after := await _capture("/tmp/grand-place-brasseurs-surface-after.png")
    ...
    if p3 < MIN_CHANGED3_PERCENT or p8 < MIN_CHANGED8_PERCENT:
        _fail("anti-micro failed changed3=%.4f%% changed8=%.4f%% fixed_thresholds=1.20/0.70" % [p3, p8])
        return
    if max_x < min_x or max_y < min_y:
        _fail("no changed-pixel bounding box")
        return
    print("GRAND_PLACE_BRASSEURS_SURFACE_MESH_AB_OK: changed3=%.4f%% changed8=%.4f%% bbox=(%d,%d)-(%d,%d) player_eye=true ui_masked=true dynamics_masked=true thresholds=1.20/0.70" % [p3, p8, min_x, min_y, max_x, max_y])
    quit(0)
