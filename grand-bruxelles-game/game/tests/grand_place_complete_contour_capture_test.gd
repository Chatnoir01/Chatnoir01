...
func _run() -> void:
    ...
    if contour != null:
        if not contour.has_method("ready_complete") or not bool(contour.call("ready_complete")):
            _fail("head contour runtime present but not complete")
            return
        if contour.has_method("failed") and bool(contour.call("failed")):
            _fail("head contour runtime failed")
            return
    ...
    if contour != null:
        contour_state = "owners=%d" % int(contour.call("owner_count"))
    print("GRAND_PLACE_COMPLETE_CONTOUR_CAPTURE_OK: %s camera=%s target=%s fov=%.1f contour=%s" % [absolute, str(witness_camera.global_position), str(camera_target), witness_camera.fov, contour_state])
    viewport.queue_free()
    quit(0)
