...
func _run() -> void:
    ...
    if contract.is_empty() or str(contract.get("schema", "")) != "grand-bruxelles-grand-place-complete-contour-witness-v1":
        ...
        return
    var base_main := str(contract.get("base_main", ""))
    if base_main.length() != 40:
        _fail("witness base main must be an exact 40-char commit SHA")
        return
    ...
    print("GRAND_PLACE_COMPLETE_CONTOUR_OK: owners=23 source_faces=%d source_triangles=%d render_triangles=%d collisions=%d masked_osm=%d base=%s" % [source_faces, source_triangles, rendered_triangles_expected, owners_with_walls, int(runtime.get("masked_osm_count")), base_main])
    quit(0)
