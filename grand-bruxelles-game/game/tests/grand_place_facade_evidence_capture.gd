...
func _run() -> void:
    ...
    camera.global_position = Vector3(float(camera_position[0]), float(camera_position[1]), float(camera_position[2]))
    ...
    if not _write_json("%s/manifest.json" % OUTPUT_DIR, manifest):
        ...
        return
    print("GRAND_PLACE_FACADE_CAPTURE_OK views=4 resolution=1280x720 human_review=pending")
    camera.queue_free()
    scene.queue_free()
    await process_frame
    quit(0)
