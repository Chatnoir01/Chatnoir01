...
func _capture(path: String, main: Node) -> bool:
...
func _run() -> void:
    ...
    for _frame: int in range(900):
        if contour != null and bool(contour.get("geometry_loaded")):
            if state == "before" or (facade != null and bool(facade.get("built"))):
                break
        await process_frame
    ...
    for raw_view: Variant in views:
        ...
        if not await _capture(path,main):
            _fail("capture failed: %s" % path)
            return
        print("GRAND_PLACE_FACADE_CAPTURE_VIEW: id=%s state=%s target=[%.3f,%.3f,%.3f]" % [str(view.get("id","")),state,target.x,target.y,target.z])
    print("GRAND_PLACE_FACADE_CAPTURE_OK: state=%s views=%d camera_fixed=true fov_fixed=true" % [state,views.size()])
    quit(0)
