...
func _owner_center(contour: Node) -> Variant:
    ...
    var min_v := Vector3(
        minf(wall_bounds.position.x, roof_bounds.position.x),
        minf(wall_bounds.position.y, roof_bounds.position.y),
        minf(wall_bounds.position.z, roof_bounds.position.z)
    )
    var max_v := Vector3(
        maxf(wall_bounds.end.x, roof_bounds.end.x),
        maxf(wall_bounds.end.y, roof_bounds.end.y),
        maxf(wall_bounds.end.z, roof_bounds.end.z)
    )
...
func _pixel_delta(before: Image, after: Image) -> Dictionary:
    ...
    return {
        "changed_pixels_gt_0_001": changed,
        "changed_fraction_gt_0_001": float(changed) / float(total),
        "mean_rgb_abs_delta": sum_abs / float(total),
        "max_rgb_abs_delta": max_abs,
    }
