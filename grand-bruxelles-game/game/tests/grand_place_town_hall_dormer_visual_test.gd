...
func _run() -> void:
    ...
    if gate.get("resolution", []) != [WIDTH, HEIGHT]:
    ...
    if official == null or not bool(official.get("geometry_loaded")):
        ...
        return
    if dormers == null or not bool(dormers.call("ready_complete")):
        _fail("dormer runtime did not load")
        return
    if int(dormers.call("dormer_count")) != 16:
        _fail("runtime dormer count drift")
        return
