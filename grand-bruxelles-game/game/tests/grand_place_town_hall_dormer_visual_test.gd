...
func _gate(metrics: Dictionary, gate: Dictionary, label: String, require_margin: bool) -> bool:
    if int(metrics.get("changed_gt8_pixels", 0)) < int(gate.get("minimum_changed_pixels", 0)):
        _fail(label + " changed-pixel floor missed")
        return false
    if int(metrics.get("bbox_width", 0)) < int(gate.get("minimum_bbox_width_px", 0)) or int(metrics.get("bbox_height", 0)) < int(gate.get("minimum_bbox_height_px", 0)):
        _fail(label + " bbox floor missed")
        return false
    if require_margin:
        var bbox: Array = metrics.get("bbox", [])
        var margin := int(gate.get("minimum_frame_margin_px", 0))
        if bbox.size() != 4 or int(bbox[0]) < margin or int(bbox[1]) < margin or int(bbox[2]) > WIDTH - 1 - margin or int(bbox[3]) > HEIGHT - 1 - margin:
            _fail(label + " detail is clipped against frame edge")
            return false
    return true
...
func _run() -> void:
    ...
    _set_camera(camera, canonical_position, canonical_target, canonical_fov)
    ...
    var canonical_metrics := await _capture_pair(
        dormers,
        OUT_DIR + "/grand_place_town_hall_dormer_before.png",
        OUT_DIR + "/grand_place_town_hall_dormer_after.png",
        float(canonical_gate.get("rgb_delta_threshold", 8))
    )
    if canonical_metrics.is_empty() or not _gate(canonical_metrics, canonical_gate, "canonical", false):
        return

    var detail_gate: Dictionary = gates.get("player_look_up", {})
    var detail_target := _v3(detail_gate.get("target", []))
    if detail_target == Vector3.INF:
        _fail("player-look-up target missing")
        return
    _set_camera(camera, canonical_position, detail_target, float(detail_gate.get("fov_deg", 45.0)))
    await process_frame
    var detail_metrics := await _capture_pair(
        dormers,
        OUT_DIR + "/grand_place_town_hall_dormer_lookup_before.png",
        OUT_DIR + "/grand_place_town_hall_dormer_lookup_after.png",
        float(detail_gate.get("rgb_delta_threshold", 8))
    )
    if detail_metrics.is_empty() or not _gate(detail_metrics, detail_gate, "player-look-up", true):
        return

    var output_data := {
        "schema": "grand-bruxelles-town-hall-dormer-visual-evidence-v2",
        ...
        "player_look_up": {
            "position": [canonical_position.x, canonical_position.y, canonical_position.z],
            "target": [detail_target.x, detail_target.y, detail_target.z],
            "fov_deg": float(detail_gate.get("fov_deg", 45.0)),
    ...
    print("GRAND_PLACE_TOWN_HALL_DORMER_VISUAL_OK canonical_pixels=%d canonical_bbox=%dx%d lookup_pixels=%d lookup_bbox=%dx%d dormers=16" % [
        int(canonical_metrics.get("changed_gt8_pixels", 0)), int(canonical_metrics.get("bbox_width", 0)), int(canonical_metrics.get("bbox_height", 0)),
        int(detail_metrics.get("changed_gt8_pixels", 0)), int(detail_metrics.get("bbox_width", 0)), int(detail_metrics.get("bbox_height", 0))
    ])
    quit(0)
