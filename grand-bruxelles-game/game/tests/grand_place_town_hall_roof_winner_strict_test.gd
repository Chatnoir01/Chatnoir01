...
const PRODUCTION_BASE := "39f6471a8872024f909728c9187e678b83e2e115"
const WIDTH := 1280
const HEIGHT := 720
...
func _capture(path: String) -> Image:
    ...
    if texture == null:
        _fail("viewport texture unavailable; strict witness requires a real GL renderer")
        return Image.new()
    var image := texture.get_image()
    if image == null or image.is_empty():
        _fail("viewport image unavailable; strict witness requires a real GL renderer")
        return Image.new()
...
func _diff(a: Image, b: Image) -> Dictionary:
    if a == null or b == null or a.is_empty() or b.is_empty():
        return {"changed_gt8_pixels": 0, "changed_gt8_percent": 0.0, "bbox": null, "bbox_width": 0, "bbox_height": 0}
    var pixels := 0
    var x0 := WIDTH
    var y0 := HEIGHT
    var x1 := -1
