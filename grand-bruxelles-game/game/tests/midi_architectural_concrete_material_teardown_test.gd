...
func _bind_fixture(runtime: Node, targets: Array[MeshInstance3D], identity: Dictionary) -> void:
...
func _scenario_restore_legacy(identity: Dictionary) -> void:
    ...
    _bind_fixture(runtime, targets, identity)
...
func _run() -> void:
    ...
    print("MIDI_ARCHITECTURAL_CONCRETE_MATERIAL_TEARDOWN_OK: surfaces=74 owner_aware_restore=true later_owner_preserved=true geometry_changed=false")
    quit(0)
