...
func _run() -> void:
    ...
    if not await _wait_for_runtime_bind(runtime, replacement_scene):
        _fail("replacement scene did not receive architectural-glazing material from surviving autoload")
        return
    ...
    if _count_owned_material(replacement_midi, replacement_material) != EXPECTED_SURFACES:
        _fail("replacement scene glazing ownership coverage drifted")
        return

    print("MIDI_ARCHITECTURAL_GLAZING_SCENE_REPLACEMENT_OK: surfaces=%d autoload_rebind=true geometry_changed=false" % EXPECTED_SURFACES)
    quit(0)
