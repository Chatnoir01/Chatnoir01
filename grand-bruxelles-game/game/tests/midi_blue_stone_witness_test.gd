...
func _run() -> void:
    ...
    if ratio8 < MIN_CHANGED_OVER_8:
        _fail("normal-distance >8 RGB area below 1.0% recognition gate")
        return
    print("MIDI_BLUE_STONE_WITNESS_OK")
    quit(0)

func _capture(use_shipped_blue_stone: bool, path: String) -> Image:
    ...
    await process_frame
    if use_shipped_blue_stone:
        _restore_shipped_blue_stone(world)
        await process_frame
...
func _restore_shipped_blue_stone(root: Node) -> void:
    var shipped := StandardMaterial3D.new()
    shipped.albedo_color = Color(0.235, 0.255, 0.27, 1.0)
    shipped.roughness = 0.86
    shipped.metallic = 0.0
    _replace_blue_stone_materials(root, shipped)
