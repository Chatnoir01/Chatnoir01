...
const AMBIENT_UPPER_ARM_DROP_DEG := 58.0
const MIN_AMBIENT_UPPER_ARM_DROP_DEG := 50.0
...
func _run() -> void:
    ...
    if not bool(pose_audit.get("applied", false)):
        _fail("ambient pose gate requires a rigged body: %s" % JSON.stringify(pose_audit))
        return
    if float(pose_audit.get("extra_upper_arm_degrees", 0.0)) < MIN_AMBIENT_UPPER_ARM_DROP_DEG:
        _fail("ambient pose regression: upper-arm drop %.1f is below %.1f degrees" % [float(pose_audit.get("extra_upper_arm_degrees", 0.0)), MIN_AMBIENT_UPPER_ARM_DROP_DEG])
        return
    ...
    var metrics := {
        "schema": "grand-bruxelles-civ1-vitruvian-witness-v5",
        "production_authorized": false,
        "actual_scene": MAIN_SCENE,
        "base_main_sha": "292ce27314dae6362a0b3845b4adde4dac0ff468",
        ...
        "ambient_pose_min_upper_arm_drop_deg": MIN_AMBIENT_UPPER_ARM_DROP_DEG,
    ...
    print("GB_CIV1_VITRUVIAN_WITNESS_OK height=%.3f source_height=%.3f cluster=%d triangles_each=%d animations=%d footwear=cc0 pose_drop=%.1f production_authorized=false" % [PRIMARY_HEIGHT_M, source_height, cluster.size(), triangles, animation_clips, float(pose_audit.get("extra_upper_arm_degrees", 0.0))])
    quit(0)
...
func _relax_pose_if_rigged(root: Node) -> Dictionary:
    ...
    # 24 degrees remained visibly near-horizontal in the exact full-frame artifact;
    # 58 degrees is the next bounded review pose, still animation-free and review-only.
    skeleton.set_bone_pose_rotation(left, Quaternion(Vector3(0.0, 0.0, 1.0), deg_to_rad(AMBIENT_UPPER_ARM_DROP_DEG)))
    skeleton.set_bone_pose_rotation(right, Quaternion(Vector3(0.0, 0.0, 1.0), deg_to_rad(-AMBIENT_UPPER_ARM_DROP_DEG)))
    var applied := {"applied": true, "left_bone": skeleton.get_bone_name(left), "right_bone": skeleton.get_bone_name(right), "extra_upper_arm_degrees": AMBIENT_UPPER_ARM_DROP_DEG, "minimum_gate_degrees": MIN_AMBIENT_UPPER_ARM_DROP_DEG, "direction_verified_from_red_run": 12, "previous_rejected_degrees": 24.0}
