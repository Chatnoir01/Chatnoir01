...
const BEFORE_PATH := OUTPUT_DIR + "/midi_fonsny_porch_before.png"
const AFTER_PATH := OUTPUT_DIR + "/midi_fonsny_porch_after.png"
const METRICS_PATH := OUTPUT_DIR + "/midi_fonsny_porch_metrics.json"
const WIDTH := 1280
const HEIGHT := 720
...
func _run() -> void:
    ...
    if mount == null:
        ...
        return
    var runtime: Node = mount.fonsny_porch_runtime()
    if runtime == null:
        _fail("Fonsny porch runtime missing")
        return
    for _i in range(30):
    ...
    if runtime.build_failure() or not runtime.built():
        _fail("Fonsny porch failed to build")
        return
    var porch := runtime.porch_node() as Node3D
    if porch == null:
        _fail("Fonsny porch node missing")
        return
    _mask_dynamic_world(world, porch)
    var camera := Camera3D.new()
    camera.name = "MidiFonsnyPorchWitnessCamera"
    ...
    var metrics := {"width": WIDTH, "height": HEIGHT, "camera": [CAMERA_POSITION.x, CAMERA_POSITION.y, CAMERA_POSITION.z], "fov": 69.0, "over3_pixels": over3, "over3_ratio": ratio3, "over8_pixels": over8, "over8_ratio": ratio8, "bbox_width": bbox_w, "bbox_height": bbox_h, "min_over3_ratio": MIN_CHANGED_OVER_3, "min_over8_ratio": MIN_CHANGED_OVER_8, "min_bbox_width": MIN_BBOX_WIDTH, "min_bbox_height": MIN_BBOX_HEIGHT}
    var f := FileAccess.open(METRICS_PATH, FileAccess.WRITE)
    if f != null:
        f.store_string(JSON.stringify(metrics, "  "))
    print("MIDI_FONSNY_PORCH_DELTA ratio3=%.6f ratio8=%.6f bbox=%dx%d" % [ratio3, ratio8, bbox_w, bbox_h])
    if ratio3 < MIN_CHANGED_OVER_3 or ratio8 < MIN_CHANGED_OVER_8 or bbox_w < MIN_BBOX_WIDTH or bbox_h < MIN_BBOX_HEIGHT:
        ...
        return
    print("MIDI_FONSNY_PORCH_WITNESS_OK")
    quit(0)
...
func _fail(message: String) -> void:
    push_error("MIDI_FONSNY_PORCH_WITNESS_FAIL: " + message)
    quit(1)
