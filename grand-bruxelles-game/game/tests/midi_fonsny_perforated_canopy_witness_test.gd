...
const BEFORE_PATH := OUTPUT_DIR + "/midi_fonsny_perforated_canopy_before.png"
const AFTER_PATH := OUTPUT_DIR + "/midi_fonsny_perforated_canopy_after.png"
const METRICS_PATH := OUTPUT_DIR + "/midi_fonsny_perforated_canopy_metrics.json"
const WIDTH := 1280
const HEIGHT := 720
...
func _run() -> void:
    ...
    if mount == null:
        ...
        return
    var runtime: Node = mount.fonsny_canopy_runtime()
    if runtime == null:
        _fail("Fonsny perforated-canopy runtime missing")
        return
    for _i in range(120):
        if runtime.built() or runtime.build_failure():
            break
        await process_frame
    if runtime.build_failure() or not runtime.built():
        _fail("Fonsny perforated canopy failed to build")
        return

    var entrance := get_root().find_child("MidiMainEntranceFonsny", true, false) as Node3D
    if entrance == null:
        _fail("Fonsny entrance missing")
        return
    var replacement := runtime.replacement_root() as Node3D
    var baseline := runtime.baseline_surface() as MeshInstance3D
    if replacement == null or baseline == null:
        _fail("Fonsny canopy baseline/replacement missing")
        return

    _mask_dynamic_world(world, entrance)
    var camera := Camera3D.new()
    camera.name = "MidiFonsnyPerforatedCanopyWitnessCamera"
    ...
    runtime.set_source_backed_enabled(false)
    if not baseline.visible or replacement.visible:
        _fail("baseline toggle invariant failed")
        return
    var before := await _capture(BEFORE_PATH)

    runtime.set_source_backed_enabled(true)
    if baseline.visible or not replacement.visible:
        _fail("replacement toggle invariant failed")
        return
    ...
    var metrics := {
        ...
        "source_urban_id": 9423,
        "baseline_surface": "EntranceConcreteCanopy",
        "replacement_surface": "EntranceSourceBackedPerforatedCanopy"
    }
    var f := FileAccess.open(METRICS_PATH, FileAccess.WRITE)
    if f != null:
        f.store_string(JSON.stringify(metrics, "  "))

    print("MIDI_FONSNY_PERFORATED_CANOPY_DELTA ratio3=%.6f ratio8=%.6f bbox=%dx%d" % [ratio3, ratio8, bbox_w, bbox_h])
    if ratio3 < MIN_CHANGED_OVER_3 or ratio8 < MIN_CHANGED_OVER_8 or bbox_w < MIN_BBOX_WIDTH or bbox_h < MIN_BBOX_HEIGHT:
        ...
        return
    print("MIDI_FONSNY_PERFORATED_CANOPY_WITNESS_OK")
    quit(0)
...
func _fail(message: String) -> void:
    push_error("MIDI_FONSNY_PERFORATED_CANOPY_WITNESS_FAIL: " + message)
    quit(1)
