...
func _capture(viewport: SubViewport, path: String) -> Image:
    ...
    if image == null or image.is_empty() or image.get_size() != Vector2i(WIDTH, HEIGHT): return null
...
func _stats(before: Image, after: Image, threshold: int) -> Dictionary:
    var changed := 0; var min_x := WIDTH; var min_y := HEIGHT; var max_x := -1; var max_y := -1
...
func _run() -> void:
    ...
    if packed == null: _fail("production main missing"); return
    var viewport := SubViewport.new(); viewport.size = Vector2i(WIDTH, HEIGHT); viewport.own_world_3d = true
    viewport.render_target_clear_mode = SubViewport.CLEAR_MODE_ALWAYS; viewport.render_target_update_mode = SubViewport.UPDATE_ALWAYS; root.add_child(viewport)
    var scene := packed.instantiate() as Node3D; viewport.add_child(scene)
    ...
    if hero == null or urbis == null: _fail("Midi production owners missing"); return
    if hero.get_node_or_null("BruxellesMidiStation") != null: _fail("production AFTER still builds hand-authored station complex"); return
    for required in ["MidiMainEntranceFonsny", "FonsnyForecourt"]:
        if hero.get_node_or_null(required) == null: _fail("required street-level detail missing: %s" % required); return
    var camera := Camera3D.new(); camera.fov = 69.0; scene.add_child(camera)
    camera.look_at_from_position(ENTRANCE + ROAD_SIDE * 28.0 + Vector3(0.0, 1.65, 0.0), ENTRANCE + Vector3(0.0, 3.2, 0.0), Vector3.UP); camera.current = true
    ...
    if legacy == null: _fail("could not reconstruct legacy BEFORE mass"); return
    var before := await _capture(viewport, BEFORE_PATH)
    if before == null: _fail("BEFORE capture missing"); return
    legacy.queue_free()
    for _i in range(4): await process_frame
    if hero.get_node_or_null("BruxellesMidiStation") != null: _fail("legacy station mass survived AFTER"); return
    var after := await _capture(viewport, AFTER_PATH)
    if after == null: _fail("AFTER capture missing"); return
    var s3 := _stats(before, after, 3); var s8 := _stats(before, after, 8)
    var f3 := float(s3["fraction"]); var f8 := float(s8["fraction"]); var bbox := s3["bbox"] as Rect2i
    print("MIDI_M3_URBIS_OWNER_METRICS: gt3=%.4f%% gt8=%.4f%% bbox=%dx%d" % [f3 * 100.0, f8 * 100.0, bbox.size.x, bbox.size.y])
    if f3 < MIN_CHANGED_3 or f8 < MIN_CHANGED_8: _fail("station-mass correction below predeclared full-frame impact gate"); return
    if bbox.size.x < MIN_BBOX_WIDTH or bbox.size.y < MIN_BBOX_HEIGHT: _fail("station-mass correction bbox too small"); return
    if hero.get_node_or_null("MidiMainEntranceFonsny") == null or scene.get_node_or_null("UrbISMidiExact") == null: _fail("source-backed/street-level owner lost after A/B"); return
