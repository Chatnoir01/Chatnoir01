...
# The capture harness freezes the camera/world but not authored player animation.
# Exclude only the central actor ROI proven by the rejected A/B run; world,
# road, facade and vehicle pixels remain part of the stability contract.
...
func _run() -> void:
    ...
    print(
        "MIDI_SURFACE_STABILITY_AB_METRICS: world_changed_gt4=%.5f%% world_mean_delta=%.7f checked=%d actor_roi=%s" %
        [fraction * 100.0, mean_delta, checked_pixels, DYNAMIC_ACTOR_ROI]
    )
