...
const OUTPUT_EXTERIOR := "res://artifacts/visual/mmc_police_v5_silhouette_exterior.png"
const OUTPUT_SIDE := "res://artifacts/visual/mmc_police_v5_silhouette_side.png"

func _initialize() -> void:
    call_deferred("_run")

func _fail(message: String) -> void:
    push_error("MMC_POLICE_V5_VISUAL_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if not bool(v4.call("tune_holder", sedan_holder, "brussels_capitale_sedan")):
        _fail("sedan V4 presentation setup failed")
        return
    if not bool(v4.call("tune_holder", coupe_holder, "brussels_rapid_response_coupe")):
        _fail("coupe V4 presentation setup failed")
        return
    var v5 := V5_SCRIPT.new()
    if not bool(v5.call("install_silhouette", sedan_holder, "brussels_capitale_sedan")):
        _fail("sedan V5 silhouette failed")
        return
    if not bool(v5.call("install_silhouette", coupe_holder, "brussels_rapid_response_coupe")):
        _fail("coupe V5 silhouette failed")
        return
    if not bool(sedan_holder.get_meta("v5_smooth_lower_hull", false)) or not bool(coupe_holder.get_meta("v5_curved_roof_skin", false)):
        _fail("V5 witness metadata missing")
        return
    ...
    print("MMC_POLICE_V5_VISUAL_OK: captures=2 resolution=1280x720 profiles=2 smooth_hulls=2 curved_roofs=2")
    quit(0)
