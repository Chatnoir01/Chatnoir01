...
func _init() -> void:
	...
	var recheck_a: float = agent.pedestrian_context.curb_recheck_interval_seconds(0)
	var recheck_a_repeat: float = agent.pedestrian_context.curb_recheck_interval_seconds(0)
	var recheck_b: float = second_agent.pedestrian_context.curb_recheck_interval_seconds(0)
	_assert(recheck_a >= 0.55 and recheck_a <= 1.65, "curb recheck cadence stays physically plausible")
	_assert(is_equal_approx(recheck_a, recheck_a_repeat), "curb recheck cadence is deterministic for a stable civilian")
