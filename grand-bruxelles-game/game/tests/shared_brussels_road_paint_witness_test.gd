...
func _freeze(scene: Node3D) -> void:
    ...
    if traffic != null:
        traffic.set_process(false); traffic.set_physics_process(false)
...
func _geometry_snapshot(runtime: Node) -> Dictionary:
    ...
    for target: Variant in targets:
        if target is CSGBox3D:
            ...
            snapshot[dash.get_instance_id()] = {"transform": dash.global_transform, "size": dash.size}
    return snapshot

func _assert_geometry_unchanged(runtime: Node, before: Dictionary) -> bool:
    var targets: Array = runtime.get("_targets")
    if targets.size() != before.size(): _fail("target count changed during A/B"); return false
    for target: Variant in targets:
        if not target is CSGBox3D: _fail("non-CSG target entered road-paint set"); return false
        var dash := target as CSGBox3D
        var id := dash.get_instance_id()
        if not before.has(id): _fail("road-paint target identity changed"); return false
        var expected: Dictionary = before[id]
        if dash.global_transform != expected["transform"] or dash.size != expected["size"]:
            _fail("road-paint target geometry changed"); return false
        if bool(dash.get_meta("geometry_changed_by_road_paint_runtime", true)):
            _fail("target metadata reports geometry mutation"); return false
    return true
...
func _run() -> void:
    ...
    var scene := packed.instantiate() as Node3D; root.add_child(scene)
    var runtime := root.get_node_or_null("BrusselsSharedRoadPaintRuntime")
    if runtime == null: _fail("shared road-paint autoload missing"); return
    ...
    for _i: int in range(4): await process_frame
    if not _assert_geometry_unchanged(runtime, geometry): return
    ...
    if f3 < MIN_CHANGED_3 or f8 < MIN_CHANGED_8: _fail("material change is below predeclared normal-player pixel-impact gate"); return
    if bbox.size.x < MIN_BBOX_WIDTH or bbox.size.y < MIN_BBOX_HEIGHT: _fail("material change does not span enough of the normal-player frame"); return
    print("SHARED_BRUSSELS_ROAD_PAINT_VISUAL_OK"); quit(0)
