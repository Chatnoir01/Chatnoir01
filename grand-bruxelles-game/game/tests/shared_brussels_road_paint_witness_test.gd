...
func _assert_geometry_unchanged(runtime: Node, before: Dictionary) -> bool:
    ...
    for target: Variant in targets:
        ...
        if dash.global_transform != expected["transform"] or dash.size != expected["size"]:
            _fail("road-paint target geometry changed")
            return false
...
func _run() -> void:
    ...
    if runtime == null:
        _fail("shared road-paint autoload missing"); return
    ...
    if count < MIN_MARKINGS:
        _fail("too few generic lane dashes for reusable visible lot: %d" % count); return
    _freeze(scene)
    var geometry := _geometry_snapshot(runtime)
    if geometry.size() != count:
        _fail("geometry snapshot does not cover all targets"); return
    ...
    if f3 < MIN_CHANGED_3 or f8 < MIN_CHANGED_8:
        _fail("material change is below predeclared normal-player pixel-impact gate"); return
    if bbox.size.x < MIN_BBOX_WIDTH or bbox.size.y < MIN_BBOX_HEIGHT:
        _fail("material change does not span enough of the normal-player frame"); return
