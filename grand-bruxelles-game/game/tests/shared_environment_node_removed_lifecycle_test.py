...
def main() -> None:
    ...
    for entry in runtimes:
        ...
        match = NODE_REMOVED_CONNECT_RE.search(source)
        if match:
            detected[rel_path] = match.group(1)
            if entry.get("node_removed_cleanup_required") is not True:
                fail(
                    f"runtime connects node_removed without lifecycle declaration: "
                    f"{rel_path} handler={match.group(1)}"
                )
            if entry.get("node_removed_cleanup_handler") != match.group(1):
    ...
    print(
        ...
        f"helper_watchers={len(HELPER_WATCHERS)} street_furniture_rebind=locked discovery=fail_closed"
    )
