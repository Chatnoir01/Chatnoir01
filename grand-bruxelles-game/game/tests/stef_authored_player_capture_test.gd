...
func _fail(message: String) -> void:
    push_error("STEF_AUTHORED_PLAYER_CAPTURE_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if not ResourceLoader.exists(STEF_PATH):
        _fail("Stef GLB is not imported")
        return
    ...
    # Configure the already-authoritative VisualUpgrade before it enters the tree.
    # No production renderer file is modified by this witness branch.
    var visual := scene.get_node_or_null("Player/VisualUpgrade") as Node3D
    if visual == null:
        _fail("production Player/VisualUpgrade path missing")
        return
    visual.set("authored_scene_path", STEF_PATH)
    visual.set("allow_authored_fallback_paths", false)
    ...
    visual = scene.get_node_or_null("Player/VisualUpgrade") as Node3D
    if player == null or visual == null:
        _fail("production player visual path missing after warmup")
        return
    if not bool(visual.call("is_using_authored_character")):
        _fail("production player did not select Stef authored asset")
        return
    if String(visual.call("resolved_authored_scene_path")) != STEF_PATH:
        _fail("production player resolved unexpected authored path")
        return
    ...
    if image.save_png(absolute) != OK:
        ...
        return
    print("STEF_AUTHORED_PLAYER_CAPTURE_OK: %s player=%s camera=%s path=%s" % [absolute, str(player.global_position), str(camera.global_position), STEF_PATH])
    viewport.queue_free()
    quit(0)
