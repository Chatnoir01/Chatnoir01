...
func _fail(message: String) -> void:
    push_error("STEF_DEFAULT_PLAYER_CAPTURE_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if not ResourceLoader.exists(STEF_PATH):
        _fail("Stef production GLB is not imported")
        return
    ...
    if not bool(visual.call("is_using_authored_character")):
        _fail("production player did not select an authored asset")
        return
    if String(visual.call("resolved_authored_scene_path")) != STEF_PATH:
        _fail("production player resolved unexpected path: %s" % String(visual.call("resolved_authored_scene_path")))
        return
    ...
    if image.save_png(absolute) != OK:
        ...
        return
    print("STEF_DEFAULT_PLAYER_CAPTURE_OK: %s player=%s camera=%s path=%s" % [absolute, str(player.global_position), str(camera.global_position), STEF_PATH])
    viewport.queue_free()
    quit(0)
