...
func _run() -> void:
    ...
    if player == null:
        ...
        return
    # Keep the shipped camera rig/FOV and use a closer straight inspection pose,
    # while hiding only the player's body from the evidence frame.
    ...
    player.global_position = Vector3(-650.0, 1.05, 629.0)
    player.look_at(Vector3(-650.0, 1.10, 621.0), Vector3.UP)
