#!/usr/bin/env python3
"""Fail closed if shared OSM environment batch ownership can retain dead MultiMesh refs."""
...
assert "_prune_invalid_owned_batches()" in set_visible, "visibility pass must prune dead owned batches"

batch = source[source.index("func _batch("):source.index("func _ensure_tree_presentation_meshes")]
assert "_prune_invalid_owned_batches()" in batch, "reuse lookup must prune dead owned batches before matching by name"
