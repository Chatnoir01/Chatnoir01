...
func _fail(message: String) -> void:
    push_error("TRAFFIC_CANONICAL_NPC_CROSSING_FAIL: %s" % message)
    quit(1)
...
func _run() -> void:
    ...
    if crossing_script == null or bridge_script == null or npc_script == null:
        _fail("canonical crossing helper, NPC bridge or fixture did not load")
        return
    ...
    if int((system.call("get_crossing_state", 100) as Dictionary).get("crossing", 0)) != 0:
        _fail("NPC started crossing before minimum wait")
        return
    ...
    if int(active_state.get("waiting", 0)) != 0 or int(active_state.get("crossing", 0)) != 1:
        _fail("NPC did not transition from curb wait to crossing")
        return
    ...
    print("TRAFFIC_CANONICAL_NPC_CROSSING_OK: same-side ignored, curb wait, safe crossing and destination restoration passed")
    quit(0)
