...
## Stable IDs are the binding truth; display/node names are debug-only.
## Official sources resolve lower-sphere window presence and 18 m sphere diameter,
## but not exact contemporary bay count, mullions or dimensions.
...
const EXPECTED_REJECTED_TICKET_NODE_ID := 13156161818
...
var rejected_ticket_shop_node_id := -1
...
func build_on_sphere(sphere_id: int, sphere_center: Vector3, sphere_diameter_m: float) -> bool:
    ...
    rejected_ticket_shop_node_id = int(ids.get("rejected_osm_ticket_shop_node_id", -1))
    heritage_record_id = int(ids.get("heritage_record_id", -1))
    var evidence_sphere_id := int(ids.get("target_sphere_id", -1))
    if monument_relation_id != EXPECTED_MONUMENT_RELATION_ID:
        push_error("AtomiumBaseSphereWindowBand: monument relation id drifted")
        return false
    if rejected_ticket_shop_node_id != EXPECTED_REJECTED_TICKET_NODE_ID:
        push_error("AtomiumBaseSphereWindowBand: rejected ticket node id drifted")
        return false
    ...
    if int(binding.get("target_sphere_id", -1)) != sphere_id:
        push_error("AtomiumBaseSphereWindowBand: runtime binding sphere id drifted")
        return false
    var expected_center_raw: Variant = binding.get("target_sphere_local_center_m", [])
    if not expected_center_raw is Array or expected_center_raw.size() != 3:
        push_error("AtomiumBaseSphereWindowBand: topology centre missing")
        return false
    var expected_center := Vector3(float(expected_center_raw[0]), float(expected_center_raw[1]), float(expected_center_raw[2]))
    if sphere_center.distance_to(expected_center) > 0.001:
        push_error("AtomiumBaseSphereWindowBand: topology centre drifted")
        return false
    ...
    if band_built:
        ...
        set_meta("rejected_osm_ticket_shop_node_id", rejected_ticket_shop_node_id)
        ...
        print("ATOMIUM_BASE_SPHERE_WINDOW_BAND_READY: relation_id=%d heritage_id=%d sphere_id=%d rejected_ticket_node_id=%d exact_layout=false authored_not_survey=true geometry_moved=false collision_changed=false" % [monument_relation_id, heritage_record_id, bound_sphere_id, rejected_ticket_shop_node_id])
    return band_built
...
func _load_evidence() -> Dictionary:
    ...
    if not bool(identity.get("ticket_shop_node_must_never_be_used_as_monument_identity", false)):
        push_error("AtomiumBaseSphereWindowBand: rejected ticket-node identity rail missing")
        return {}
    if not bool(identity.get("selection_by_display_name_forbidden", false)):
        push_error("AtomiumBaseSphereWindowBand: id-selection rail missing")
        return {}
...
"""
    ...
    material.set_meta("atomium_monument_relation_id", monument_relation_id)
...
func source_monument_relation_id() -> int:
    return monument_relation_id

func source_heritage_record_id() -> int:
    return heritage_record_id
