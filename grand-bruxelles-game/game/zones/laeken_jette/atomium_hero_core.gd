...
func build_on_terrain(terrain: Node) -> bool:
    ...
    if hero_built:
        ...
        print("ATOMIUM_HERO_CORE_READY: spheres=%d tubes=%d anchor_y=%.3f unresolved_pillars=%d sphere_skin_semantics=%s base_window_band=true exact_seams=false" % [sphere_count, tube_count, anchor_position.y, unresolved_support_pillars, str(sphere_skin_semantics_applied)])
    return hero_built

func _mount_base_sphere_window_band(centres: Array[Vector3]) -> bool:
    if centres.is_empty():
        push_error("AtomiumHeroCore: base-sphere centre unavailable")
        return false
    ...
    if not bool(base_sphere_window_band.call("build_on_sphere", centres[0], source_sphere_diameter_m)):
        base_sphere_window_band.queue_free()
        base_sphere_window_band = null
        push_error("AtomiumHeroCore: base-sphere window cue failed to build")
        return false
    return true
