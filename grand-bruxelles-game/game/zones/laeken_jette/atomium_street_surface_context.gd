...
## Source-bounded immediate ground context for the Atomium arrival.
## Geometry comes only from the committed UrbIS StreetSurfaces layer and is
## draped over the accepted Atomium DTM. Material values are authored
## presentation only: no paving recipe, colour or photometry is inferred.
...
@export var context_radius_m := 160.0
...
func build_on_terrain(terrain: Node) -> bool:
    ...
    context_built = true
    set_meta("source", "Paradigm / Brussels-Capital Region UrbIS vector WFS")
    set_meta("source_class", "urbisvector:StreetSurfaces")
    set_meta("source_crs", "EPSG:31370")
    set_meta("runtime_approved", false)
    set_meta("realism_complete", false)
    set_meta("material_photometry_resolved", false)
    set_meta("paving_material_resolved", false)
...
func _append_if_official(vertices: PackedVector3Array, normals: PackedVector3Array, ids: Array, width: int, first_e: float, first_n: float, step_e: float, step_n: float, origin_e: float, origin_n: float, heights: PackedFloat32Array) -> void:
    ...
    for index: int in range(3):
        if points[index].distance_to(_anchor_xz) > context_radius_m + 1.5:
            return
    for index: int in range(3):
...
func _load_source_contract() -> bool:
    ...
    if source_total_feature_count != 3572:
        push_error("AtomiumStreetSurfaceContext: source feature-count drifted")
        return false
