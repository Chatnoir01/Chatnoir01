...
func _build_from_urbis_envelope() -> void:
    ...
    # Dark glazed rhythm on all four sides. Counts scale from the exact UrbIS
    # envelope but are capped to stay lightweight and visually conservative.
    ...
    # Pale horizontal datum reinforces the brick/stone reading documented by
    # the registered heritage reference without replacing the station mesh.
    ...
    # One restrained platform-side canopy cue. Its proportions are deliberately
    # bounded and marked decorative; placement is relative to the source-backed
    # station envelope rather than hand-traced world coordinates.
