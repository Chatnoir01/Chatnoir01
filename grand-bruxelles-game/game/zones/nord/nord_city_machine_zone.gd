...
## Visible City Machine LABO for Gare du Nord.
## Official UrbIS vector data is the only authored city geometry in this stage.
## The small review pad is an inspection aid outside the acquired envelope; it
## is not station geometry and this scene never promotes itself to JOUABLE.

const DATA_ROOT := "res://data/urbis/nord"
# Local frame is centered at Lambert72 E=149500/N=172250. The acquired envelope
# therefore spans about X=-500..500 and Z=-250..250. Keep review spawn outside it.
const REVIEW_SPAWN := Vector3(-540.0, 1.05, 290.0)

@export var building_fallback_height_m := 10.5
@export var ground_y := 0.0

var last_stats: Dictionary = {"buildings":0,"street_surfaces":0,"street_axes":0,"tram_network":0,"train_network":0}
...
func _ready() -> void:
    ...
    print("NORD_CITY_MACHINE_LABO_READY: %s promotion=false source=urbis" % JSON.stringify(last_stats))
...
func _build_ground_reference() -> void:
    ...
    pad.name = "NordCityMachineReviewPad"
    var mesh := PlaneMesh.new()
    mesh.size = Vector2(20.0,20.0)
    mesh.material = _road_material
    pad.mesh = mesh
    pad.position = Vector3(REVIEW_SPAWN.x,ground_y-0.08,REVIEW_SPAWN.z)
    add_child(pad)
    var body := StaticBody3D.new()
    body.name = "NordCityMachineReviewPadCollision"
    var collision := CollisionShape3D.new()
    var box := BoxShape3D.new()
    box.size = Vector3(20.0,0.16,20.0)
    collision.shape = box
    body.position = Vector3(REVIEW_SPAWN.x,ground_y-0.16,REVIEW_SPAWN.z)
    body.add_child(collision)
    add_child(body)

func _load_collection(path: String) -> Dictionary:
    if not FileAccess.file_exists(path):
        push_warning("Nord City Machine layer missing: %s" % path)
        return {}
    var file := FileAccess.open(path,FileAccess.READ)
    if file == null:
        push_warning("Unable to open Nord City Machine layer: %s" % path)
        return {}
    var parsed = JSON.parse_string(file.get_as_text())
    if typeof(parsed) != TYPE_DICTIONARY:
        push_warning("Invalid Nord City Machine JSON: %s" % path)
        return {}
...
func _build_official_geometry() -> void:
    ...
    _build_polygon_layer(roads,"NordCityMachineStreetSurfaces",_road_material,ground_y+0.02,true)
    _build_buildings(buildings)
    _build_line_layer(trams,"NordCityMachineTramNetwork",_tram_material,1.0,ground_y+0.055)
    _build_line_layer(trains,"NordCityMachineTrainNetwork",_rail_material,1.45,ground_y+0.07)

func _outer_rings(geometry: Dictionary) -> Array:
    var result: Array = []
    var kind := String(geometry.get("type",""))
    var coords = geometry.get("coordinates",[])
    if kind == "Polygon" and coords is Array and not coords.is_empty(): result.append(coords[0])
    elif kind == "MultiPolygon" and coords is Array:
        for polygon in coords:
            if polygon is Array and not polygon.is_empty(): result.append(polygon[0])
    return result
...
func _line_strings(geometry: Dictionary) -> Array:
    ...
    var kind := String(geometry.get("type",""))
    var coords = geometry.get("coordinates",[])
    if kind == "LineString" and coords is Array: result.append(coords)
    elif kind == "MultiLineString" and coords is Array:
        for line in coords:
            if line is Array: result.append(line)
    return result

func _ring_points(ring: Array) -> PackedVector2Array:
    var points := PackedVector2Array()
    for raw in ring:
        if raw is Array and raw.size() >= 2: points.append(Vector2(float(raw[0]),float(raw[1])))
    if points.size() >= 2 and points[0].distance_to(points[points.size()-1]) < 0.001: points.resize(points.size()-1)
    return points
...
func _append_walls(st: SurfaceTool, points: PackedVector2Array, bottom_y: float, top_y: float) -> void:
    for i in range(points.size()):
        ...
        var edge := b-a
        if edge.length_squared() < 0.0001: continue
        var normal := Vector3(edge.y,0.0,-edge.x).normalized()
        var a0 := Vector3(a.x,bottom_y,a.y); var a1 := Vector3(a.x,top_y,a.y)
        var b0 := Vector3(b.x,bottom_y,b.y); var b1 := Vector3(b.x,top_y,b.y)
        for vertex in [a0,b0,b1,a0,b1,a1]: st.set_normal(normal); st.add_vertex(vertex)
...
func _commit_mesh(st: SurfaceTool, node_name: String, material: Material, collision_enabled: bool=false) -> void:
    ...
    var instance := MeshInstance3D.new(); instance.name=node_name; instance.mesh=mesh; add_child(instance)
    if collision_enabled: instance.create_trimesh_collision()

func _build_polygon_layer(document: Dictionary,node_name:String,material:Material,y:float,collision_enabled:bool) -> void:
    var features = document.get("features",[])
    if not (features is Array) or features.is_empty(): return
    var st := SurfaceTool.new(); st.begin(Mesh.PRIMITIVE_TRIANGLES)
    for feature in features:
        if not (feature is Dictionary): continue
        var geometry = feature.get("geometry",{})
        if not (geometry is Dictionary): continue
        for ring in _outer_rings(geometry):
            var points := _ring_points(ring)
            if points.size() >= 3: _append_flat(st,points,y)
...
func _building_height(properties: Dictionary) -> float:
    for key in ["height","Height","HEIGHT","roof_height","RoofHeight","ROOFHEIGHT"]:
        if properties.has(key):
            ...
            if value >= 2.5 and value <= 220.0: return value
    for key in ["floors","Floors","FLOORS","levels","Levels","LEVELS"]:
        if properties.has(key):
            var levels := float(properties[key])
            if levels >= 1.0 and levels <= 60.0: return levels*3.15
...
func _build_buildings(document: Dictionary) -> void:
    ...
    for feature in features:
        ...
        if not (properties is Dictionary): properties={}
    ...
    _commit_mesh(st,"NordCityMachineBuildings",_building_material,true)
...
func _build_line_layer(document: Dictionary,node_name:String,material:Material,width:float,y:float) -> void:
    ...
    for feature in features:
        ...
        var geometry = feature.get("geometry",{})
        if not (geometry is Dictionary): continue
        for line in _line_strings(geometry): _append_line_ribbon(st,line,width,y)
    _commit_mesh(st,node_name,material)

func _append_line_ribbon(st: SurfaceTool,line:Array,width:float,y:float) -> void:
    if line.size() < 2: return
    for i in range(line.size()-1):
        var raw_a=line[i]; var raw_b=line[i+1]
        if not (raw_a is Array and raw_b is Array and raw_a.size()>=2 and raw_b.size()>=2): continue
        var a:=Vector2(float(raw_a[0]),float(raw_a[1])); var b:=Vector2(float(raw_b[0]),float(raw_b[1])); var delta:=b-a
        if delta.length_squared()<0.0001: continue
        var side:=Vector2(-delta.y,delta.x).normalized()*width*0.5
        var p0:=Vector3(a.x+side.x,y,a.y+side.y); var p1:=Vector3(a.x-side.x,y,a.y-side.y)
        var p2:=Vector3(b.x-side.x,y,b.y-side.y); var p3:=Vector3(b.x+side.x,y,b.y+side.y)
        for vertex in [p0,p1,p2,p0,p2,p3]: st.set_normal(Vector3.UP); st.add_vertex(vertex)
