...
def test_runtime_index_descriptors_match_live_source_bytes_and_unique_roads() -> None:
    ...
    for descriptor in documents:
        ...
        source_roads = source_document.get("roads")
        assert isinstance(source_roads, list) and source_roads, raw_path
        source_road_ids = {
            road.get("osm_id")
            for road in source_roads
            if isinstance(road, dict) and type(road.get("osm_id")) is int and road.get("osm_id") > 0
        }
        ...
        for road_id in road_ids:
            ...
            assert road_id in source_road_ids, f"indexed OSM road id {road_id} absent from {raw_path}"
