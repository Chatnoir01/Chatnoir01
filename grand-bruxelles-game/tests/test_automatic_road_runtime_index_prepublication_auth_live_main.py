...
def test_runtime_index_authenticates_source_bytes_before_canonical_publication() -> None:
    ...
    # Freeze the fail-closed sequence, not merely the presence of each guard.
    # A future refactor must not hash a missing source, stage descriptor authority
    # before byte equality, or consult canonical maps while validation is in flight.
