...
def test_runtime_index_authenticates_source_bytes_before_any_staging_or_publication() -> None:
    ...
    # Reject a repeated canonical descriptor before touching the filesystem again.
    # This keeps duplicate-document rejection deterministic and avoids attacker-
    # controlled redundant hashing work for an index that is invalid anyway.
    assert loader.index(duplicate_guard) < loader.index(source_path_guard)
