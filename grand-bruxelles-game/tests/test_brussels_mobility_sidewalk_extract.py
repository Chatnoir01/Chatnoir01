...
def test_contract_scope_and_digest_policy():
    ...
    assert e["validated_snapshot"]["feature_count"] == 3158
...
def test_timestamp_and_feature_order_do_not_change_canonical_content_digest():
    ...
    a = m.canonicalize_feature_collection(raw(fs, "2026-08-22T14:00:00Z"), attribute_domain_raw=DOMAIN, query_bbox=BBOX)
    b = m.canonicalize_feature_collection(raw(list(reversed(fs)), "2026-08-22T15:00:00Z"), attribute_domain_raw=DOMAIN, query_bbox=BBOX)
...
def test_unpublished_ssft_identity_drift_duplicate_and_domain_drift_are_rejected():
    m = module()
    cases = [(raw([feature(ssft="ZZ")]), DOMAIN, "unpublished ssft")]
