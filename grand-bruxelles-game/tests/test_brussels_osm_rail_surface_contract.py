...
def main():
    ...
    for anchor_id, anchor in ANCHORS.items():
        for osm_id, index, a, b in visible_segments:
            ...
            if nearest is None or candidate < nearest:
                nearest = candidate
    assert nearest is not None and nearest[0] <= MAX_ANCHOR_DISTANCE_M
    print(f"BRUSSELS_OSM_RAIL_SOURCE_AUDIT: railways={len(railways)} renderable_segments={len(visible_segments)} nearest_anchor={nearest[1]} nearest_m={nearest[0]:.3f} osm_id={nearest[2]} segment={nearest[3]} visual_gate=gt3>={MIN_GT3_RATIO:.2%},gt8>={MIN_GT8_RATIO:.2%},bbox>={MIN_BBOX_WIDTH}x{MIN_BBOX_HEIGHT}")
    ...
    assert MATERIAL.exists() and RUNTIME.exists()
    material = MATERIAL.read_text(encoding="utf-8")
    runtime = RUNTIME.read_text(encoding="utf-8")
    project = PROJECT.read_text(encoding="utf-8")
    for token in ('brussels_osm_rail_surface_v1','surface_composition_claimed", false','exact_rgb_is_photometric_measurement", false','wear_pattern_claimed", false','geometry_changed", false','ODbL-1.0'):
        assert token in material, f"missing material contract: {token}"
