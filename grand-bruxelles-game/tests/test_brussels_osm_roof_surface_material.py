...
def main() -> None:
    ...
    for token in ['class_name BrusselsOsmRoofSurfaceMaterial','MATERIAL_FAMILY := "brussels_osm_roof_surface_v1"','ODbL-1.0','roof_material_claimed','exact_rgb_is_photometric_measurement','weathering_claimed','geometry_changed','authored_presentation_not_source_measurement']:
        assert token in material, token
    for token in ['begins_with("Roof_")','generic_osm_roof','geometry_unchanged','set_enhanced_enabled','BRUSSELS_OSM_ROOF_SURFACE_READY']:
