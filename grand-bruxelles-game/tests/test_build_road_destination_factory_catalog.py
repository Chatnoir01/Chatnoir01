...
def test_derivation_rejects_registry_scope_drift():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(registry)
    mutated["scope"] = "Brussels playable corridor only"
    with pytest.raises(SystemExit, match="registry scope contract drift"):
        build_catalog(mutated, evidence)


def test_derivation_rejects_registry_evidence_baseline_drift():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(registry)
    mutated["evidence_baseline"]["missing_niscodes"][0] = "21001"
    with pytest.raises(SystemExit, match="registry evidence baseline contract drift"):
        build_catalog(mutated, evidence)
...
@pytest.mark.parametrize(
    ("field", "drifted"),
    [
        ("provider", "Unverified coordinated provider"),
        ("license", "UNKNOWN"),
        ("endpoint", "https://example.invalid/overpass"),
    ],
)
def test_derivation_rejects_coordinated_common_source_provenance_drift(field, drifted):
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated_registry = copy.deepcopy(registry)
    mutated_evidence = copy.deepcopy(evidence)
    mutated_registry["source"][field] = drifted
    mutated_evidence["source"][field] = drifted
    with pytest.raises(SystemExit, match="source provenance contract drift"):
        build_catalog(mutated_registry, mutated_evidence)


def test_derivation_rejects_registry_relation_reference_drift():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(registry)
    mutated["source"]["relation_reference"] = "Unverified municipality relation crosswalk"
    with pytest.raises(SystemExit, match="registry relation reference contract drift"):
        build_catalog(mutated, evidence)
...
def test_derivation_rejects_locked_artifact_name_not_bound_to_municipality_identity():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(evidence)
    row = mutated["successful_acquisitions"][0]
    row["artifact"]["name"] = f"road-source-{row['niscode']}-wrong-municipality"
    with pytest.raises(SystemExit, match="artifact name identity drift"):
        build_catalog(registry, mutated)


def test_derivation_rejects_noncanonical_osm_base_timestamp():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(evidence)
    mutated["successful_acquisitions"][0]["osm_base_timestamp"] = "2026-09-07T12:00Z"
    with pytest.raises(SystemExit, match="invalid locked source evidence"):
        build_catalog(registry, mutated)


def test_derivation_rejects_duplicate_locked_semantic_source_identity():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(evidence)
    first = mutated["successful_acquisitions"][0]
    second = mutated["successful_acquisitions"][1]
    second["raw_snapshot_semantic_sha256"] = first["raw_snapshot_semantic_sha256"]
    second["normalized_game_source_semantic_sha256"] = first["normalized_game_source_semantic_sha256"]
    with pytest.raises(SystemExit, match="duplicate locked source semantic identity"):
        build_catalog(registry, mutated)


def test_derivation_rejects_cross_role_locked_semantic_source_identity_alias():
    registry = load(REGISTRY)
    evidence = load(EVIDENCE)
    mutated = copy.deepcopy(evidence)
    first = mutated["successful_acquisitions"][0]
    second = mutated["successful_acquisitions"][1]
    second["normalized_game_source_semantic_sha256"] = first["raw_snapshot_semantic_sha256"]
    with pytest.raises(SystemExit, match="duplicate locked source semantic identity"):
        build_catalog(registry, mutated)
