...
def test_osm_road_frame_is_bridged_to_the_same_canonical_world_frame() -> None:
    ...
    assert '"road_runtime_index_bound": true' in result.stdout
    assert f'"road_source_sha256": "{CURRENT_SOURCE_SHA256}"' in result.stdout
    assert f'"road_runtime_catalog_sha256": "{CURRENT_CATALOG_SHA256}"' in result.stdout
...
def test_runtime_index_authorization_drift_is_rejected(tmp_path: Path) -> None:
    index = json.loads(ROAD_INDEX.read_text(encoding="utf-8"))
    index["authorization"]["runtime_mount_authorized"] = True
    modified_index = tmp_path / "road-index.json"
    modified_index.write_text(json.dumps(index), encoding="utf-8")
    result = run_validator("--runtime-index", str(modified_index))
    assert result.returncode == 2, result.stdout + result.stderr
    assert "runtime road index authorization rail opened" in result.stdout


def test_runtime_index_source_descriptor_drift_is_rejected(tmp_path: Path) -> None:
    index = json.loads(ROAD_INDEX.read_text(encoding="utf-8"))
