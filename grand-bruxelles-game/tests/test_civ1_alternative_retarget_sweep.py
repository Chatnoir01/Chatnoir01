...
# LeftFoot source minimum sits at the end of the 120-sample loop, so the true
# chronological five-sample window crosses 119 -> 0. This must remain valid.
LEFT_SAMPLES = [117, 118, 119, 0, 1]
RIGHT_SAMPLES = [57, 58, 59, 60, 61]
BASE = {
...
def main():
    ...
    forged_samples = copy.deepcopy(BASE); forged_samples["candidates"][1]["feet"]["RightFoot"]["planted_sample_indices"] = [56,57,58,59,60]; expect_error(forged_samples)
    duplicate_samples = copy.deepcopy(BASE); duplicate_samples["candidates"][1]["feet"]["LeftFoot"]["planted_sample_indices"] = [117,118,118,0,1]; expect_error(duplicate_samples)
    nonchronological = copy.deepcopy(BASE); nonchronological["candidates"][1]["feet"]["LeftFoot"]["planted_sample_indices"] = [117,118,119,1,2]; expect_error(nonchronological)
    bool_sample = copy.deepcopy(BASE); bool_sample["candidates"][1]["feet"]["LeftFoot"]["planted_sample_indices"] = [117,118,119,0,True]; expect_error(bool_sample)
    bad_cycle = copy.deepcopy(BASE); bad_cycle["candidates"][1]["feet"]["LeftFoot"]["animation_cycle_sample_count"] = 121; expect_error(bad_cycle)
