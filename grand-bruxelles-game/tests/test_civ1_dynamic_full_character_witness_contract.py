...
def test_dynamic_full_character_witness_contract() -> None:
    ...
    required = [
        'const BODY_PATH := "res://civ1_body.glb"', 'const HEAD_PATH := "res://vitruvian_head.glb"',
        'const HEAD_BONE := "mixamorig_Head"', 'frames.size() != 120', 'BoneAttachment3D.new()',
        'head_rig.global_transform = Transform3D.IDENTITY', 'func _head_expected_world(',
        ...
        '"head_attachment_calibration_offset_m":head_attachment_calibration_offset_m', '"head_settle_frames":2',
        '_support_band_path(frames, "RightFoot")', '_support_band_path(frames, "LeftFoot")',
        '"runtime_authorized":false', '"visual_approval_claimed":false', '"player_view_claimed":false',
        'max_pose_error > 0.0001 or max_head_follow_error > 0.0001', '"JETER_DYNAMIC_TECHNICAL_DRIFT"',
        'root.size = Vector2i(WIDTH, HEIGHT)', 'var lo: float = float(ys.min())', 'var hi: float = float(ys.max())',
        'var knee_corr: float = max(', '69, 70, 71, 72', '"targeted_right_foot_transition_samples":[69,70,71,72]',
    ]
    for token in required:
        assert token in source, token
    forbidden = ['var lo := ys.min()', 'var hi := ys.max()', 'var knee_corr := max(',
