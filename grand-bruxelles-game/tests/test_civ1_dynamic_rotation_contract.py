...
def test_radius_below_generator_domain_fails_closed():
    data = evidence()
    data["radius_samples"] = 1
    data["samples"][57]["rotation_delta_rad"] = 0.0
    data["samples"][61]["rotation_delta_rad"] = 0.0
...
def test_promotion_or_native_measurement_claims_fail_closed():
    for field in ("candidate_is_native_measurement", "runtime_authorized", "visual_approval_claimed"):
        ...
        assert dyn.assess(data)["verdict"] == dyn.BLOCK
    data = evidence()
    data["candidate_kind"] = "other"
    assert dyn.assess(data)["verdict"] == dyn.BLOCK
    data = evidence()
    del data["runtime_authorized"]
    assert dyn.assess(data)["verdict"] == dyn.BLOCK
...
def test_asymmetric_window_fails_closed():
    data = evidence()
    data["samples"][58]["rotation_delta_rad"] = 0.09
    data["samples"][60]["rotation_delta_rad"] = 0.10
    assert dyn.assess(data)["verdict"] == dyn.BLOCK
    data = evidence()
    data["samples"][58]["rotation_delta_rad"] = -0.10
    data["samples"][60]["rotation_delta_rad"] = 0.10
    assert dyn.assess(data)["verdict"] == dyn.BLOCK
