...
def evidence():
    ...
    for i in range(120):
        ...
        angle = 0.20 if distance == 0 else 0.10 if distance == 1 else 0.05 if distance == 2 else 0.0
        samples.append({"rotation_delta_rad": angle, "right_foot_length_error_m": 0.0, "left_foot_delta_m": 0.0})
    return {"candidate_kind":"civ1_rightfoot_dynamic_rotation_schedule","rotation_axis":"target_local_z","candidate_is_native_measurement":False,"runtime_authorized":False,"visual_approval_claimed":False,"center_sample":59,"radius_samples":2,"cycle_sample_count":120,"baseline_source_plant_sample":59,"baseline_cycle_sample_count":120,"samples":samples}


def test_valid_dynamic_candidate_requires_native_measurement():
    out=dyn.assess(evidence()); assert out["verdict"]==dyn.ALLOW; assert out["rotation_axis"]=="target_local_z"; assert out["runtime_authorized"] is False; assert out["visual_approval_claimed"] is False


def test_axis_identity_fails_closed():
    for value in (None,"target_local_x","local_z",True):
        data=evidence()
        if value is None: del data["rotation_axis"]
        else: data["rotation_axis"]=value
        assert dyn.assess(data)["verdict"]==dyn.BLOCK


def test_radius_below_generator_domain_fails_closed():
    data=evidence(); data["radius_samples"]=1; data["samples"][57]["rotation_delta_rad"]=0.0; data["samples"][61]["rotation_delta_rad"]=0.0; assert dyn.assess(data)["verdict"]==dyn.BLOCK
...
def test_length_leftfoot_center_taper_and_symmetry_fail_closed():
    ...
    data=evidence(); data["samples"][58]["rotation_delta_rad"]=0.09; assert dyn.assess(data)["verdict"]==dyn.BLOCK
    data=evidence(); data["samples"][58]["rotation_delta_rad"]=-0.10; assert dyn.assess(data)["verdict"]==dyn.BLOCK
