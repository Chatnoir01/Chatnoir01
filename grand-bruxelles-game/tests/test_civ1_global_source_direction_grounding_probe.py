...
def test_rejects_left_control_token_spoofing():
    for spoof in ['left_foot_reference_ab_drift', 'prefix_left_foot_reference_ab', 'left_foot_reference_ab2']:
        mutated=BASE.replace('left_foot_reference_ab',spoof,1)
        with pytest.raises(ValueError): probe.transform(mutated)
