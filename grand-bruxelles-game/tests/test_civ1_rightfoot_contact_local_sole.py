...
# Preserved LeftFoot-style semantics: single side + <=15% cross-distance spread
# is not enough; every distance must also stay <=15% within-distance deviation.
good = {
...
# Real-shape causal regression from artifact 10001048846. Cross-distance means
# are still inside ~15%, but the per-frame sole identity moves far too much
# inside each distance, so this contact-context interval must be JETER.
measured_shape = {
...
# Side flips and threshold rescue remain rejected.
