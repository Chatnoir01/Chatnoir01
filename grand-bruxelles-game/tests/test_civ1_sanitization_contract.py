...
def test_followup_requires_integrity_locomotion_and_player_view_evidence() -> None:
    followup = STATUS["sanitization_contract"]["required_followup"]
    text = "\n".join(followup).lower()
...
def main() -> int:
    tests = [
        test_sanitization_contract_is_fail_closed,
        test_body_pin_still_names_embedded_animation_blocker,
        test_followup_requires_integrity_locomotion_and_player_view_evidence,
        test_tool_exists_in_repo,
    ]
    for test in tests:
        test()
