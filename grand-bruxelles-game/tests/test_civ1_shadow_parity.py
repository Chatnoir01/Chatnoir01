...
def payload() -> dict:
    per_bone = {
        "RightFoot": {
            "source_vertical_min_sample_index": 59,
            "target_vertical_min_sample_index": 86,
            "phase_delta_samples": 27,
            "phase_delta_seconds": 0.15,
            "material_phase_divergence": True,
        },
        "RightLowerLeg": {
            "source_vertical_min_sample_index": 3,
            "target_vertical_min_sample_index": 2,
            "phase_delta_samples": -1,
            "phase_delta_seconds": -0.0055,
            "material_phase_divergence": False,
        },
    }
    return {
...
def main() -> int:
    base = payload()
    shadow = payload()
    # Reproduce the native evidence: a one-sample non-material RightLowerLeg jitter is allowed.
    ...
    assert result["verdict"] == "ALLOW_QA_SHADOW_PARITY"

    # Material-class drift must fail closed even if indices stay close.
    bad = payload()
    bad["phase_vertical_summary"]["per_bone"]["RightFoot"]["material_phase_divergence"] = False
    result = module.assess(base, bad)
    assert result["semantic_parity_verified"] is False
    assert "material_class_mismatch:RightFoot" in result["failures"]

    # More than one sample of phase drift is not attributed to run jitter.
    bad = payload()
    bad["phase_vertical_summary"]["per_bone"]["RightLowerLeg"]["source_vertical_min_sample_index"] = 1
    result = module.assess(base, bad)
    assert result["semantic_parity_verified"] is False
    assert any(item.startswith("phase_jitter_exceeded:RightLowerLeg") for item in result["failures"])

    # Promotion-critical RightFoot counterfactual conclusions remain exact.
    bad = payload()
    bad["right_foot_reference_ab"]["normalized_phase_delta_samples"] = 1
    result = module.assess(base, bad)
    assert result["semantic_parity_verified"] is False
    assert "top_level_mismatch:right_foot_reference_ab" in result["failures"]
