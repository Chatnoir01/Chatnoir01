...
assert good["success_marker"] == "CIV1_GLOBAL_CHAIN_DIAGNOSTIC_OK"
assert good["runtime_authorized"] is False and good["visual_approval_claimed"] is False

preflight = m.assess(
    "Godot Engine v4.7.1\nCIV1_SKIN_BIND_PREFLIGHT_OK\n",
    "CIV1_SKIN_BIND_PREFLIGHT_OK",
)
assert preflight["diagnostic_complete"] is True
assert preflight["skin_bind_integrity_verified"] is True
assert preflight["verdict"] == "ALLOW_DIAGNOSTIC_ONLY"
assert preflight["success_marker"] == "CIV1_SKIN_BIND_PREFLIGHT_OK"

wrong_marker = m.assess(
    "Godot Engine v4.7.1\nCIV1_GLOBAL_CHAIN_DIAGNOSTIC_OK\n",
    "CIV1_SKIN_BIND_PREFLIGHT_OK",
)
assert wrong_marker["diagnostic_complete"] is False
assert wrong_marker["skin_bind_integrity_verified"] is False
assert wrong_marker["verdict"] == "BLOCK_INCOMPLETE_DIAGNOSTIC"
