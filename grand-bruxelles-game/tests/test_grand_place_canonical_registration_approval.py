...
def fixture(root: Path) -> None:
    write_json(root / mod.CANDIDATE_REVIEW_PATH, {
        ...
        "source_evidence": {
            ...
            "license": "CC0-1.0",
            "source_manifest_sha256": mod.SOURCE_MANIFEST_SHA,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert result["destination_readiness"] == "APPROVED_FOR_EVIDENCE_ONLY_REGISTRATION_NOT_RENDERED"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        write_json(root / mod.REGISTERED_INDEX_PATH, payload)
        expect_error(lambda: mod.review(root, base), "registered index count drift")
    ...
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        fixture(root)
        payload = json.loads((root / mod.SOURCE_LOCK_PATH).read_text())
        payload["source"]["license"] = "UNKNOWN"
        write_json(root / mod.SOURCE_LOCK_PATH, payload)
        expect_error(lambda: mod.review(root, base), "source-lock license drift")

    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        fixture(root)
        payload = json.loads((root / mod.REGISTRATION_REVIEW_PATH).read_text())
        payload["municipality_evidence"]["municipality_niscode"] = "99999"
        write_json(root / mod.REGISTRATION_REVIEW_PATH, payload)
        expect_error(lambda: mod.review(root, base), "municipality NIS drift")

    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        fixture(root)
        payload = json.loads((root / mod.CANDIDATE_REVIEW_PATH).read_text())
        payload["source_evidence"]["source_semantic_sha256"] = "0" * 64
        write_json(root / mod.CANDIDATE_REVIEW_PATH, payload)
        expect_error(lambda: mod.review(root, base), "candidate source semantic SHA drift")
