...
MODULE_PATH = ROOT / "tools/qa/measure_grand_place_correct_source_cell_municipality.py"


def load_module():
    spec = importlib.util.spec_from_file_location("grand_place_correct_municipality", MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot import corrected Grand-Place municipality preflight")
...
def main() -> int:
    module = load_module()
    measurement_path = ROOT / "data/provenance/grand_place_correct_urbis_source_cell.measurement.json"
    ...
    semantic_fixture = {
        ...
        "municipality_source": {"authority":"Paradigm / Brussels-Capital Region","raw_payload_sha256":"a" * 64},
    ...
    full = {"type":"FeatureCollection","features":[municipality_feature("single","99999",box(min_x-10,min_y-10,max_x+10,max_y+10))]}
    ...
    boundary = {"type":"FeatureCollection","features":[municipality_feature("west","11111",box(min_x,min_y,mid_x,max_y)),municipality_feature("east","22222",box(mid_x,min_y,max_x,max_y))]}
    ...
    majority_boundary = {"type":"FeatureCollection","features":[municipality_feature("majority","33333",box(min_x,min_y,dominant_x,max_y)),municipality_feature("minority","44444",box(dominant_x,min_y,max_x,max_y))]}
    ...
    print("GRAND_PLACE_CORRECT_MUNICIPALITY_REGRESSIONS_OK source_lock=true single=true boundary_hold=true majority_inference_rejected=true forensic_drift_ignored=true semantic_drift_rejected=true rails_closed=true")
    return 0
