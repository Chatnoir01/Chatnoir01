...
def test_facade_evidence_upload_is_single_root_and_preserves_full_frame_witness():
    workflow = WORKFLOW.read_text(encoding="utf-8")
    assert "Stage witness logs inside artifact root" in workflow, (
        "the upload currently mixes repository-relative evidence with /tmp logs; "
        "GitHub artifact packaging can then retain only the logs and silently drop the PNG witness"
    )
