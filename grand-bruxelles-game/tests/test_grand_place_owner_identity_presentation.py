...
def test_maison_du_roi_winding_mitigation_is_exact_owner_reversible_and_fail_closed():
    script = SCRIPT.read_text(encoding="utf-8")
    assert 'TWO_SIDED_SOURCE_OWNER_IDS := ["1654360"]' in script
    assert 'func set_source_winding_mitigation_enabled(enabled: bool) -> bool:' in script
    assert 'source winding mitigation toggle requested before presentation readiness' in script
    assert 'source winding mitigation material ownership drifted' in script
    ...
    assert 'source_winding_mitigation_enabled", enabled' in script
    assert 'source_winding_mitigation_enabled", true' in script
    assert 'source_winding_mitigation", mitigation_enabled' in script
