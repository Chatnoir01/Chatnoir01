...
def test_identity_module_points_to_real_script_and_is_enabled():
    ...
    assert module == {
        ...
        "name": "GrandPlaceOwnerIdentityPresentation",
        "path": "res://game/scripts/grand_place_owner_identity_presentation.gd",
        "enabled": True,
    }
...
def test_maison_du_roi_winding_diagnostic_defaults_to_cull_back_and_is_reversible():
    script = SCRIPT.read_text(encoding="utf-8")
    assert 'WINDING_DIAGNOSTIC_OWNER_IDS := ["1654360"]' in script
    assert 'mat.cull_mode = BaseMaterial3D.CULL_BACK' in script
    assert 'func set_source_winding_mitigation_enabled(enabled: bool) -> bool:' in script
    assert 'source winding diagnostic toggle requested before presentation readiness' in script
    assert 'source winding diagnostic material ownership drifted' in script
    assert 'GrandPlaceContour_%s_%s' in script
    assert 'mat.cull_mode = BaseMaterial3D.CULL_DISABLED if enabled else BaseMaterial3D.CULL_BACK' in script
    assert 'source_winding_mitigation_enabled", false' in script
    assert 'source_winding_mitigation_production_authorized", false' in script
    assert 'source_winding_diagnostic_candidate' in script
