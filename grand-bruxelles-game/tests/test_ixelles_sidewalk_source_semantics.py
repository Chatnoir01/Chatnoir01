...
def test_ixelles_sidewalk_runtime_does_not_reintroduce_sw_only_semantics() -> None:
    ...
    # Preserve the already-shipped Ixelles LABO presentation while keeping its
    # legacy cell contract explicitly separate from the current corridor layer.
    ...
    # Ixelles. It must bind from the actual node-added candidate and validate
    # ancestry, rather than depending on SceneTree.current_scene (which is null
    # in legitimate root-instantiated production witnesses).
    assert "IxellesMidiSidewalkRuntime" in project_text
    assert "node_added.connect" in runtime_text
    assert "_try_bind_candidate" in runtime_text
    assert "_has_named_ancestor" in runtime_text
    assert "current_scene" not in runtime_text
