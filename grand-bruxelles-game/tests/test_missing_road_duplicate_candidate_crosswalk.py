...
def load_and_validate():
    ...
    sl=c['source_lock']; assert sl['path']=='data/qa/brussels_missing_road_source_artifact_lock.json'
    assert sl['source_run_id']==l['source_run_id']==33286183671 and sl['source_head_sha']==l['source_head_sha']; sha(sl['source_head_sha'],40)
    ...
    parts=c['parts']; assert type(parts) is list and len(parts)==1; meta=parts[0]; assert set(meta)==PART_FIELDS
    part_path=ROOT/meta['path']; raw=part_path.read_bytes(); assert hashlib.sha256(raw).hexdigest()==meta['file_sha256']
    part=json.loads(raw); assert part['schema']=='grand-bruxelles-road-duplicate-candidate-crosswalk-part-v1' and part['part_index']==0 and part['entry_fields']==ENTRY_FIELDS
