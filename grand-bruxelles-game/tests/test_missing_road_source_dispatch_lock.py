...
def test_dispatch_matrix_is_derived_exactly_from_immutable_unresolved_lock() -> None:
    ...
    selected = re.findall(r"- \{nis: '(\d+)', id: ([a-z0-9_]+)\}", workflow)

    # The workflow matrix is a deterministic projection of the immutable lock:
    # no reordering, duplication, omission, or undeclared extra dispatch entries.
    assert len(selected) == len(set(selected)), "duplicate unresolved dispatch entry"
