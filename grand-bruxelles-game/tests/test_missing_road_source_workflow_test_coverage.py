...
REQUIRED_SOURCE_FACTORY_TESTS = {
    "test_road_destination_factory_artifact_uniqueness.py",
}
...
def test_missing_road_source_workflow_watches_and_executes_exact_same_regressions():
    ...
    assert REQUIRED_SOURCE_FACTORY_TESTS <= watched, (
        f"dedicated gate misses required Data regressions in paths: {sorted(REQUIRED_SOURCE_FACTORY_TESTS-watched)}"
    )
    assert REQUIRED_SOURCE_FACTORY_TESTS <= executed, (
        f"dedicated gate misses required Data regressions in pytest: {sorted(REQUIRED_SOURCE_FACTORY_TESTS-executed)}"
    )
