...
def canonical_source() -> dict:
    return {
        ...
        "license": "ODbL-1.0",
        "stats": {"roads": 1},
        "roads": [{"osm_id": 1, "name": "Synthetic provenance witness", "class": "residential", "points": [[0.0, 0.0], [1.0, 1.0]]}],
...
def require_transform_rejected(raw: bytes, label: str) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        proc = subprocess.run(
            [sys.executable, str(PROJECT / "tools" / "transform_osm_to_game.py"), "--input", str(source), "--output", str(output)],
            cwd=PROJECT,
            capture_output=True,
            text=True,
            check=False,
...
def main() -> int:
    ...
    duplicate_license = canonical.replace(b'"license":"ODbL-1.0"', b'"license":"NOT-A-LICENSE","license":"ODbL-1.0"', 1)
    require_rejected(duplicate_license, "duplicate license with canonical final value")

    duplicate_source = canonical.replace(b'"source":"OpenStreetMap contributors via Overpass API"', b'"source":"untrusted provider","source":"OpenStreetMap contributors via Overpass API"', 1)
