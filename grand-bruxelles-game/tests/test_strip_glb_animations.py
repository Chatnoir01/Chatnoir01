...
def make_glb(document: dict, bin_payload: bytes = b"\x01\x02\x03\x04") -> bytes:
    ...
    body = (
        struct.pack("<II", len(json_bytes), module.JSON_CHUNK)
        + json_bytes
        + struct.pack("<II", len(bin_payload), module.BIN_CHUNK)
        + bin_payload
    )
...
def test_rejects_truncated_or_wrong_version_glb() -> None:
    try:
        module.strip_animations(b"glTF")
        raise AssertionError("truncated GLB accepted")
    except module.GlbError:
        pass
    source = make_glb({"asset": {"version": "2.0"}})
    bad = source[:4] + struct.pack("<I", 1) + source[8:]
    try:
        module.strip_animations(bad)
        raise AssertionError("GLB v1 accepted")
    except module.GlbError:
        pass
...
def main() -> int:
    tests = [
        test_strips_only_animation_json_and_preserves_binary_payload,
        test_output_is_deterministic,
        test_no_animation_source_is_a_valid_noop_semantically,
        test_rejects_truncated_or_wrong_version_glb,
        test_rejects_malformed_animation_shape,
    ]
    for test in tests:
        test()
