...
def main() -> int:
    ...
    try:
        transform_osm_to_game.building_height({"height": "12mm"})
    except ValueError:
        pass
    else:
        raise AssertionError("an explicit unsupported building height unit must fail closed instead of inventing a default height")
    assert transform_osm_to_game.building_height({}) == 10.5, "default height is allowed only when an explicit height tag is absent"
    assert transform_osm_to_game.building_height({"height": "12 m"}) == 12.0, "explicit metre height must remain supported"

    try:
        transform_osm_to_game.railway_vertical_metadata({"layer": "1m"})
    except ValueError:
        pass
    else:
        raise AssertionError("invalid railway layer must fail closed instead of being treated as layer 0")

    assert transform_osm_to_game.railway_vertical_metadata({})["layer"] == 0.0
    assert transform_osm_to_game.railway_vertical_metadata({"layer": "-1"})["surface_visible"] is False
    ...
    open_building_result = transform_osm_to_game.convert(open_building, transform_osm_to_game.DEFAULT_ORIGIN)
    assert open_building_result["stats"]["buildings"] == 0, "open OSM building way must not be implicitly closed and materialized"

    near_closed_after_projection = {
        "elements": [
            {
                "type": "way",
                "id": 33,
    ...
    print("TRANSFORM_OSM_JSON_STRICT_OK duplicate_keys_rejected=true constants_rejected=true float_overflow_rejected=true osm_identity_validated=true duplicate_osm_identity_rejected=true osm_tag_strings_required=true numeric_tag_units_strict=true explicit_building_height_invalid_rejected=true absent_building_height_default_allowed=true railway_layer_ambiguity_rejected=true open_building_rejected=true source_building_closure_required=true projection_rounding_closure_guarded=true geometry_coordinate_pair_required=true node_coordinate_pair_required=true wgs84_ranges_required=true railway_bounds_accounted=true railway_order_deterministic=true finite_origin_required=true network_used=false")
    return 0
