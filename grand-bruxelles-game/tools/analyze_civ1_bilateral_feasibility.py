...
def analyze(p):
    ...
    return {'schema':'grand-bruxelles-civ1-bilateral-feasibility-v1','diagnostic_only':True,'runtime_authorized':False,'visual_approval_claimed':False,'grounding_verified':False,'baseline_phase_delta_samples':baseline_phase,'baseline_vertical_range_m':baseline_v,'baseline_horizontal_travel_m':baseline_h,'source_pelvis_range_m':source_range,'source_pelvis_max_step_m':source_step,'contact_gated_shift_evidence':evidence,'candidate':{'vertical_shift_samples':shift,'max_used_pelvis_mm':used,'required_cap_mm':used+MIN_MARGIN_MM,'required_cap_margin_mm':MIN_MARGIN_MM,'pelvis_amplitude_fraction_of_source_range':(used/1000.0)/source_range},'phase_delta_samples':chosen['phase_delta_samples'],'vertical_range_m':chosen['vertical_range_m'],'horizontal_travel_m':chosen['horizontal_travel_m'],'max_bone_length_error_m':max_len,'max_joint_delta_rad':max_angle,'max_right_knee_displacement_m':max_rk,'max_left_knee_displacement_m':max_lk,'max_pelvis_step_m':max_step,'physical_envelope_pass':physical,'verdict':'AMELIORER_BILATERAL_ENVELOPE' if physical else 'JETER_BILATERAL_ENVELOPE'}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('native_json');ap.add_argument('output_json');a=ap.parse_args();r=analyze(json.loads(Path(a.native_json).read_text()));Path(a.output_json).write_text(json.dumps(r,indent=2,sort_keys=True)+'\n');c=r['candidate'];print('CIV1_BILATERAL_FEASIBILITY_OK '+f"shift={c['vertical_shift_samples']} used_mm={c['max_used_pelvis_mm']} cap_mm={c['required_cap_mm']} phase={r['baseline_phase_delta_samples']}->{r['phase_delta_samples']} joint_rad={r['max_joint_delta_rad']:.6f} verdict={r['verdict']}")
