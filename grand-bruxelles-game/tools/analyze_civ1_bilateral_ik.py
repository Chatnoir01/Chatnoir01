...
CYCLE=120; N=121; SHIFT=16; CAP_MM=55; MATERIAL=12; EPS=1e-9
...
def solve_two_bone(hip,knee,foot,goal):
    ...
    bend=unit(bend)
    k1=add(center,mul(bend,h)); k2=sub(center,mul(bend,h)); solved=k1 if dist(k1,knee)<=dist(k2,knee) else k2
    uq=quat_from_to(sub(knee,hip),sub(solved,hip)); lq=quat_from_to(sub(foot,knee),sub(goal,solved))
    return {'knee':solved,'upper_delta_xyzw':uq,'lower_delta_xyzw':lq,'upper_angle_rad':qangle(uq),'lower_angle_rad':qangle(lq),'foot_error_m':dist(goal,goal),'upper_length_error_m':abs(dist(hip,solved)-l1),'lower_length_error_m':abs(dist(solved,goal)-l2)}
...
def analyze(p):
    ...
    ys=[origin(samples[i],'Hips','source')[1] for i in range(CYCLE)]; source_step=max(abs(ys[i]-ys[i-1]) for i in range(1,CYCLE))
    path=smooth_path(allowed_dys(samples,seq),source_step)
    if path is None: raise ValueError('validated +16/55 pelvis path no longer reachable')
    right=[]; left=[]
    for i,s in enumerate(samples[:CYCLE]):
