...
def observation(path:Path):
    ...
    lo=max(0,min(primary)-SUPPORT_PAD_PX); hi=min(width-1,max(primary)+SUPPORT_PAD_PX); row=rows[bottom]; weighted=[]
    ...
    return {"bottom_y_px":bottom,"position_row_semantic":POSITION_ROW_SEMANTIC,"primary_min_x_px":min(primary),"primary_max_x_px":max(primary),"primary_pixel_count":len(primary),"integer_centroid_x_px":sum(primary)/len(primary),"subpixel_centroid_x_px":sum(x*w for x,w in weighted)/total,"weighted_support_min_x_px":min(x for x,_ in weighted),"weighted_support_max_x_px":max(x for x,_ in weighted),"weighted_support_count":len(weighted)}
def _path(xs):
...
def scale_calibration(measurements):
    ...
    p8=by[8]; non_increasing=p8<=p4+EPS; expected8=p4*0.5; rel8=abs(p8-expected8)/max(abs(expected8),EPS); far_scale_ok=rel8<=MAX_FAR_SCALE_REL_ERROR
    return {"near_2_to_4_expected_half":expected4,"near_2_to_4_relative_error":rel,"near_calibration_passed":near_ok,"far_4_to_8_expected_half":expected8,"far_4_to_8_relative_error":rel8,"far_non_increasing":non_increasing,"far_scale_calibration_passed":far_scale_ok,"quantitative_8m_candidate":bool(near_ok and non_increasing and far_scale_ok)}
def analyze(capture_dir:Path):
    ...
    for d in DISTANCES:
        ...
        ms.append({"distance_m":d,"records":rec,"integer_path_px":_path([r["integer_centroid_x_px"] for r in rec]),"subpixel_path_px":_path([r["subpixel_centroid_x_px"] for r in rec])})
    scale=scale_calibration(ms); verdict="AMELIORER_SUBPIXEL_8M_CANDIDATE_NO_PROMOTION" if scale["quantitative_8m_candidate"] else "AMELIORER_SUBPIXEL_DISTANCE_SCALE_REJECTED_NO_PROMOTION"
    return {"schema":"grand-bruxelles-civ1-bottomrow-subpixel-v3","diagnostic_only":True,"source_semantic":"actual_godot_1280x720_same_bottom_most_row_luminance_weighted","position_row_semantic":POSITION_ROW_SEMANTIC,"distances_m":list(DISTANCES),"samples":list(SAMPLES),"white_threshold":WHITE,"weight_floor":WEIGHT_FLOOR,"support_pad_px":SUPPORT_PAD_PX,"distance_measurements":ms,"distance_scale_calibration":scale,"perceptual_2_8m_claimed":False,"planted_contact_claimed":False,"animation_correction_authorized":False,"runtime_authorized":False,"visual_approval_claimed":False,"player_view_claimed":False,"verdict":verdict}
def main(argv):
    ...
    print("CIV1_BOTTOMROW_SUBPIXEL_OK",{d['distance_m']:d['subpixel_path_px'] for d in r['distance_measurements']},r['distance_scale_calibration']); return 0
