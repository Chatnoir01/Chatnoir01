...
MAX_NEAR_SCALE_REL_ERROR=0.25; EPS=1e-9
...
def observation(path:Path):
    ...
    return {"bottom_y_px":bottom,"primary_min_x_px":min(primary),"primary_max_x_px":max(primary),"primary_pixel_count":len(primary),
...
def scale_calibration(measurements):
    ...
    expected8=p4*0.5; rel8=abs(p8-expected8)/max(abs(expected8),EPS)
    return {"near_2_to_4_expected_half":expected4,"near_2_to_4_relative_error":rel,"near_calibration_passed":near_ok,
            ...
            "quantitative_8m_candidate": bool(near_ok and non_increasing)}
...
def analyze(capture_dir:Path):
    ...
    return {"schema":"grand-bruxelles-civ1-bottomrow-subpixel-v1","diagnostic_only":True,
