...
def _link_contribution(sample: dict, parent: str, child: str) -> dict[str, float]:
    ...
    direct_error = ((_y(sample, child, "target", _origin_key("target")) - _y(sample, parent, "target", _origin_key("target"))) - (_y(sample, child, "source", _origin_key("source")) - _y(sample, parent, "source", _origin_key("source"))))
    closure = total_error - direct_error
    if abs(closure) > 1e-9:
        raise ValueError(f"contribution closure drift for {child}: {closure}")
    return {"static_rest_error_m": static_error, "rotation_driven_error_m": rotation_error, "total_relative_error_m": direct_error, "closure_error_m": closure}


def _sample_terms(sample: dict, prefix: str) -> dict:
    upper = prefix + "UpperLeg"; lower = prefix + "LowerLeg"; foot = prefix + "Foot"
    lower_terms = _link_contribution(sample, upper, lower); foot_terms = _link_contribution(sample, lower, foot)
    return {"lowerleg": lower_terms, "foot": foot_terms, "downstream": {"static_rest_error_m": lower_terms["static_rest_error_m"] + foot_terms["static_rest_error_m"], "rotation_driven_error_m": lower_terms["rotation_driven_error_m"] + foot_terms["rotation_driven_error_m"], "total_relative_error_m": lower_terms["total_relative_error_m"] + foot_terms["total_relative_error_m"]}}
...
def analyze(payload: dict, start: int = 78, end: int = 79) -> dict:
    ...
    for sample_index in range(start, end + 1):
        ...
        right = _sample_terms(sample, "Right"); left = _sample_terms(sample, "Left")
        rows.append({"sample_index": sample_index, "time_s": float(sample["time_s"]), "right": right, "left": left})
    before, after = rows
    before_total = before["right"]["downstream"]["total_relative_error_m"]; after_total = after["right"]["downstream"]["total_relative_error_m"]
    ...
    for section in ("lowerleg", "foot", "downstream"):
        contributions[section] = {"static_rest_delta_m": _delta(before["right"], after["right"], section, "static_rest_error_m"), "rotation_driven_delta_m": _delta(before["right"], after["right"], section, "rotation_driven_error_m"), "total_delta_m": _delta(before["right"], after["right"], section, "total_relative_error_m")}
    ranked = []
    ...
    ranked.sort(reverse=True); _, dominant_link, dominant_kind, dominant_delta = ranked[0]
    return {"schema": SCHEMA, "diagnostic_only": True, "window": [start, end], "rows": rows, "right_transition_contributions": contributions, "dominant_transition_driver": {"link": dominant_link, "kind": dominant_kind, "delta_m": dominant_delta}, "runtime_authorized": False, "grounding_verified": False, "foot_slide_verified": False, "visual_approval_claimed": False, "verdict": "DIAGNOSTIC_RIGHT_DOWNSTREAM_TRANSITION_DECOMPOSED"}


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("native_json"); parser.add_argument("output_json"); args = parser.parse_args()
    result = analyze(json.loads(Path(args.native_json).read_text())); Path(args.output_json).write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    driver = result["dominant_transition_driver"]; downstream = result["right_transition_contributions"]["downstream"]
    print("CIV1_DOWNSTREAM_CONTRIBUTION_OK " f"driver={driver['link']}:{driver['kind']} " f"driver_delta_mm={driver['delta_m'] * 1000:.3f} " f"downstream_rotation_delta_mm={downstream['rotation_driven_delta_m'] * 1000:.3f} " f"downstream_static_delta_mm={downstream['static_rest_delta_m'] * 1000:.3f}")
