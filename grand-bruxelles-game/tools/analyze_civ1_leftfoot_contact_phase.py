...
def assess_bundle(bundle:dict,rendered_samples:set[int]|None=None)->dict:
 ...
 rendered=set(IDENTITY_SAMPLES if rendered_samples is None else rendered_samples); contact_complete=set(CONTACT_RENDER_SAMPLES).issubset(rendered); identity_contains=min_sample in set(IDENTITY_SAMPLES); contact_contains=min_sample in rendered
 horizontal_path=0.0
 for a,b in ((prev_i,min_sample),(min_sample,next_i)):
  pa,pb=left[a],left[b]; horizontal_path+=math.hypot(pb[0]-pa[0],pb[2]-pa[2])
 planted=two_sided and contact_contains and contact_complete
 if planted:
...
def main(argv:list[str])->int:
 ...
 print('CIV1_LEFTFOOT_CONTACT_PHASE_OK',out['leftfoot_min_sample'],out['prev_low'],out['next_low'],out['identity_window_contains_contact_min'],out['contact_render_complete'],out['verdict']); return 0
