...
SCHEMA='grand-bruxelles-civ1-lowerleg-local-basis-sweep-v1'; SAMPLE_COUNT=121; CYCLE=120; MATERIAL=12
...
def state(s,side):
    ...
    UR=mat(quat(up.get('model_rotation_xyzw'),f'UpperLeg {side} rotation'))
    LR=mat(quat(lo.get('model_rotation_xyzw'),f'LowerLeg {side} rotation'))
    loo=vec(lo.get('model_origin'),f'LowerLeg {side} origin'); fto=vec(ft.get('model_origin'),f'Foot {side} origin'); hipso=vec(hips.get('model_origin'),f'Hips {side} origin')
    ...
    return {'UR':UR,'LR':LR,'local':local,'rest':rest,'lo':loo,'foot':fto,'hips':hipso}
...
def candidate_seq(states,rest,anchor):
    corr=mmul(states['source'][anchor]['local'],tr(states['target'][anchor]['local']))
    out=[]
...
def analyze(p):
    ...
    states={side:{i:state(samples[i],side) for i in range(CYCLE)} for side in ('source','target')}; rest=mean_rest(states,'target')
    ...
    for anchor in range(CYCLE):
        seq=candidate_seq(states,rest,anchor); phase=circ(minidx(seq),si); sup=support(seq,si); vertical=vr(sup); horizontal=ht(sup)
        phase_ok=abs(phase)<abs(bp) and abs(phase)<=MATERIAL; vertical_ok=vertical<=bv+1e-9; horizontal_ok=horizontal<=bh+1e-9
    ...
    viable=[r for r in rows if r['viable']]; best=min(rows,key=lambda r:(0 if r['viable'] else 1,abs(r['phase_delta_samples']),max(0,r['horizontal_travel_m']-bh),max(0,r['vertical_range_m']-bv),r['anchor_sample']))
    return {'schema':SCHEMA,'diagnostic_only':True,'runtime_authorized':False,'visual_approval_claimed':False,'grounding_verified':False,'temporal_resampling_used':False,'foot_link_rest_preserved':True,'correction_space':'RightUpperLeg-local/bone-space','target_rest_local_m':rest,'source_vertical_min_sample':si,'baseline_vertical_min_sample':bi,'baseline_phase_delta_samples':bp,'material_phase_threshold_samples':MATERIAL,'baseline_vertical_range_m':bv,'baseline_horizontal_travel_m':bh,'tested_anchor_count':len(rows),'viable_anchor_count':len(viable),'viable_anchors':[r['anchor_sample'] for r in viable],'best':best,'rows':rows,'family_viable':bool(viable),'verdict':'AMELIORER_LOWERLEG_LOCAL_BASIS_FAMILY' if viable else 'JETER_LOWERLEG_LOCAL_BASIS_FAMILY'}
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('native_json'); ap.add_argument('output_json'); a=ap.parse_args(); r=analyze(json.loads(Path(a.native_json).read_text())); Path(a.output_json).write_text(json.dumps(r,indent=2,sort_keys=True)+'\n'); b=r['best']; print('CIV1_LOWERLEG_LOCAL_BASIS_SWEEP_OK '+f"baseline_phase={r['baseline_phase_delta_samples']} viable={r['viable_anchor_count']}/{r['tested_anchor_count']} best_anchor={b['anchor_sample']} best_phase={b['phase_delta_samples']} vertical={r['baseline_vertical_range_m']:.6f}->{b['vertical_range_m']:.6f} horizontal={r['baseline_horizontal_travel_m']:.6f}->{b['horizontal_travel_m']:.6f} verdict={r['verdict']}")
