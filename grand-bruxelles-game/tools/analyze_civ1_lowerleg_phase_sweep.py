...
SCHEMA='grand-bruxelles-civ1-lowerleg-phase-sweep-v1'
SAMPLE_COUNT=121
CYCLE=120
MATERIAL=12
MAX_SHIFT=40
...
def shifted_seq(states,rest,shift):
    out=[]
    for i in range(CYCLE):
        R=states['target'][(i+shift)%CYCLE]['R']
...
def analyze(p):
    ...
    for shift in range(-MAX_SHIFT,MAX_SHIFT+1):
        seq=shifted_seq(states,rt,shift); phase=circ(minidx(seq),si); sup=support(seq,si)
        ...
        rows.append({'shift_samples':shift,'phase_delta_samples':phase,'vertical_range_m':vertical,'horizontal_travel_m':horizontal,'phase_gate_pass':phase_ok,'vertical_no_regression':vertical_ok,'horizontal_no_regression':horizontal_ok,'viable':phase_ok and vertical_ok and horizontal_ok})
    viable=[r for r in rows if r['viable']]
    best=min(rows,key=lambda r:(0 if r['viable'] else 1,abs(r['phase_delta_samples']),max(0,r['horizontal_travel_m']-bh),max(0,r['vertical_range_m']-bv),abs(r['shift_samples'])))
    return {'schema':SCHEMA,'diagnostic_only':True,'runtime_authorized':False,'visual_approval_claimed':False,'grounding_verified':False,'foot_link_rest_preserved':True,'target_rest_local_m':rt,'source_vertical_min_sample':si,'baseline_vertical_min_sample':bi,'baseline_phase_delta_samples':bp,'material_phase_threshold_samples':MATERIAL,'baseline_vertical_range_m':bv,'baseline_horizontal_travel_m':bh,'sweep_min_shift':-MAX_SHIFT,'sweep_max_shift':MAX_SHIFT,'tested_shift_count':len(rows),'viable_shift_count':len(viable),'viable_shifts':[r['shift_samples'] for r in viable],'best':best,'rows':rows,'family_viable':bool(viable),'verdict':'AMELIORER_LOWERLEG_PHASE_FAMILY' if viable else 'JETER_LOWERLEG_PHASE_FAMILY'}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('native_json');ap.add_argument('output_json');a=ap.parse_args()
    r=analyze(json.loads(Path(a.native_json).read_text()));Path(a.output_json).write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
    b=r['best'];print('CIV1_LOWERLEG_PHASE_SWEEP_OK '+f"baseline_phase={r['baseline_phase_delta_samples']} viable={r['viable_shift_count']}/{r['tested_shift_count']} best_shift={b['shift_samples']} best_phase={b['phase_delta_samples']} vertical={r['baseline_vertical_range_m']:.6f}->{b['vertical_range_m']:.6f} horizontal={r['baseline_horizontal_travel_m']:.6f}->{b['horizontal_travel_m']:.6f} verdict={r['verdict']}")
