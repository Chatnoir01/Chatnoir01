...
def _terms(sample: dict, prefix: str) -> dict[str, float]:
    ...
    lower_relative_error = (
        (_y(sample, lower, "target") - _y(sample, upper, "target"))
        - (_y(sample, lower, "source") - _y(sample, upper, "source"))
    )
    foot_relative_error = (
        (_y(sample, foot, "target") - _y(sample, lower, "target"))
        - (_y(sample, foot, "source") - _y(sample, lower, "source"))
    )
    downstream_error = lower_relative_error + foot_relative_error
    return {
        "upperleg_hips_error_m": upper_error,
        "lowerleg_relative_error_m": lower_relative_error,
        "foot_relative_error_m": foot_relative_error,
        "downstream_error_m": downstream_error,
        "final_foot_error_m": upper_error + downstream_error,
    }
...
def _first_crossing(rows: list[dict], key: str) -> dict | None:
    for left, right in zip(rows, rows[1:]):
        ...
        if a == 0.0 or b == 0.0 or a * b < 0.0:
            return {
                "term": key,
                "from_sample": left["sample_index"],
                "to_sample": right["sample_index"],
                "from_m": a,
                "to_m": b,
            }
    return None
...
def analyze(payload: dict, start: int = 58, end: int = 88) -> dict:
    ...
    return {
        ...
        "window": [start, end],
        "first_right_downstream_zero_crossing": downstream,
        "first_right_lowerleg_relative_zero_crossing": lower,
        "first_right_foot_relative_zero_crossing": foot,
        "rows": rows,
        "runtime_authorized": False,
        "grounding_verified": False,
        "foot_slide_verified": False,
        "visual_approval_claimed": False,
        "verdict": "DIAGNOSTIC_RIGHT_DOWNSTREAM_ZERO_CROSSING_ISOLATED",
    }
...
def main() -> None:
    ...
    print(
        "CIV1_RIGHT_DOWNSTREAM_TRANSITION_OK "
        f"crossing={crossing['from_sample']}->{crossing['to_sample']} "
        f"from_mm={crossing['from_m'] * 1000:.3f} "
        f"to_mm={crossing['to_m'] * 1000:.3f}"
    )
