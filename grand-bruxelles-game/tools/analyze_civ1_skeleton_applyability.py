...
def analyze(payload, previous_module):
    ...
    if summary.get('verdict')!='AMELIORER_BILATERAL_RECONSTRUCTION' or not summary.get('joint_continuity_pass'):
        raise ValueError('reconstruction is not applyable')
    ...
    if pathmm is None or used!=int(summary['candidate']['max_used_pelvis_mm']): raise ValueError('candidate path drift')
    ...
    for i,s in enumerate(samples[:CYCLE]):
        ...
        for q in (ru,rl,lu,ll):
            if not qfinite_unit(q): raise ValueError('non-unit apply quaternion')
    ...
    if len(frames)!=CYCLE: raise ValueError('incomplete application cycle')
    return {'schema':'grand-bruxelles-civ1-skeleton-applyability-v1','diagnostic_only':True,'runtime_authorized':False,'visual_approval_claimed':False,'source_reconstruction':summary,'frame_count':len(frames),'max_apply_quaternion_angle_rad':max_qangle,'frames':frames,'verdict':'AMELIORER_SKELETON_APPLYABILITY'}
