#!/usr/bin/env python3
"""Classify CIV-1 foot phase/rest-direction evidence without authorizing runtime changes.

A one-sided rest counterfactual can localize a cause, but cannot authorize a one-sided
production patch. Both feet must carry equivalent counterfactual evidence first.
"""
...
def assess(payload: dict) -> dict:
    ...
    for foot in SIDES:
        ...
        if cf is not None:
            ...
            if missing:
                ...
                if not bool(cf["target_foot_length_preserved"]):
                    failures.append(f"foot_length_not_preserved:{foot}")
                if not bool(cf["counterfactual_only"]):
                ...
                row["rest_direction_causality_supported"] = (
                    baseline_material
                    and bool(cf["normalization_improves_phase"])
                    and bool(cf["normalization_reaches_non_material_phase"])
                    and bool(cf["target_foot_length_preserved"])
                )
        feet[foot] = row
    ...
    return {
        "format": "grand-bruxelles-civ1-bilateral-rest-causality-v1",
