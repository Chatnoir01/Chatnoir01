...
def assess(data: dict) -> dict:
    ...
    if center != baseline_center or cycle != baseline_cycle:
        return result
    if cycle <= 0 or radius < MIN_RADIUS_SAMPLES or radius >= cycle // 2 or not 0 <= center < cycle:
        return result
    ...
    changed = []
    unchanged = 0
    active_angles_by_distance: dict[int, list[float]] = {}
    for i, sample in enumerate(samples):
        ...
        if active:
            active_angles_by_distance.setdefault(distance, []).append(float(angle))
    ...
    previous = None
    for distance in sorted(active_angles_by_distance):
        signed_levels = active_angles_by_distance[distance]
        if distance == 0:
            if len(signed_levels) != 1:
                return result
        else:
            if len(signed_levels) != 2 or abs(signed_levels[0] - signed_levels[1]) > SYMMETRY_EPSILON_RAD:
                return result
        level = max(abs(v) for v in signed_levels)
