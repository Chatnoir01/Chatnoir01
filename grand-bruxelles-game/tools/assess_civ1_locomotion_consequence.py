...
def assess(bilateral: dict, locomotion: dict, frame_bytes: bytes | None = None) -> dict:
    ...
    if not isinstance(capture, dict):
        ...
        if frame_bytes is None:
            ...
            if dimensions is None:
                failures.append("invalid_frame_png")
            else:
                if dimensions != (1280, 720):
                    failures.append("frame_receipt_not_1280x720")
                if capture.get("width") != dimensions[0] or capture.get("height") != dimensions[1]:
                    failures.append("frame_metadata_dimension_mismatch")
            if isinstance(frame_sha256, str) and SHA256_RE.fullmatch(frame_sha256):
                actual_sha256 = hashlib.sha256(frame_bytes).hexdigest()
                if actual_sha256.lower() != frame_sha256.lower():
                    failures.append("frame_sha256_mismatch")
                elif dimensions == (1280, 720):
                    frame_receipt_verified = True
    ...
    return {
        "format": "grand-bruxelles-civ1-locomotion-consequence-assessment-v2",
        "measurement_complete": complete,
        "frame_receipt_verified": frame_receipt_verified,
        "feet": rows,
