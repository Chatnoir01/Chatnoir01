...
DEFAULT_SUCCESS_MARKER = "CIV1_GLOBAL_CHAIN_DIAGNOSTIC_OK"
PATTERNS = [
...
def assess(text: str, success_marker: str = DEFAULT_SUCCESS_MARKER):
    if not success_marker:
        raise ValueError("success_marker must be non-empty")
    lines = []
    bones = set()
    ...
    return {
        "format": "grand-bruxelles-civ1-skin-bind-integrity-v5",
...
def main():
    ...
    ap.add_argument("--success-marker", default=DEFAULT_SUCCESS_MARKER)
    a = ap.parse_args()
    result = assess(Path(a.log).read_text(errors="replace"), a.success_marker)
