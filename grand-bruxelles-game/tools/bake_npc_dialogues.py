...
def bake_payload(source: dict[str, Any], source_bytes: bytes | None = None) -> dict[str, Any]:
    ...
    for raw_persona in personas:
        ...
        baked_personas.append({"id": persona_id, "zone": zone, "locale": locale, "intents": dict(sorted(baked_intents.items()))})
    ...
    baked_generator = {"mode": "offline_llm_bake", "version": "1", "source_sha256": source_sha256}
    for key in ("provider", "model", "run_id"):
        value = generator.get(key)
        if value is not None and str(value).strip():
            baked_generator[key] = str(value).strip()
    return {"schema": SCHEMA, "generator": baked_generator, "personas": sorted(baked_personas, key=lambda row: row["id"])}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print("NPC_DIALOGUE_BAKE_OK: personas=%d source_sha256=%s" % (len(baked["personas"]), baked["generator"]["source_sha256"]))
    return 0
