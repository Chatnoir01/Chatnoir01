...
The official bm_urbis:urbadm_ssw layer is the sidewalk dataset. `ssft` is
preserved and validated against the publisher domain, but is not used as a
local SW filter. Raw WFS bytes remain acquisition evidence; a canonical source
content digest ignores only the volatile top-level `timeStamp` and normalizes
feature order by exact WFS id. This tool never authorizes runtime geometry,
...
def build_wfs_url(query_bbox: list[float] | None = None) -> str:
    ...
    params = {
        ...
        "typeName": LAYER, "outputFormat": "json", "srsName": CRS,
...
def _parse_ssft_domain(raw: bytes) -> dict[str, dict[str, str]]:
    ...
    if domain.get(PUBLISHED_SIDEWALK_DOMAIN_CODE) != {"label_fr": "Trottoir", "label_nl": "Voetpad"}:
...
def canonicalize_feature_collection(raw: bytes, *, attribute_domain_raw: bytes, query_bbox: list[float] | None = None) -> dict[str, Any]:
    ...
    return {
        "schema": SCHEMA,
        "source": {
            "publisher": "Paradigm", "dataset": "Trottoir", "layer": LAYER,
            "license": "CC0-1.0", "crs": CRS,
            "sidewalk_semantics_basis": "official_dataset_and_layer_identity",
            "semantic_field": "ssft", "ssft_filter_applied": False,
            "published_sidewalk_domain_code": PUBLISHED_SIDEWALK_DOMAIN_CODE,
            "observed_ssft_values": sorted(ssft_counts),
...
def fetch_attribute_domain(timeout_seconds: int = 60) -> bytes:
    request = Request(ATTRIBUTE_DOMAIN_ENDPOINT, headers={"Accept": "application/json", "User-Agent": "Grand-Bruxelles-Source-Gate/1.0"})
    with urlopen(request, timeout=timeout_seconds) as response:
        content_type = str(response.headers.get("Content-Type", ""))
        raw = response.read()
...
def main() -> int:
    ...
    parser.add_argument("--attribute-domain", type=Path)
