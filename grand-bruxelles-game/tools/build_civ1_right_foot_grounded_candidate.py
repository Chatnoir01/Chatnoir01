...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: build_civ1_right_foot_grounded_candidate.py <input.gd> <output.gd>", file=sys.stderr)
        return 2
    src = Path(sys.argv[1])
    dst = Path(sys.argv[2])
    text = src.read_text(encoding="utf-8")
    for token in (OLD_LINE, FORMAT_OLD, METHOD_OLD, LENGTH_FIELD):
    ...
    print("CIV1_RIGHT_FOOT_GROUNDED_CANDIDATE_BUILT alpha=0.75")
    return 0
