...
def build_runtime_index(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    for raw_osm_id, raw_entry in entries.items():
        ...
        if not isinstance(raw_entry, dict) or osm_id <= 0 or osm_id in seen_road_ids:
    ...
    payload = {"authorization": dict(AUTHORIZATION), "catalog_sha256": catalog_sha, "documents": documents, "format": FORMAT, "source_lookup_only": True}
    validate_contract(payload, expected_catalog_sha256=catalog_sha)
    return payload


def canonical_catalog_sha256() -> str:
    """Re-derive the identity only through the lock-bound catalog factory."""
    catalog = _catalog_module.build_catalog(PROJECT_DIR / "data" / "osm")
    _catalog_module.validate_contract(catalog)
    return require_sha256(catalog.get("catalog_sha256"), "locked catalog_sha256")


def validate_contract(index: dict[str, Any], *, expected_catalog_sha256: str | None = None) -> None:
    ...
    catalog_sha = require_sha256(index.get("catalog_sha256"), "catalog_sha256")
    expected_sha = require_sha256(expected_catalog_sha256, "expected catalog_sha256") if expected_catalog_sha256 is not None else canonical_catalog_sha256()
    if catalog_sha != expected_sha:
        raise SystemExit(f"ROAD_RUNTIME_INDEX_FAIL: catalog identity drift expected={expected_sha} actual={catalog_sha}")
    ...
    seen_paths: set[str] = set(); seen_road_ids: set[int] = set(); previous_path: str | None = None
    for descriptor in documents:
        ...
        if source_path in seen_paths or (previous_path is not None and source_path <= previous_path):
            raise SystemExit("ROAD_RUNTIME_INDEX_FAIL: duplicate/unsorted source path")
        previous_path = source_path; seen_paths.add(source_path)
        if not isinstance(road_ids, list) or not road_ids or road_ids != sorted(road_ids):
            raise SystemExit(f"ROAD_RUNTIME_INDEX_FAIL: invalid road ids for {source_path!r}")
...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--source-root", type=Path, default=Path("data/osm")); parser.add_argument("--output", type=Path, required=True); args = parser.parse_args()
    catalog = _catalog_module.build_catalog(args.source_root); _catalog_module.validate_contract(catalog)
    index = build_runtime_index(catalog); write_index(index, args.output)
    road_count = sum(len(document["road_ids"]) for document in index["documents"])
    print(f"ROAD_RUNTIME_INDEX_OK: roads={road_count} documents={len(index['documents'])} catalog_sha256={index['catalog_sha256']}")
    return 0
