...
REGISTRY_ROOT_KEYS = frozenset(("schema", "scope", "evidence_baseline", "source", "game_frame", "municipalities", "authorization"))
MANIFEST_ROOT_KEYS = frozenset(("schema", "municipality", "source", "game_frame", "output", "authorization"))
...
def expected_manifest_output(municipality_id: str) -> dict[str, str]:
    return {
        "normalized_game_source": f"artifacts/{municipality_id}_road_source.game.json",
        "raw_overpass_snapshot": f"artifacts/{municipality_id}_road_source.raw.json",
        "receipt": f"artifacts/{municipality_id}_road_source.receipt.json",
    }
...
def reject_unexpected_keys(payload: dict[str, Any], allowed_keys: frozenset[str], label: str) -> None:
    unexpected = sorted(set(payload) - allowed_keys)
    if unexpected:
        raise SystemExit(f"MUNICIPALITY_ROAD_ACQUISITION_FAIL: unexpected {label} keys: {','.join(unexpected)}")


def reject_aliased_paths(named_paths: dict[str, Path], label: str) -> None:
    ...
    for name, path in named_paths.items():
        ...
        if previous is not None:
            raise SystemExit(
                "MUNICIPALITY_ROAD_ACQUISITION_FAIL: "
                f"{label} paths alias: {previous}={name}"
            )
        resolved[canonical] = name


def validate_acquisition_output_paths(manifest: dict[str, Any], raw_output: Path, game_output: Path, receipt_output: Path) -> None:
    output = manifest["output"]
    expected = {
        "raw-output": Path(output["raw_overpass_snapshot"]).expanduser().resolve(strict=False),
        "game-output": Path(output["normalized_game_source"]).expanduser().resolve(strict=False),
        "receipt-output": Path(output["receipt"]).expanduser().resolve(strict=False),
    }
    actual = {
        "raw-output": raw_output.expanduser().resolve(strict=False),
        "game-output": game_output.expanduser().resolve(strict=False),
        "receipt-output": receipt_output.expanduser().resolve(strict=False),
    }
    drift = [name for name in expected if actual[name] != expected[name]]
    if drift:
        raise SystemExit(
            "MUNICIPALITY_ROAD_ACQUISITION_FAIL: acquisition output contract drift: "
            + ",".join(drift)
        )
...
def read_registry(path: Path) -> dict[str, Any]:
    ...
    reject_unexpected_keys(payload, REGISTRY_ROOT_KEYS, "registry root")
...
def build_manifest_from_registry(registry: dict[str, Any], niscode: str, municipality_id: str) -> dict[str, Any]:
    ...
    return {"schema": SCHEMA,"municipality": matches[0],"source": dict(LOCKED_MANIFEST_SOURCE),"game_frame": dict(LOCKED_GAME_FRAME),"output": expected_manifest_output(municipality_id),"authorization": registry["authorization"]}


def read_manifest(path: Path) -> dict[str, Any]:
    payload = read_json_object(path, "manifest")
    reject_unexpected_keys(payload, MANIFEST_ROOT_KEYS, "manifest root")
    ...
    validate_exact_mapping(payload.get("output"), expected_manifest_output(str(municipality["id"])), "manifest output contract")
...
def validate_overpass_road_elements(raw: dict[str, Any]) -> None:
    ...
    for element in elements:
        ...
        if highway not in HIGHWAY_CLASSES:
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid OSM highway class")


def validate_wgs84_geometry(raw: dict[str, Any]) -> None:
    elements = raw.get("elements")
    if not isinstance(elements, list) or not elements:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid WGS84 road geometry")
    for element in elements:
...
def validate_normalized_road_identity(raw: dict[str, Any], converted: dict[str, Any]) -> None:
    raw_elements = raw.get("elements") or []
    raw_identity = {element["id"]: element["tags"]["highway"] for element in raw_elements}
    roads = converted.get("roads")
    if not isinstance(roads, list) or not roads:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: normalized road identity drift")
    normalized_identity: dict[int, str] = {}
    for road in roads:
        if not isinstance(road, dict):
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: normalized road identity drift")
        osm_id = road.get("osm_id")
        highway = road.get("class")
        if (
            isinstance(osm_id, bool)
            or not isinstance(osm_id, int)
            or osm_id <= 0
            or highway not in HIGHWAY_CLASSES
            or road.get("drivable") is not True
            or osm_id in normalized_identity
        ):
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: normalized road identity drift")
        normalized_identity[osm_id] = highway
    if normalized_identity != raw_identity:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: normalized road identity drift")


def validate_projected_road_geometry(converted: dict[str, Any]) -> None:
    roads = converted.get("roads")
    if not isinstance(roads, list) or not roads:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid projected road geometry")
    for road in roads:
        if not isinstance(road, dict):
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid projected road geometry")
        ...
        for point in points:
            ...
            if (
                ...
                raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid projected road geometry")
            distinct_positions.add((float(x), float(z)))
        if len(distinct_positions) < 2:
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid projected road geometry")


def validate_normalized_drivable_road_count(converted: dict[str, Any], roads: list[Any]) -> None:
    stats = converted.get("stats")
    count = stats.get("drivable_roads") if isinstance(stats, dict) else None
    if isinstance(count, bool) or not isinstance(count, int) or count <= 0:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid normalized drivable road count")
    if count != len(roads):
...
def build_outputs(manifest: dict[str, Any], raw: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    validate_normalized_road_identity(raw, converted)
    roads = converted.get("roads") or []
    if not roads:
        raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: source produced no drivable roads")
    validate_normalized_drivable_road_count(converted, roads)
    validate_projected_road_geometry(converted)
...
def write_manifest_from_registry(registry_path: Path, niscode: str, municipality_id: str, output: Path) -> None:
    reject_aliased_paths({"registry": registry_path, "manifest-output": output}, "registry/manifest")
...
def main() -> int:
    ...
    reject_aliased_paths(
        {
            "manifest": args.manifest,
            "raw-output": args.raw_output,
            "game-output": args.game_output,
            "receipt-output": args.receipt_output,
        },
        "acquisition",
    )
    manifest = read_manifest(args.manifest)
    validate_acquisition_output_paths(manifest, args.raw_output, args.game_output, args.receipt_output)
