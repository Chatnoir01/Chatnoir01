...
def read_registry(path: Path) -> dict[str, Any]:
    ...
    seen_nis: set[str] = set(); seen_ids: set[str] = set(); seen_relations: set[int] = set()
    for row in municipalities:
        if not isinstance(row, dict):
            raise SystemExit("MUNICIPALITY_ROAD_ACQUISITION_FAIL: invalid registry municipality row")
        nis = row.get("niscode"); municipality_id = row.get("id"); relation_id = row.get("osm_relation_id")
        ...
        seen_nis.add(nis); seen_ids.add(municipality_id); seen_relations.add(relation_id)
...
def build_manifest_from_registry(registry: dict[str, Any], niscode: str, municipality_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "source": {"provider": source["provider"], "license": source["license"], "endpoint": source["endpoint"], "query_scope": "administrative_relation", "highway_classes": list(HIGHWAY_CLASSES)},
...
def read_manifest(path: Path) -> dict[str, Any]:
    ...
    municipality = payload.get("municipality") or {}; relation_id = municipality.get("osm_relation_id")
    if not isinstance(relation_id, int) or relation_id <= 0:
...
def build_outputs(manifest: dict[str, Any], raw: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    frame = manifest["game_frame"]; origin = (float(frame["origin_lat"]), float(frame["origin_lon"]))
    converted = convert(raw, origin); roads = converted.get("roads") or []
...
def write_manifest_from_registry(registry_path: Path, niscode: str, municipality_id: str, output: Path) -> None:
    registry = read_registry(registry_path); manifest = build_manifest_from_registry(registry, niscode, municipality_id)
    output.parent.mkdir(parents=True, exist_ok=True); output.write_bytes(canonical_bytes(manifest) + b"\n")
...
def main() -> int:
    ...
    parser.add_argument("--registry", type=Path); parser.add_argument("--municipality-nis"); parser.add_argument("--municipality-id"); parser.add_argument("--manifest-output", type=Path)
    parser.add_argument("--manifest", type=Path); parser.add_argument("--raw-output", type=Path); parser.add_argument("--game-output", type=Path); parser.add_argument("--receipt-output", type=Path); parser.add_argument("--retries", type=int, default=4)
    ...
    if args.registry is not None:
        ...
        write_manifest_from_registry(args.registry, args.municipality_nis, args.municipality_id, args.manifest_output); return 0
    ...
    manifest = read_manifest(args.manifest); query = build_query(manifest); raw = fetch(str(manifest["source"]["endpoint"]), query, max(1, args.retries)); game_source, receipt = build_outputs(manifest, raw)
    for path in (args.raw_output, args.game_output, args.receipt_output): path.parent.mkdir(parents=True, exist_ok=True)
