#!/usr/bin/env python3
"""Measure pinned OSM road overlap with registered Lambert72 cells.

Evidence only. This script consumes the already-proven Grand Bruxelles project
frame (E=origin+x, N=origin-z), validates the pinned OSM source bytes and the
registered-cell manifests, then measures geometric overlap. It never authorizes
road->cell mapping, runtime mount, collision, safe spawn, or JOUABLE promotion.
"""
...
EXPECTED_ROAD_SHA256 = "a96123a6098c2a94dcef2622b6ea099c831f426e1ebfeb28a2edda74675c2493"
...
def segment_intersects_bbox(a: tuple[float, float], b: tuple[float, float], bbox: list[float]) -> bool:
    """Liang-Barsky line clipping against a closed axis-aligned rectangle."""
    x0, y0 = a
    x1, y1 = b
    dx = x1 - x0
    dy = y1 - y0
    ...
    for entry in entries:
        ...
        for flag in (
            ...
            if entry.get(flag) is not False:
                raise ValueError(f"registered cell {cell_id} must keep {flag}=false")
...
def measure(road_source: Path, cell_index: Path) -> dict[str, Any]:
    ...
    for road in roads:
        ...
        if matched:
            overlaps.append({
                ...
                "cells": sorted(matched, key=lambda v: v["cell_id"]),
            })

    overlaps.sort(key=lambda v: v["osm_id"])
    core = {
        "schema": "grand-bruxelles-road-registered-cell-overlap-measurement-v1",
...
def main() -> int:
    ...
    try:
        result = measure(args.road_source.resolve(), args.cell_index.resolve())
