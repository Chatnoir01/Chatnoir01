...
def osm_pair(kind: str = "tree") -> tuple[dict, dict]:
    tags = {"natural": "tree"} if kind == "tree" else {"highway": "street_lamp"}
    cache = {
        ...
        "counts": {"tree": int(kind == "tree"), "street_lamp": int(kind == "street_lamp"), "bollard": 0},
        "elements": [{"id": 7, "lat": 50.87, "lon": 4.31, "tags": tags}],
    }
    stats = {"tree": int(kind == "tree"), "street_lamp": int(kind == "street_lamp"), "bollard": 0, "total": 1}
    runtime = {
...
def main() -> int:
    ...
    assert cm.gate_spawn(zone, profile, manifest)["status"] == "PASS"
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        runtime["source_digest"] = "0" * 64; write_json(root / "runtime.json", runtime)
