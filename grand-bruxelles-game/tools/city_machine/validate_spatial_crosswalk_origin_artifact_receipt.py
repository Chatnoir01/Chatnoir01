...
def _road_ids_semantic_sha256(road_ids: list[int]) -> str:
...
def _require_runtime_index_bytes(runtime_index_raw: bytes, source_road_ids: set[int]) -> None:
    ...
    if _road_ids_semantic_sha256(road_ids) != EXPECTED_ROAD_IDS_SHA256:
        raise ValueError("origin evidence road runtime identity-set semantic digest drift")
