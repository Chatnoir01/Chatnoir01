...
# Locked production/C01 world transform.
...
def _clip_segment(
    a: list[float], b: list[float], bbox: tuple[float, float, float, float]
) -> tuple[list[float], list[float]] | None:
    """Liang-Barsky clip against an axis-aligned cell bbox."""
    x0, y0 = a
    x1, y1 = b
    dx, dy = x1 - x0, y1 - y0
    min_x, min_y, max_x, max_y = bbox
...
def build(cell_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    street_axes, street_types = _network_rows(
        documents["street_axes"], bbox, default_kind="street", dedupe=street_dedupe
    )
    rail_dedupe: set[tuple[Any, ...]] = set()
    tram_from_tram, tram_types = _network_rows(
        documents["tram_network"], bbox, default_kind="rail", dedupe=rail_dedupe
    )
    rail_from_train, train_types = _network_rows(
        documents["train_network"], bbox, default_kind="rail", dedupe=rail_dedupe
    )
    ...
    input_files = {
        ...
        **{
            relative: _file_sha256(cell_dir / relative)
            for _, relative in REQUIRED_LAYERS
        },
    }
    output_files = {
