...
def build(
    ...
    result = {
        ...
        "next_action": (
            "explicit_validated_promotion_required"
            if all_passed
            else "collect_measured_runtime_gate_evidence"
        ),
        "policy": {
...
def main() -> int:
    ...
    print(
        "TERRAIN_RUNTIME_READINESS_OK "
        f"cell={result['cell_id']} passed={result['passed_runtime_gate_count']}/{result['runtime_gate_count']} "
        f"promotion_ready={str(result['promotion_ready_for_explicit_review']).lower()} runtime_promotion=false"
    )
    return 0
