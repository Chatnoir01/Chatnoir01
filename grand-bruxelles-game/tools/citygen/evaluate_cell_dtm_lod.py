...
def bilinear_sample(array: Any, transform: Any, xs: Any, ys: Any) -> Any:
    ...
    c0 = np.floor(cols).astype(np.int64); r0 = np.floor(rows).astype(np.int64)
    dc = cols - c0; dr = rows - r0
    out = np.full(xs.shape, np.nan, dtype=np.float64)
    valid_bounds = (r0 >= 0) & (c0 >= 0) & (r0 + 1 < array.shape[0]) & (c0 + 1 < array.shape[1])
    if not np.any(valid_bounds): return out
    idx = np.where(valid_bounds)[0]; rr, cc = r0[idx], c0[idx]
    vals = np.stack([array[rr,cc],array[rr,cc+1],array[rr+1,cc],array[rr+1,cc+1]],axis=1).astype(np.float64)
    weights = np.stack([(1-dr[idx])*(1-dc[idx]),(1-dr[idx])*dc[idx],dr[idx]*(1-dc[idx]),dr[idx]*dc[idx]],axis=1)
    finite = np.isfinite(vals); weighted = np.where(finite, vals*weights, 0.0); denom = np.where(finite, weights, 0.0).sum(axis=1)
    good = denom > 0; sampled = np.full(idx.shape, np.nan, dtype=np.float64); sampled[good] = weighted[good].sum(axis=1)/denom[good]
    out[idx] = sampled
    return out
...
def _source_pixels_in_bbox(array: Any, transform: Any, bbox: tuple[float,float,float,float]) -> tuple[Any,Any,Any]:
    ...
    cols = np.where((xs>=west)&(xs<east))[0]; rows = np.where((ys>=south)&(ys<north))[0]
    if rows.size == 0 or cols.size == 0: raise ValueError(f"no source DTM pixels intersect cell bbox {bbox}")
    rr,cc = np.meshgrid(rows,cols,indexing="ij")
    x = transform.c + (cc.reshape(-1)+0.5)*transform.a; y = transform.f + (rr.reshape(-1)+0.5)*transform.e
    return x,y,array[rr,cc].reshape(-1).astype(np.float64)
...
def _make_coarse_grid(array: Any, source_transform: Any, bbox: tuple[float,float,float,float], resolution_m: float) -> tuple[Any,Any]:
    ...
    west,south,east,north = bbox
    width = int(math.ceil((east-west)/resolution_m)); height = int(math.ceil((north-south)/resolution_m))
    if width <= 1 or height <= 1: raise ValueError("terrain LOD grid is too small")
    transform = Affine(resolution_m,0.0,west,0.0,-resolution_m,north)
    rows=np.arange(height); cols=np.arange(width); rr,cc=np.meshgrid(rows,cols,indexing="ij")
    xs=transform.c+(cc.reshape(-1)+0.5)*transform.a; ys=transform.f+(rr.reshape(-1)+0.5)*transform.e
...
def _error_metrics(source: Any, reconstructed: Any) -> dict[str,Any]:
    import numpy as np
    paired=np.isfinite(source)&np.isfinite(reconstructed); count=int(np.count_nonzero(paired))
    if count==0: raise ValueError("no paired terrain samples available for LOD error measurement")
    error=reconstructed[paired]-source[paired]; abs_error=np.abs(error)
    return {"paired_samples":count,"source_min_m":round(float(np.min(source[paired])),5),"source_max_m":round(float(np.max(source[paired])),5),"mean_bias_m":round(float(np.mean(error)),6),"rmse_m":round(float(np.sqrt(np.mean(error*error))),6),"p50_abs_error_m":round(float(np.percentile(abs_error,50)),6),"p95_abs_error_m":round(float(np.percentile(abs_error,95)),6),"max_abs_error_m":round(float(np.max(abs_error)),6)}
...
def evaluate_array(array: Any, transform: Any, bbox: tuple[float,float,float,float], resolutions_m: Iterable[float]=DEFAULT_RESOLUTIONS_M) -> dict[str,Any]:
    ...
    if not math.isclose(source_pixel_size,abs(float(transform.e)),rel_tol=0.0,abs_tol=1e-9): raise ValueError("DTM source pixels must be square")
    xs,ys,source=_source_pixels_in_bbox(array,transform,bbox); finite_source=source[np.isfinite(source)]
    if finite_source.size==0 or float(np.max(finite_source)-np.min(finite_source))<0.01: raise ValueError("DTM source inside cell is empty or degenerate")
    levels=[]; source_sample_count=int(finite_source.size)
    for raw in resolutions_m:
...
def build(raster_validation_path: Path, value_evidence_path: Path, extract_root: Path, resolutions_m: Iterable[float]=DEFAULT_RESOLUTIONS_M, p95_threshold_m: float=DEFAULT_P95_THRESHOLD_M) -> dict[str,Any]:
    validation=_read(raster_validation_path); values=_read(value_evidence_path)
    if validation.get("format")!=RASTER_FORMAT or validation.get("crs")!=CRS: raise ValueError("unsupported elevation raster validation")
    if values.get("format")!=VALUE_FORMAT or values.get("crs")!=CRS: raise ValueError("unsupported elevation value evidence")
    cell_id=validation.get("cell_id")
    if not isinstance(cell_id,str) or values.get("cell_id")!=cell_id: raise ValueError("terrain LOD evidence cell identity mismatch")
    if values.get("terrain_source_evidence_ready") is not True: raise ValueError("terrain source value evidence is not ready")
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("--raster-validation",type=Path,required=True); parser.add_argument("--value-evidence",type=Path,required=True); parser.add_argument("--extract-root",type=Path,required=True); parser.add_argument("--output",type=Path,required=True); parser.add_argument("--resolutions",type=float,nargs="*",default=list(DEFAULT_RESOLUTIONS_M)); parser.add_argument("--p95-threshold",type=float,default=DEFAULT_P95_THRESHOLD_M); args=parser.parse_args()
    result=build(args.raster_validation,args.value_evidence,args.extract_root,tuple(args.resolutions),args.p95_threshold); args.output.parent.mkdir(parents=True,exist_ok=True); args.output.write_text(json.dumps(result,ensure_ascii=False,indent=2,sort_keys=True)+"\n",encoding="utf-8"); print("CELL_DTM_LOD_OK",result["cell_id"],result["selection"]["selected_resolution_m"],result["evidence_digest"])
