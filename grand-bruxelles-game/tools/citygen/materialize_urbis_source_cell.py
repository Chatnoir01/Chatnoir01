...
This is source ingestion only: it writes the authoritative building extract and a
provenance manifest, never Godot/runtime geometry. Buildings crossing cell borders
are assigned to exactly one globally aligned 500 m cell by polygon centroid so a
regional pass cannot duplicate INSPIRE_ID ownership across adjacent cells.
"""
...
def _centroid(feature: dict[str,Any]) -> tuple[float,float] | None:
    ...
    if not coords or not isinstance(coords[0],list): return None
    raw=coords[0]
    points=[]
    for p in raw:
...
def owner_cell(feature: dict[str,Any]) -> str | None:
    ...
    e=math.floor(center[0]/CELL_SIZE)*CELL_SIZE
    n=math.floor(center[1]/CELL_SIZE)*CELL_SIZE
    return canonical_cell_id(e,n,CELL_SIZE)
...
def request_buildings(bbox: tuple[float,float,float,float], retries: int = 4) -> dict[str,Any]:
    ...
    request=urllib.request.Request(WFS_URL+"?"+urllib.parse.urlencode(params),headers={"User-Agent":USER_AGENT,"Accept":"application/geo+json, application/json"})
    last=None
    for attempt in range(1,max(1,retries)+1):
        try:
            ...
            if payload.get("type") != "FeatureCollection":
                raise RuntimeError("unexpected UrbIS Buildings WFS payload")
            return payload
...
def materialize(
    ...
    output_dir: Path,
    fetcher: Callable[[tuple[float,float,float,float]],dict[str,Any]],
) -> dict[str,Any]:
    _validate(cell_id,bbox)
    document=fetcher(bbox)
    ...
    for feature in document.get("features") or []:
        ...
        if owner is None:
            # Preserve malformed authoritative features so downstream QA sees and quarantines them.
            kept.append(feature); invalid_ownership += 1
        elif owner == cell_id:
            kept.append(feature)
        else:
            ownership_filtered += 1
    kept.sort(key=lambda f: str((f.get("properties") or {}).get("INSPIRE_ID") or f.get("id") or digest(f)))
    source={k:v for k,v in document.items() if k != "features"}
    source["type"]="FeatureCollection"; source["features"]=kept; source["numberReturned"]=len(kept)
    source["grand_bruxelles_source"]={"authority":"Paradigm / Brussels-Capital Region","service":WFS_URL,"layer":LAYER,"crs":CRS,"bbox":list(bbox),"cell_id":cell_id,"ownership":"polygon_centroid_global_500m_cell"}
    raw_path=output_dir/"raw"/"buildings.geojson"
    _write(raw_path,source,True)
    manifest={"format":"grand-bruxelles-urbis-source-cell-v1","cell_id":cell_id,"crs":CRS,"bbox":list(bbox),"layers":{"buildings":{"wfs_name":LAYER,"features":len(kept),"ownership_filtered":ownership_filtered,"invalid_ownership_features":invalid_ownership,"file":"raw/buildings.geojson"}},"promotion":"source_only_no_runtime_mutation"}
...
def main() -> int:
    ...
    args=ap.parse_args()
    manifest=materialize(args.cell_id,args.bbox,args.output_dir,lambda bbox:request_buildings(bbox,args.retries))
    print(f"MATERIALIZE_URBIS_SOURCE_CELL_OK cell={args.cell_id} features={manifest['layers']['buildings']['features']} filtered={manifest['layers']['buildings']['ownership_filtered']} digest={manifest['source_digest']}")
    return 0
