...
def write_source(root: Path) -> Path:
    ...
    docs = {
        ...
        "street_surfaces": collection(LAYERS["street_surfaces"][0], [feature("surface-crossing-west", polygon([[148950,169080],[149080,169080],[149080,169120],[148950,169120]]), AREA=5200, TYPE="S", LVL=0, STRNAMEFRE="Rue Test", STRNAMEDUT="Teststraat")]),
        "street_axes": collection(LAYERS["street_axes"][0], [feature("street-crossing-west", line([[148900,169100],[149100,169100]]), TYPE="S")]),
        "tram_network": collection(LAYERS["tram_network"][0], [feature("rail-tw", line([[149050,169150],[149450,169150]]), TYPE="TW")]),
    ...
    for logical_name, (wfs_name, relative) in LAYERS.items():
        ...
        manifest_layers[logical_name] = {"wfs_name": wfs_name, "features": len(docs[logical_name]["features"]), "file": relative}
    manifest = {
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    assert building["footprint"][0] == mod._world_point(149020.0, 169020.0)
...
print("BUILD_RUNTIME_CANDIDATE_BUNDLE_OK deterministic=true layers=5 clipping=true no_fake_heights=true collision=false runtime_mount=false rail_dedupe=true fail_closed=true")
