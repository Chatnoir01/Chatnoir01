...
assert "CITYGEN_RUNTIME_FRONTIER_DISPATCH_OK durable_progress=true limit=32" in source_repair_workflow, "source repair must expose its immediate runtime frontier contract"
...
assert "needs.repair-frontier.outputs.durable_progress == 'true'" in source_repair_workflow, "all follow-up dispatches must require durable source-repair progress"
assert "CITYGEN_SOURCE_REPAIR_CHAIN_OK durable_progress=true next=runtime_frontier+autonomous_citygen batch=32" in source_repair_workflow, "source repair must expose its full follow-up chain contract"
...
print("REGIONAL_ACCELERATION_CONTRACT_OK municipalities=19 batch=32 cadence=2h immediate_push=true sequential_source_repair=true runtime_frontier_immediate=true promotion_bypass=false")
