...
def write_source(root: Path) -> Path:
    ...
    docs = {
        ...
        "street_surfaces": collection(
            ...
            [feature("surface-1", polygon([[149010,169080],[149100,169080],[149100,169120],[149010,169120]]), AREA=3600, TYPE="S", LVL=0)],
        ),
        "street_axes": collection(
            LAYERS["street_axes"][0],
            [feature("street-1", line([[149010,169100],[149200,169100]]), TYPE="S")],
        ),
        "tram_network": collection(
            LAYERS["tram_network"][0],
            [feature("tram-1", line([[149020,169150],[149220,169150]]), TYPE="TW")],
        ),
        "train_network": collection(
            LAYERS["train_network"][0],
            [feature("train-1", line([[149020,169180],[149220,169180]]), TYPE="RW")],
        ),
    }
    manifest_layers = {}
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # A sealed candidate cannot be silently sealed/promoted a second time as if it
    # were a fresh production-shaped compiler output.
    try:
    ...
    # Any forbidden authorization must block sealing before a candidate can persist.
    unsafe = root / "unsafe"
