...
SCHEMA = "grand-bruxelles-civ1-roster-registration-truth-v24"
...
def _path_percent_encoding_is_canonical(path):
    i=0
    while i<len(path):
        if path[i]!='%':
            i+=1; continue
        ...
        pair=path[i+1:i+3]
        if any(ch not in HEX for ch in pair): return False
        decoded=chr(int(pair,16))
        if decoded in UNRESERVED: return False
        i+=3
    return True

def _path_has_dot_segments(path):
    return any(segment in {'.','..'} for segment in path.split('/'))

def _raw_host_and_port(hostport):
    if hostport.startswith('['):
        close=hostport.find(']')
        raw_host=hostport[1:close] if close>=0 else hostport
        suffix=hostport[close+1:] if close>=0 else ''
        raw_port=suffix[1:] if suffix.startswith(':') else None
        return raw_host,raw_port
    if ':' in hostport:
        raw_host,raw_port=hostport.rsplit(':',1)
        return raw_host,raw_port
...
def _source_url_reasons(value):
    ...
    if not parsed.path: reasons.append('source_url_not_canonical')
    if not _path_percent_encoding_is_canonical(parsed.path) or _path_has_dot_segments(parsed.path): reasons.append('source_url_not_canonical')
    parsed_host=parsed.hostname or ''
    hostport=parsed.netloc.rsplit('@',1)[-1]
    raw_host,raw_port=_raw_host_and_port(hostport)
    ...
    if raw_port is not None and (parsed_port is None or raw_port!=str(parsed_port)): reasons.append('source_url_not_canonical')
    if parsed.scheme.lower()=='https' and parsed_port==443 and raw_port=='443': reasons.append('source_url_not_canonical')
