...
# v47 remains a deliberately thin policy layer over the previously qualified
...
def _percent_encoded_component_has_noncharacter(component: str) -> bool:
    ...
    while i < len(component):
        ...
        while i < len(component) and component[i] == "%":
            ...
            i += 3
        try:
            decoded = bytes(octets).decode("utf-8", "strict")
...
# v38 source validation resolves this global at call time; patching only the
# policy hook keeps the qualified parser/structural core immutable.
