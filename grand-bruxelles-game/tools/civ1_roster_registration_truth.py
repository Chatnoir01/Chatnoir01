...
# v48 remains a deliberately thin policy layer over the previously qualified
# v38 parser/structural validator. Keeping the v38 core immutable makes each
...
_base_canonical_path = _v38._canonical_path
...
def _percent_encoded_utf8_runs(component: str):
    """Yield percent-encoded byte runs; malformed escapes are represented by None."""
    i = 0
    while i < len(component):
        if component[i] != "%":
            ...
            continue
        octets = bytearray()
        while i < len(component) and component[i] == "%":
            if i + 2 >= len(component):
                yield None
                return
            try:
                octets.append(int(component[i + 1:i + 3], 16))
            except ValueError:
                yield None
                return
            i += 3
...
def _percent_encoded_component_has_noncharacter(component: str) -> bool:
    """Inspect percent-encoded UTF-8 runs without changing canonical URL identity."""
    for octets in _percent_encoded_utf8_runs(component):
        if octets is None:
            continue
        try:
            decoded = octets.decode("utf-8", "strict")
        except UnicodeDecodeError:
            continue
        if any(_is_unicode_noncharacter(char) for char in decoded):
            return True
    return False


def _percent_encoded_component_has_malformed_utf8(component: str) -> bool:
    """Reject malformed percent escapes and byte runs that are not strict UTF-8."""
    for octets in _percent_encoded_utf8_runs(component):
        if octets is None:
            return True
        try:
...
def _canonical_path_without_noncharacters(path: str) -> bool:
    if not _base_canonical_path(path):
        return False
    return not _percent_encoded_component_has_noncharacter(path)


_v38._canonical_path = _canonical_path_without_noncharacters
...
def _uses_ipv6_transition(value: str) -> bool:
    address = _parsed_ip(value)
    return isinstance(address, ipaddress.IPv6Address) and (address.sixtofour is not None or address.teredo is not None)


def _uses_ipv6_nat64(value: str) -> bool:
    address = _parsed_ip(value)
    return isinstance(address, ipaddress.IPv6Address) and any(address in prefix for prefix in NAT64_TRANSLATION_PREFIXES)
...
def _source_reasons(value: str) -> list[str]:
    ...
    if isinstance(value, str) and value.strip():
        if is_local_network_source(value): reasons.append("source_url_local_network_forbidden")
        if _uses_ipv4_compatible_ipv6(value): reasons.append("source_url_ipv4_compatible_ipv6_forbidden")
        if _uses_ipv4_mapped_ipv6(value): reasons.append("source_url_ipv4_mapped_ipv6_forbidden")
        if _uses_ipv6_scope(value): reasons.append("source_url_ipv6_scope_forbidden")
        if _uses_ipv6_transition(value): reasons.append("source_url_ipv6_transition_forbidden")
        if _uses_ipv6_nat64(value): reasons.append("source_url_ipv6_nat64_forbidden")
        if _uses_special_use_namespace(value): reasons.append("source_url_special_use_namespace_forbidden")
        if _uses_private_use_namespace(value): reasons.append("source_url_private_use_namespace_forbidden")
        if _uses_dns_infrastructure_namespace(value): reasons.append("source_url_dns_infrastructure_namespace_forbidden")
        if _uses_ambiguous_dotted_numeric_host(value): reasons.append("source_url_ambiguous_dotted_numeric_host_forbidden")
        try:
            parsed = urlsplit(value)
        except ValueError:
            parsed = None
        if parsed is not None and (
            _percent_encoded_component_has_noncharacter(parsed.query)
            or _percent_encoded_component_has_noncharacter(parsed.fragment)
            or _percent_encoded_component_has_malformed_utf8(parsed.query)
            or _percent_encoded_component_has_malformed_utf8(parsed.fragment)
        ):
...
def build_payload(registry, repo_root):
    ...
    payload["source_url_unicode_noncharacter_forbidden"] = True
    payload["source_url_malformed_utf8_forbidden"] = True
    return payload
