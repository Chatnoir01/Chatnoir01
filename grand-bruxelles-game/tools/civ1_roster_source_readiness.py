...
CIV1_ROOT=PurePosixPath("grand-bruxelles-game/assets/characters/civilians/civ1")
...
REQUIRED_READY_FLAGS=("production_authorized","activation_ready","source_package_present","runtime_package_present")
EXPECTED_CHARACTER_REPOSITORY="https://github.com/ibrews/VitruvianGodot"
EXPECTED_CHARACTER_COMMIT="bdecdcd537b4031fdd0fb299b7e4f93f084fffa0"
EXPECTED_CHARACTER_LICENSE_CLAIM="MIXED_BY_COMPONENT"
...
def _status_consistent(status,repo_root):
    ...
    if "blocker" not in status or status["blocker"] is not None: return False
    character_source=status.get("character_source")
    if not isinstance(character_source,dict): return False
    if character_source.get("repository") != EXPECTED_CHARACTER_REPOSITORY: return False
    if character_source.get("commit") != EXPECTED_CHARACTER_COMMIT: return False
    if character_source.get("license_claim") != EXPECTED_CHARACTER_LICENSE_CLAIM: return False
...
def _canonical_asset_path(value):
    ...
    return canonical
def _is_civ1_asset_path(asset_path):
    parts=PurePosixPath(asset_path).parts
    root_parts=CIV1_ROOT.parts
    return len(parts)>len(root_parts) and tuple(part.casefold() for part in parts[:len(root_parts)])==tuple(part.casefold() for part in root_parts)
...
def blocking_entries(registry,repo_root):
    ...
    return [e["asset_path"] for e in registry["entries"] if _is_civ1_asset_path(e["asset_path"])]
def main():
