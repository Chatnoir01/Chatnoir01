...
def _parse_finite_float(token):
    ...
    return value
def _load_strict_json(path): return json.loads(path.read_text(encoding="utf-8"),object_pairs_hook=_reject_duplicate_keys,parse_float=_parse_finite_float,parse_constant=_reject_nonstandard_constant)
...
def _canonical_path_text(value):
    ...
    if any(unicodedata.category(c) in ("Cc","Cf","Cs","Zs","Zl","Zp") for c in value): return None
...
def _source_manifest_integrity(status,repo_root):
    ...
    if not isinstance(source_paths,list) or not source_paths: return False
    ...
    if not isinstance(manifest,dict) or not manifest or set(source_paths)!=set(manifest): return False
    ...
    for source_path in source_paths:
        ...
        if candidate is None or not candidate.is_file(): return False
        try: data=candidate.read_bytes()
        except OSError: return False
        if len(data)!=expected_size or _git_blob_sha1(data)!=expected_sha1: return False
    return True
...
def source_ready(repo_root):
    ...
    try: status=_load_strict_json(repo_root/STATUS_PATH)
    except (OSError,json.JSONDecodeError,DuplicateJSONKeyError,NonStandardJSONConstantError): return False
