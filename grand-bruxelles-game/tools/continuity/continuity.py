...
def next_lot(zone_id: str, report_dir: Path | None) -> dict[str, Any]:
    ...
    if reports:
        oldest = reports[0]
        return {"zone_id":zone_id,"decision":"fix_oldest_open_report","report_id":str(oldest.get("id","")),"captured_unix":int(oldest.get("captured_unix",0)),"note":str(oldest.get("note","")),"source_path":str(oldest.get("_source_path","")),"maturity":row["maturity"],"target_maturity":row["target_maturity"]}
