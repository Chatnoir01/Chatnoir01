...
def test_artifact_gate(tmp: Path) -> None:
    artifact = tmp / "artifact"; artifact.mkdir()
    payload = artifact / "payload.geojson"
    payload.write_text('{"type":"FeatureCollection","features":[]}\n', encoding="utf-8")
    digest = hashlib.sha256(payload.read_bytes()).hexdigest()
    manifest = artifact / "manifest.json"
    manifest.write_text(json.dumps({"license":"CC0","crs":"EPSG:31370","runtime_authorized":False,"production_authorized":False,"results":[{"file":"payload.geojson","bytes":payload.stat().st_size,"sha256":digest}]}), encoding="utf-8")
    ...
    assert parsed["result"] == "ARTIFACT_VERIFIED" and parsed["verified_file_count"] == 1
    assert parsed["reuse_terms_resolved"] is True and parsed["production_gate"] == "TERMS_OK"

    unresolved = artifact / "manifest-unresolved.json"
    unresolved.write_text(json.dumps({"license":"REUSE_TERMS_UNRESOLVED_PER_LAYER","runtime_authorized":False,"production_authorized":False,"results":[{"file":"payload.geojson","bytes":payload.stat().st_size,"sha256":digest}]}), encoding="utf-8")
    ...
    assert parsed_unresolved["reuse_terms_resolved"] is False and parsed_unresolved["production_gate"] == "TERMS_BLOCKED"

    payload.write_text("tampered", encoding="utf-8")
    result = subprocess.run([sys.executable,str(TOOLS/"verify_intake_artifact.py"),"--manifest",str(manifest),"--root",str(artifact),"--require-crs"],cwd=ROOT,text=True,capture_output=True)
    assert result.returncode != 0


def test_urbis_addresses(tmp: Path) -> None:
    numbers=tmp/"AddressNumbers.geojson"; addresses=tmp/"Addresses.geojson"
    numbers.write_text(json.dumps({"type":"FeatureCollection","features":[{"type":"Feature","id":"fallback-id","geometry":{"type":"Point","coordinates":[147900.0,169600.0]},"properties":{"INSPIRE_ID":"addr-1","STRNAMEFRE":"Rue Exemple","STRNAMEDUT":"Voorbeeldstraat","POLICENUM":"12A","MUNNAMEFRE":"Bruxelles","MUNNAMEDUT":"Brussel","POSTCODE":"1000"}}]}),encoding="utf-8")
    ...
    parsed=json.loads(output.read_text(encoding="utf-8")); record=parsed["records"][0]
    assert record["source_id"]=="addr-1" and record["house_number"]=="12A" and record["game_xz"]==[31.706,-61.376]
    assert parsed["runtime_authorized"] is False


def test_stib_open_data(tmp: Path) -> None:
    stops=tmp/"gtfs-stops-production.csv"; stops.write_text("stop_id,stop_name,stop_lat,stop_lon\nS1,Gare du Midi,50.836,4.336\n",encoding="utf-8")
    network=tmp/"shapefiles-production.geojson"; network.write_text(json.dumps({"type":"FeatureCollection","features":[{"type":"Feature","id":"line-3","geometry":{"type":"LineString","coordinates":[[4.336,50.836],[4.337,50.837]]},"properties":{"source_label":"published-commercial-route"}},{"type":"Feature","id":"stop-point","geometry":{"type":"Point","coordinates":[4.336,50.836]},"properties":{"source_label":"published-stop-position"}}]}),encoding="utf-8")
    ...
    assert parsed["stats"]["normalized_stop_count"]==1 and parsed["stats"]["normalized_network_feature_count"]==2
    assert "routes" not in parsed and "trips" not in parsed and parsed["runtime_authorized"] is False
...
def test_stib_gtfs(tmp: Path) -> None:
    ...
    parsed=json.loads(output.read_text(encoding="utf-8"))
    assert parsed["stats"]=={"stop_count":1,"route_count":1,"trip_count":1,"shape_count":1} and parsed["routes"][0]["colour"]=="FFCC00"


def test_planning_permits(tmp: Path) -> None:
    source=tmp/"permits.json"; source.write_text(json.dumps([{"numero_dossier":"PU-1","adresse":"Rue Exemple 12, 1000 Bruxelles","date_decision":"2026-07-01","objet":"transformation facade"}]),encoding="utf-8")
    ...
    sig=json.loads(output.read_text(encoding="utf-8"))["signals"][0]
    assert sig["permit_id"]=="PU-1" and sig["exact_building_crosswalk"] is False and sig["requires_review"] is True
...
def write_minimal_xlsx(path: Path) -> None:
    ...
    with zipfile.ZipFile(path,"w",compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("xl/workbook.xml",workbook); zf.writestr("xl/_rels/workbook.xml.rels",rels); zf.writestr("xl/worksheets/sheet1.xml",sheet)


def test_statistical_xlsx(tmp: Path) -> None:
    source=tmp/"periods.xlsx"; write_minimal_xlsx(source); output=tmp/"periods-evidence.json"
    run(str(TOOLS/"extract_statistical_xlsx.py"),"--xlsx",str(source),"--output",str(output),"--publisher","Statbel","--license","CC BY 4.0")
    parsed=json.loads(output.read_text(encoding="utf-8"))
    assert parsed["stats"]=={"file_count":1,"sheet_count":1} and parsed["source"]["files"][0]["sheets"][0]["name"]=="Periods"
...
def test_context_archive(tmp: Path) -> None:
    ...
    with zipfile.ZipFile(source,"w",compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("data/context.tif",b"synthetic-raster-placeholder"); zf.writestr("data/readme.txt",b"metadata")
    ...
    parsed=json.loads(output.read_text(encoding="utf-8"))
    assert parsed["stats"]["archive_count"]==1 and parsed["stats"]["geospatial_member_count"]==1
    unsafe=tmp/"unsafe-context.zip"
    with zipfile.ZipFile(unsafe,"w") as zf: zf.writestr("../escape.tif",b"bad")
...
def test_flood_context(tmp: Path) -> None:
    layer=tmp/"flood.geojson"
    layer.write_text(json.dumps({"type":"FeatureCollection","features":[{"type":"Feature","id":"hazard-1","geometry":{"type":"Polygon","coordinates":[[[147000,169000],[147010,169000],[147010,169010],[147000,169010],[147000,169000]]]},"properties":{"return_period":"TR100","cause":"pluvial"}}]}),encoding="utf-8")
    output=tmp/"flood-context.json"
    run(str(TOOLS/"normalize_flood_hazard_context.py"),"--layer",str(layer),"--output",str(output),"--license","CC BY + specific flood terms","--crs","EPSG:31370")
    parsed=json.loads(output.read_text(encoding="utf-8"))
    assert parsed["stats"]=={"layer_count":1,"feature_count":1}
    assert parsed["source"]["semantic_status"]=="indicative modeled hazard; no regulatory value"
    assert parsed["runtime_authorized"] is False and parsed["production_authorized"] is False
...
def test_queue_contract() -> None:
    ...
    assert families["flood_hazard_context"]["processor"].endswith("normalize_flood_hazard_context.py")
    assert families["flood_hazard_context"]["state"]=="PROCESSOR_READY_ARTIFACT_BLOCKED"
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        tmp=Path(td)
        test_artifact_gate(tmp); test_urbis_addresses(tmp); test_stib_open_data(tmp); test_stib_gtfs(tmp)
        test_planning_permits(tmp); test_statistical_xlsx(tmp); test_context_archive(tmp); test_flood_context(tmp)
    test_queue_contract()
