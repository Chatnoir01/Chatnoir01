...
const ALIASES := {
    "Hips": ["hips", "pelvis"], "LeftUpperLeg": ["leftupperleg", "leftupleg", "lupperleg"],
    "LeftLowerLeg": ["leftlowerleg", "leftleg", "llowerleg"], "LeftFoot": ["leftfoot", "lfoot"],
...
func _init() -> void:
    ...
    if args.size() != 3:
        push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: expected bundle report capture_dir"); quit(2); return
...
func _normalize(value: String) -> String:
    ...
    for token in [":", "/", ".", "-", "_", " "]: n = n.replace(token, "")
...
func _read_json(path: String) -> Variant:
    ...
    var parsed: Variant = JSON.parse_string(f.get_as_text()); f.close(); return parsed
...
func _support_band_path(frames: Array, semantic: String) -> Dictionary:
    ...
    var lo: float = float(ys.min()); var hi: float = float(ys.max())
    var threshold: float = lo + (hi - lo) * 0.10
    var path := 0.0; var count := 0; var previous := Vector3.ZERO; var have_previous := false
    for f in frames:
...
func _apply_frame(skeleton: Skeleton3D, mapping: Dictionary, frame: Dictionary) -> float:
    var max_error := 0.0; var poses: Dictionary = frame.get("poses", {})
    ...
    for semantic in POSE_BONES:
        var wanted := _pose(poses[semantic]); var actual := skeleton.get_bone_global_pose(int(mapping[semantic]))
...
func _head_expected_world(skeleton: Skeleton3D, head_idx: int) -> Transform3D:
    return skeleton.global_transform * skeleton.get_bone_global_pose(head_idx)
...
func _run() -> void:
    ...
    if frames.size() != 120: push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: frame count"); quit(5); return
    var body_scene := load(BODY_PATH) as PackedScene; var head_scene := load(HEAD_PATH) as PackedScene
    if body_scene == null or head_scene == null: push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: imported assets"); quit(6); return
    ...
    if skeleton == null: push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: skeleton"); quit(7); return
    var mapping := {}
    for semantic in POSE_BONES:
        var idx := _bone_index(skeleton, semantic)
        if idx < 0: push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: missing " + semantic); quit(8); return
        mapping[semantic] = idx
    var head_idx := skeleton.find_bone(HEAD_BONE)
    if head_idx < 0: push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: head bone"); quit(9); return
    ...
    var head_attachment_calibration_offset_m := head_follow_calibration.origin.length(); var max_head_follow_error := 0.0
    var pelvis_min := INF; var pelvis_max := -INF; var max_pelvis_frame := 0
    var max_knee_correction := -1.0; var max_knee_frame := 0; var min_right_foot_y := INF; var right_contact_frame := 0
    for i in range(frames.size()):
        var frame: Dictionary = frames[i]; max_pose_error = max(max_pose_error, _apply_frame(skeleton, mapping, frame)); await process_frame
        var expected_head_world := _head_expected_world(skeleton, head_idx); var predicted_attachment_world := expected_head_world * head_follow_calibration
    ...
    for candidate in [max_knee_frame, max_pelvis_frame, right_contact_frame, 69, 70, 71, 72]:
        if not capture_frames.has(candidate): capture_frames.append(candidate)
    DirAccess.make_dir_recursive_absolute(_capture_dir); var captures: Array = []
    for idx in capture_frames:
        var frame: Dictionary = frames[idx]; var err := _apply_frame(skeleton, mapping, frame); max_pose_error = max(max_pose_error, err)
        var png := _capture_dir.path_join("frame-%03d.png" % idx)
        if not await _capture(png): push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: capture"); quit(11); return
        captures.append({"sample_index": idx, "png": png, "pelvis_delta_mm": int(frame.get("pelvis_delta_mm", 0))})
    var right_support := _support_band_path(frames, "RightFoot"); var left_support := _support_band_path(frames, "LeftFoot")
    var report := {"schema":"grand-bruxelles-civ1-dynamic-full-character-v1","diagnostic_only":true,"runtime_authorized":false,"visual_approval_claimed":false,"player_view_claimed":false,"resolution":[WIDTH,HEIGHT],"frame_count":frames.size(),"body_mesh_count":body_meshes.size(),"head_mesh_count":head_meshes.size(),"head_surface_count":head_surfaces,"head_material_surface_count":head_materials,"head_attachment_bone":HEAD_BONE,"head_follow_metric":"settled_calibrated_bone_attachment_world_residual","head_attachment_calibration_offset_m":head_attachment_calibration_offset_m,"head_settle_frames":2,"max_pose_origin_error_m":max_pose_error,"max_head_follow_error_m":max_head_follow_error,"pelvis_vertical_range_m":pelvis_max-pelvis_min,"max_knee_correction_m":max_knee_correction,"right_support_band":right_support,"left_support_band":left_support,"captures":captures,"targeted_right_foot_transition_samples":[69,70,71,72],"verdict":"REQUIRE_HUMAN_DYNAMIC_VISUAL_REVIEW"}
    if max_pose_error > 0.0001 or max_head_follow_error > 0.0001: report["verdict"] = "JETER_DYNAMIC_TECHNICAL_DRIFT"
    if not _write_json(_report_path, report): push_error("CIV1_DYNAMIC_FULL_CHARACTER_FAIL: report"); quit(12); return
    print("CIV1_DYNAMIC_FULL_CHARACTER_OK pose_error=%.9f head_follow_residual=%.9f head_calibration=%.9f pelvis_range=%.6f knee=%.6f" % [max_pose_error,max_head_follow_error,head_attachment_calibration_offset_m,pelvis_max-pelvis_min,max_knee_correction])
