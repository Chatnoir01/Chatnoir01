...
func _run()->void:
    ...
    var camera:=Camera3D.new(); camera.position=Vector3(0.0,0.95+placement_y,3.0); camera.fov=40.0; world.add_child(camera); camera.look_at(Vector3(0.0,0.85+placement_y,0.0)); camera.current=true
