...
func _initialize() -> void:
    ...
    var sum_min := INF
    var sum_max := -INF
    var influence_count_histogram := {}
    var normalization_violation_count := 0
    for v in selected:
        var s := float(v["influence_weight_sum"])
        sum_min = min(sum_min, s)
        sum_max = max(sum_max, s)
        var n := int(v["influences"].size())
        influence_count_histogram[str(n)] = int(influence_count_histogram.get(str(n), 0)) + 1
        if abs(s - 1.0) > NORMALIZATION_TOLERANCE:
            normalization_violation_count += 1

    var report := {
        "schema":"grand-bruxelles-civ1-rightfoot-full-skin-basis-v2",
        ...
        "vertices":selected,
        "influence_weight_sum_min":sum_min,
        "influence_weight_sum_max":sum_max,
        "influence_count_histogram":influence_count_histogram,
    ...
    if normalization_violation_count != 0:
        push_error("CIV1_FULL_SKIN_BASIS_FAIL:non-normalized:%d" % normalization_violation_count)
        quit(9); return
    print("CIV1_FULL_SKIN_BASIS_OK vertices=", selected.size(), " positive_slots=", counters["positive_slots"], " sum_min=", sum_min, " sum_max=", sum_max)
    quit(0)
