...
def inspect_layer(layer: ogr.Layer, sample_features: int, max_coords_per_feature: int) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_for_height_validation": bool(epsg == EXPECTED_EPSG and declared_has_z and actual_has_nonzero_z),
    }
...
def inspect_gpkg(path: Path, sample_features: int, max_coords_per_feature: int) -> dict[str, Any]:
    ...
    layers = [inspect_layer(dataset.GetLayerByIndex(index), sample_features, max_coords_per_feature) for index in range(dataset.GetLayerCount())]
    candidate_layers = [layer["name"] for layer in layers if layer["candidate_for_height_validation"]]
    return {"path": str(path), "size_bytes": path.stat().st_size, "sha256": sha256_file(path), "layer_count": len(layers), "candidate_layer_count": len(candidate_layers), "candidate_layers": candidate_layers, "layers": layers}


def build_inventory(root: Path, selected_source: dict[str, Any] | None, sample_features: int, max_coords_per_feature: int) -> dict[str, Any]:
    ...
    all_epsg = sorted({str(layer["epsg"]) for package in packages for layer in package["layers"] if layer.get("epsg")})
    return {
        "schema": SCHEMA,
        ...
        "inspection_policy": {"read_only": True, "sample_features_per_layer": sample_features, "max_coordinates_per_sampled_feature": max_coords_per_feature, "candidate_requires": ["EPSG:31370", "declared Z geometry", "sampled finite non-zero Z coordinates"]},
...
def main() -> int:
    ...
    inventory = build_inventory(args.root, selected_source, max(1, args.sample_features), max(1, args.max_coords_per_feature))
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(inventory, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print("URBIS_3D_GPKG_INVENTORY:", "gpkg=", inventory["gpkg_count"], "candidate_layers=", inventory["candidate_layer_count"], "epsg=", inventory["observed_epsg_codes"], "usable=", inventory["usable_as_second_height_source"])
