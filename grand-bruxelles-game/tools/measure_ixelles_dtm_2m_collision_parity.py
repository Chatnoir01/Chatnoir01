...
from measure_ixelles_dtm_2m_seams import (
    CELLS,
    SOURCE_ARCHIVES,
    SPACING_M,
    make_grid,
...
def compare_surface_collision(
    render_grid: np.ndarray,
    ...
    *,
    ...
    passed = bool(
        ...
        and nonzero_xy == 0
        and indices_equal
        and render_vertex_sha == collision_vertex_sha
        and render_index_sha == collision_index_sha
    )
    return {
...
def measure(paths: list[Path], archive_hashes: dict[str, str]) -> dict:
    ...
    for cell_id, bbox in CELLS:
        ...
        # Generate each candidate independently from the same locked source mosaic.
        render_grid = make_grid(array, transform, bbox)
        collision_grid = make_grid(array, transform, bbox)
        parity = compare_surface_collision(
            render_grid,
            collision_grid,
            west=west,
            south=south,
