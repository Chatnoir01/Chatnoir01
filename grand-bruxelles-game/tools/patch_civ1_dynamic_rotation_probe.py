#!/usr/bin/env python3
"""Wire a validated CIV-1 dynamic RightFoot rotation schedule into the native Godot probe.

QA-only. The generated schedule is applied around the target RightFoot local X axis at
individual animation samples. The original target rest length is preserved by rotation;
LeftFoot stays untouched. This transformer does not authorize runtime or visual promotion.
"""
...
def _validated_angles(payload: dict) -> list[float]:
    ...
    for index, sample in enumerate(samples):
        ...
        for field, tolerance in (("right_foot_length_error_m", 1e-6), ("left_foot_delta_m", 1e-9)):
...
def transform(text: str, payload: dict) -> str:
    ...
    right = (
        f"    # {MARKER} center=59 cycle=120 axis=local_x\n"
    ...
    sample = (
        ...
        "        var right_dynamic_sample_rest := Basis(Vector3.RIGHT, right_dynamic_angle) * normalized_target_local_rest_origin\n"
        "        if abs(right_dynamic_sample_rest.length() - target_local_rest_origin.length()) > 0.000000001:\n"
