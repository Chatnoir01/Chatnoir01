#!/usr/bin/env python3
"""Wire a validated CIV-1 dynamic RightFoot Z-component schedule into the native Godot probe.

QA-only. The candidate preserves target-local X and total RightFoot rest length, changes only
Z toward the source-derived reference, recomputes Y with the original sign, leaves LeftFoot
untouched, and never authorizes runtime or visual promotion.
"""
from __future__ import annotations

import argparse
import json
import math
from pathlib import Path

RIGHT = "    var normalized_target_local_rest_origin := normalized_target_local_direction * target_local_rest_origin.length()\n"
LEFT = "    var normalized_target_left_local_rest_origin := normalized_target_left_local_direction * target_left_local_rest_origin.length()\n"
SAMPLE = "        var normalized_target_foot_origin := target_right_parent_pose.origin + target_right_parent_pose.basis * normalized_target_local_rest_origin\n"
MARKER = "CIV1_DYNAMIC_Z_COMPONENT_NATIVE_PROBE"
EXPECTED_KIND = "civ1_rightfoot_dynamic_z_component_schedule"
EXPECTED_COMPONENT = "target_local_rest_z"
EXPECTED_CENTER = 59
EXPECTED_CYCLE = 120
LENGTH_TOLERANCE_M = 1e-6


def _validated_blends(payload: dict) -> list[float]:
    if payload.get("candidate_kind") != EXPECTED_KIND:
        raise ValueError("unexpected candidate kind")
    if payload.get("component") != EXPECTED_COMPONENT:
        raise ValueError("candidate must explicitly target target-local rest Z")
    if payload.get("candidate_is_native_measurement") is not False:
        raise ValueError("input must remain pre-native candidate evidence")
    if payload.get("runtime_authorized") is not False or payload.get("visual_approval_claimed") is not False:
        raise ValueError("promotion claim present in candidate evidence")
    if payload.get("center_sample") != EXPECTED_CENTER or payload.get("baseline_source_plant_sample") != EXPECTED_CENTER:
        raise ValueError("candidate is not bound to measured plant sample 59")
    if payload.get("cycle_sample_count") != EXPECTED_CYCLE or payload.get("baseline_cycle_sample_count") != EXPECTED_CYCLE:
        raise ValueError("candidate is not bound to measured 120-sample cycle")
    radius = payload.get("radius_samples")
    if isinstance(radius, bool) or not isinstance(radius, int) or not 2 <= radius < EXPECTED_CYCLE // 2:
        raise ValueError("invalid radius")
    samples = payload.get("samples")
    if not isinstance(samples, list) or len(samples) != EXPECTED_CYCLE:
        raise ValueError("candidate must contain exactly 120 samples")

    blends: list[float] = []
    for index, sample in enumerate(samples):
        if not isinstance(sample, dict):
            raise ValueError("invalid sample payload")
        value = sample.get("z_component_blend")
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError("z_component_blend must be numeric")
        blend = float(value)
        if not math.isfinite(blend) or not 0.0 <= blend <= 1.0:
            raise ValueError("z_component_blend must be finite and in [0,1]")
        distance = min((index - EXPECTED_CENTER) % EXPECTED_CYCLE, (EXPECTED_CENTER - index) % EXPECTED_CYCLE)
        if distance > radius and abs(blend) > 1e-12:
            raise ValueError("blend outside declared window")
        for field, tolerance in (("right_foot_length_error_m", LENGTH_TOLERANCE_M), ("left_foot_delta_m", 1e-9)):
            metric = sample.get(field)
            if isinstance(metric, bool) or not isinstance(metric, (int, float)) or not math.isfinite(float(metric)):
                raise ValueError(f"invalid {field}")
            if abs(float(metric)) > tolerance:
                raise ValueError(f"candidate violates {field} rail")
        blends.append(blend)

    if blends[EXPECTED_CENTER] <= 1e-9:
        raise ValueError("center sample must be active")
    active = [round(v, 12) for v in blends if v > 1e-12]
    if len(set(active)) < 2:
        raise ValueError("candidate is not time-varying")
    for distance in range(radius):
        inner = blends[(EXPECTED_CENTER + distance) % EXPECTED_CYCLE]
        outer = blends[(EXPECTED_CENTER + distance + 1) % EXPECTED_CYCLE]
        if outer > inner + 1e-12:
            raise ValueError("blend taper grows away from plant center")
    return blends


def transform(text: str, payload: dict) -> str:
    blends = _validated_blends(payload)
    if MARKER in text:
        raise ValueError("input already contains dynamic Z-component probe")
    if "func _make_shadow_skeleton" not in text or "left_foot_reference_ab" not in text:
        raise ValueError("input is not the validated bilateral shadow probe")
    if text.count(RIGHT) != 1 or text.count(LEFT) != 1 or text.count(SAMPLE) != 1:
        raise ValueError("validated dynamic Z-component anchors drifted")

    encoded = ", ".join(format(v, ".17g") for v in blends)
    right = (
        f"    # {MARKER} center=59 cycle=120 component=target_local_rest_z\n"
        f"    var right_dynamic_z_blend_schedule := PackedFloat64Array([{encoded}])\n"
        "    var right_dynamic_source_z: float = normalized_target_local_direction.z * target_local_rest_origin.length()\n"
        "    var normalized_target_local_rest_origin := target_local_rest_origin\n"
    )
    left = (
        "    # Dynamic RightFoot Z-component candidates intentionally leave LeftFoot rest translation unchanged.\n"
        "    var normalized_target_left_local_rest_origin := target_left_local_rest_origin\n"
    )
    sample = (
        "        var right_dynamic_blend: float = right_dynamic_z_blend_schedule[sample_idx % 120]\n"
        "        var right_dynamic_length: float = target_local_rest_origin.length()\n"
        "        var right_dynamic_x: float = target_local_rest_origin.x\n"
        "        var right_dynamic_z: float = lerp(target_local_rest_origin.z, right_dynamic_source_z, right_dynamic_blend)\n"
        "        var right_dynamic_y_sq: float = right_dynamic_length * right_dynamic_length - right_dynamic_x * right_dynamic_x - right_dynamic_z * right_dynamic_z\n"
        "        if right_dynamic_y_sq < -0.000000000001:\n"
        "            push_error(\"CIV1_GLOBAL_CHAIN_DIAGNOSTIC_FAIL: dynamic Z component exceeds preserved RightFoot length\")\n"
        "            quit(20)\n"
        "            return\n"
        "        var right_dynamic_y_sign: float = -1.0 if target_local_rest_origin.y < 0.0 else 1.0\n"
        "        var right_dynamic_sample_rest := Vector3(right_dynamic_x, right_dynamic_y_sign * sqrt(max(0.0, right_dynamic_y_sq)), right_dynamic_z)\n"
        "        if abs(right_dynamic_sample_rest.length() - right_dynamic_length) > 0.000001:\n"
        "            push_error(\"CIV1_GLOBAL_CHAIN_DIAGNOSTIC_FAIL: dynamic Z component changed RightFoot length\")\n"
        "            quit(20)\n"
        "            return\n"
        "        var normalized_target_foot_origin := target_right_parent_pose.origin + target_right_parent_pose.basis * right_dynamic_sample_rest\n"
    )
    out = text.replace(RIGHT, right, 1).replace(LEFT, left, 1).replace(SAMPLE, sample, 1)
    if out.count(MARKER) != 1:
        raise ValueError("dynamic Z-component marker insertion failed")
    return out


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("input_probe", type=Path)
    parser.add_argument("schedule_json", type=Path)
    parser.add_argument("output_probe", type=Path)
    args = parser.parse_args()
    payload = json.loads(args.schedule_json.read_text(encoding="utf-8"))
    args.output_probe.write_text(transform(args.input_probe.read_text(encoding="utf-8"), payload), encoding="utf-8")
    print("CIV1_DYNAMIC_Z_COMPONENT_NATIVE_PROBE_OK")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
