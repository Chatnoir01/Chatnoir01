...
LEFT_AB_TOKEN=re.compile(r'(?<![A-Za-z0-9_])left_foot_reference_ab(?![A-Za-z0-9_])')

def transform(text:str)->str:
    if MARKER in text: raise ValueError('input already patched')
    if 'func _make_shadow_skeleton' not in text or not LEFT_AB_TOKEN.search(text): raise ValueError('input is not validated bilateral shadow probe')
