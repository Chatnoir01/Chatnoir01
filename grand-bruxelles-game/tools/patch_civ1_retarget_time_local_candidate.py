...
def transform(text: str, center_sample: int, radius: int) -> str:
    ...
    right = (
        f"    # {MARKER} center_sample={center_sample} radius={radius}\n"
    ...
    left = (
        "    # Time-local candidates intentionally leave LeftFoot rest translation unchanged.\n"
        "    var normalized_target_left_local_rest_origin := target_left_local_rest_origin\n"
    )
    sample = (
        "        var right_time_local_forward: int = posmod(sample_idx - right_time_local_center, SAMPLE_COUNT - 1)\n"
    ...
    if out.count(MARKER) != 1:
        raise ValueError("time-local candidate marker insertion failed")
    return out
...
def main() -> int:
    ...
    parser.add_argument("--center-sample", type=int, required=True)
    parser.add_argument("--radius", type=int, required=True)
    args = parser.parse_args()
    args.output.write_text(transform(args.input.read_text(encoding="utf-8"), args.center_sample, args.radius), encoding="utf-8")
    print(f"CIV1_RETARGET_TIME_LOCAL_CANDIDATE_OK center_sample={args.center_sample} radius={args.radius}")
    return 0
