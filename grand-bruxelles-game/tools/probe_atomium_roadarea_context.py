#!/usr/bin/env python3
import hashlib, json, math, pathlib, urllib.parse, urllib.request, urllib.error
...
payload=None; accepted_url=None; accepted_raw=None; attempts=[]
for endpoint,type_name in ROUTES:
    for version,type_key,output,count_key in VARIANTS:
        ...
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'Grand-Bruxelles-source-gate/1.2','Accept':'application/json,application/geo+json,*/*'})
            ...
            if raw.lstrip().startswith((b'{',b'[')):
                ...
                if isinstance(candidate,dict) and isinstance(candidate.get('features'),list):
                    payload=candidate; accepted_url=url; accepted_raw=raw; break
...
if payload is None or accepted_raw is None:
...
for f in features:
    ...
    rank.append({'id':f['id'],'area_m2':round(f['area_m2'],3),'bbox_epsg31370':[round(min_e,3),round(min_n,3),round(max_e,3),round(max_n,3)],'properties':f['properties'],'geometry':f['geometry']})
rank.sort(key=lambda r:-r['area_m2'])
out={'schema':2,'format':'grand-bruxelles-atomium-roadarea-source-gate-v2',
     'source':{'organization':'Paradigm','service':'INSPIRE vector Brussels WFS','query_url':accepted_url,'crs':'EPSG:31370',
               'layer':'TN.RoadTransportNetwork.RoadArea','response_sha256':hashlib.sha256(accepted_raw).hexdigest(),
               'license':'CC-BY-4.0','license_url':'https://creativecommons.org/licenses/by/4.0/',
               'attribution':'Paradigm — INSPIRE RoadTransportNetwork RoadArea, Brussels-Capital Region',
     ...
     'runtime_policy':{'runtime_approved':False,'material_photometry_resolved':False,'geometry_exact_from_wfs':True,
                       'presentation_material_only':True,'may_proceed_if_ground_wedge_coverage_fraction_at_least':0.08}}
...
print('ATOMIUM_ROADAREA_PROBE_OK',json.dumps({'features':len(features),'coverage':round(coverage,4),'sha256':out['source']['response_sha256'],'top':[{'id':x['id'],'area_m2':x['area_m2']} for x in rank[:8]]},ensure_ascii=False))
if coverage < 0.08:
