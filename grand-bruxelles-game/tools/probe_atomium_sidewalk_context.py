...
for f in payload.get('features',[]):
    g=f.get('geometry') or {}
    if polygons(g): features.append({'id':str(f.get('id','')),'geometry':canon_geom(g)})
features.sort(key=lambda f:f['id'])
green=json.load(open('data/environment/laeken_jette/atomium_landcover_context.game.json',encoding='utf-8'))['geometry']
samples=[]; sidewalk_hits=exclusive_hits=overlap_hits=0
for forward in range(5,121,5):
    ...
    while lateral<=half_width+1e-9:
        p=(CAMERA[0]-forward,CAMERA[1]+lateral); ids=[f['id'] for f in features if point_in_geom(p,f['geometry'])]
        ins=bool(ids); ing=point_in_geom(p,green); sidewalk_hits+=int(ins); overlap_hits+=int(ins and ing); exclusive_hits+=int(ins and not ing)
...
print('ATOMIUM_SIDEWALK_SOURCE_OK',json.dumps(out['witness'],ensure_ascii=False),'sha256='+geometry_sha,'features='+str(len(features)))
