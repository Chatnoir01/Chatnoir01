...
def measurement_production_base_sha(contract):
    status = contract["status"]
    if status == "LOCKED_SOURCE_ONLY_ARTIFACT":
        continuity = contract.get("continuity")
        assert isinstance(continuity, dict) and continuity, "Locked phase requires continuity"
        assert continuity["policy"] == "artifact-lock-independent-of-original-git-ancestry"
        original = continuity.get("original_acquisition_base_sha")
        assert isinstance(original, str) and len(original) == 40, "Locked phase requires original acquisition base SHA"
        return original
    return contract["production_base_sha"]
...
def main():
    ...
    measurement = {
        ...
        "production_base_sha": measurement_production_base_sha(contract),
    ...
    if status == "LOCKED_SOURCE_ONLY_ARTIFACT":
        ...
        assert measurement["production_base_sha"] == contract["continuity"]["original_acquisition_base_sha"], "Locked acquisition base drift"
