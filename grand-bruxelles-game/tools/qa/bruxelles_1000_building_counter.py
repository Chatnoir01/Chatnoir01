...
import argparse,csv,hashlib,json,os,re,tempfile,time,urllib.parse,urllib.request,zipfile
...
LAYER='urbisvector:Buildings'; CRS='EPSG:31370'; UA='Grand-Bruxelles-1000-Counter/1.0'
...
def buildings(boundary):
 box=boundary.bounds; P=prep(boundary); out={}; start=0; pages=0; total=0
 while 1:
  q={'service':'WFS','version':'2.0.0','request':'GetFeature','typeNames':LAYER,'outputFormat':'application/json','srsName':CRS,'bbox':f'{box[0]:.3f},{box[1]:.3f},{box[2]:.3f},{box[3]:.3f},{CRS}','count':'5000','startIndex':str(start)}
  d=json.loads(get(WFS+'?'+urllib.parse.urlencode(q))); fs=d.get('features') or []; pages+=1; total+=len(fs)
  for f in fs:
  ...
  n=int(d.get('numberReturned',len(fs)))
  if n<5000 or not fs:break
  start+=n
  if pages>100:raise RuntimeError('WFS pagination runaway')
 if not out:raise RuntimeError('zero buildings in 1000')
 return out,{'pages':pages,'bbox_features':total,'owners':len(out),'owner_sha256':digest(sorted(out,key=int))}
...
def main():
 ...
 a=ap.parse_args(); c=json.loads(a.contract.read_text()); boundary,bmeta=postal(a.ngi_postal_zip); off,wmeta=buildings(boundary); O=set(off)
 ...
 for x in sorted(O,key=int):
  ...
  elif x in c1 or x in data or x in run:s='source_registered_main'
 ...
 total=len(O); report={'schema':'grand-bruxelles-postcode-building-counter-v1','scope':'1000 Bruxelles / Brussel','production_base_sha':c['production_base_sha'],'postal_boundary':{**c['postal_boundary'],**bmeta},'buildings':{**c['building_source'],**wmeta},'evidence':{'planner_missing_inside_1000':len(O&missing),'c01_inside_1000':len(O&c1),'c02_inside_1000':len(O&c2),'repo_data_refs':len(O&data),'repo_runtime_refs':len(O&run),'open_pr_refs':len(set(prs))},'counts':{'official_building_owner_total':total,'finished_perfect':counts['finished_perfect'],'started_open_pr':counts['started_open_pr'],'source_registered_main':counts['source_registered_main'],'source_draft':counts['source_draft'],'unstarted':counts['unstarted'],'unresolved':counts['unresolved'],'remaining_to_perfect':total-len(finished)},'status_rules':c['status_rules'],'runtime_authorized':False,'geometry_modified':False}
 if sum(counts.values())!=total:raise RuntimeError('accounting drift')
