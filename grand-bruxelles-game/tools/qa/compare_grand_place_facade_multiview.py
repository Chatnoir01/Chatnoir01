...
def main() -> int:
    ...
    for view in gate['views']:
        ...
        row={'id':vid,'changed_gt3_percent':p3,'changed_gt8_percent':p8,'bbox_gt8':[min_x,min_y,max_x,max_y],'bbox_width':bw,'bbox_height':bh,'thresholds':{'minimum_gt3_percent':view['minimum_gt3_percent'],'minimum_gt8_percent':view['minimum_gt8_percent'],'minimum_bbox_width':view['minimum_bbox_width'],'minimum_bbox_height':view['minimum_bbox_height']}}
        rows.append(row)
        print(f'GRAND_PLACE_FACADE_MULTIVIEW_METRIC: {vid} >3={p3:.6f}% >8={p8:.6f}% bbox={bw}x{bh}')
        if p3<float(view['minimum_gt3_percent']) or p8<float(view['minimum_gt8_percent']) or bw<int(view['minimum_bbox_width']) or bh<int(view['minimum_bbox_height']):
            Path(args.output).write_text(json.dumps({'schema':'grand-bruxelles-grand-place-facade-multiview-result-v1','pass':False,'views':rows},indent=2)+'\n')
            raise SystemExit(f'GRAND_PLACE_FACADE_MULTIVIEW_FAIL: {vid} threshold')
    payload={'schema':'grand-bruxelles-grand-place-facade-multiview-result-v1','pass':True,'camera_position_fixed':True,'fov_fixed':True,'thresholds_frozen_before_first_render':True,'human_full_frame_pass_required':True,'finished_perfect':False,'views':rows}
