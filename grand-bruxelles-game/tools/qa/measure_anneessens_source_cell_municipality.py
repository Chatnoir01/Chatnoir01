#!/usr/bin/env python3
"""Measure official municipality coverage for the persisted Anneessens UrbIS source cell.
...
EXPECTED_SOURCE_SEMANTIC_SHA256 = "496e1049db535dbf3db2192066f41f60e9297e7b7931d9a8a427d6ffa378f8b0"
EXPECTED_MANIFEST_SHA256 = "10aac9d4a2404ff23bb7d9af40b9e256b65aa6f25d33c7de1c1e3067244f43d2"
EXPECTED_MATURITY_SHA256 = "7ebbe70634650db6b2fe21b10e4ae87ede7f4c0c170b21b63d1be9765cc69f89"
...
CLOSED_OUTPUT_RAILS = (
    ...
    "municipality_assignment_authorized",
...
def validate_measurement(measurement: dict[str, Any]) -> None:
    if measurement.get("schema") != "grand-bruxelles-urbis-source-cell-semantic-measurement-v1":
        raise RuntimeError("Anneessens source measurement schema drift")
    if measurement.get("cell_id") != CELL_ID or measurement.get("crs") != CRS:
        raise RuntimeError("Anneessens source cell identity/CRS drift")
    if [float(v) for v in measurement.get("bbox", [])] != BBOX:
        raise RuntimeError("Anneessens source bbox drift")
    if measurement.get("maturity_state") != "data_ready":
        raise RuntimeError("Anneessens source must remain data_ready")
    if measurement.get("source_semantic_sha256") != EXPECTED_SOURCE_SEMANTIC_SHA256:
        raise RuntimeError("Anneessens source semantic SHA drift")
    if measurement.get("manifest_sha256") != EXPECTED_MANIFEST_SHA256:
        raise RuntimeError("Anneessens source manifest SHA drift")
    if measurement.get("maturity_sha256") != EXPECTED_MATURITY_SHA256:
        raise RuntimeError("Anneessens maturity SHA drift")
    ...
    for name, expected in EXPECTED_LAYER_COUNTS.items():
        if int((layers.get(name) or {}).get("features", -1)) != expected:
            raise RuntimeError(f"Anneessens {name} count drift")
    buildings = layers.get("buildings") or {}
    if int(buildings.get("ownership_filtered", -1)) != 52:
        raise RuntimeError("Anneessens building ownership-filter accounting drift")
    if int(buildings.get("invalid_ownership_features", -1)) != 0:
        raise RuntimeError("Anneessens source contains invalid building ownership")
    for key in CLOSED_MEASUREMENT_RAILS:
        if measurement.get(key) is not False:
            raise RuntimeError(f"Anneessens source rail {key} must remain false")


def validate_persisted_source(repo_root: Path) -> dict[str, Any]:
    measurement_path = repo_root / "data/provenance/anneessens_urbis_source_cell.measurement.json"
    ...
    if sha256_bytes(manifest_raw) != EXPECTED_MANIFEST_SHA256:
        raise RuntimeError("persisted Anneessens manifest bytes drift")
    if sha256_bytes(maturity_raw) != EXPECTED_MATURITY_SHA256:
        raise RuntimeError("persisted Anneessens maturity bytes drift")

    manifest = json.loads(manifest_raw)
    maturity = json.loads(maturity_raw)
    if manifest.get("cell_id") != CELL_ID or manifest.get("crs") != CRS:
        raise RuntimeError("persisted Anneessens manifest identity/CRS drift")
    if [float(v) for v in manifest.get("bbox", [])] != BBOX:
        raise RuntimeError("persisted Anneessens manifest bbox drift")
    if manifest.get("promotion") != "source_only_no_runtime_mutation":
        raise RuntimeError("Anneessens source promotion contract drift")
    if (maturity.get("maturity") or {}).get("state") != "data_ready":
        raise RuntimeError("persisted Anneessens maturity state drift")
    ...
    for key in ("runtime_geometry", "collisions", "streaming", "terrain", "heights", "photo_match", "performance"):
        if gates.get(key) is not False:
            raise RuntimeError(f"Anneessens maturity gate {key} must remain false")

    for name, expected_count in EXPECTED_LAYER_COUNTS.items():
        layer = measurement["layers"][name]
        rel = str(layer["file"])
        raw = (cell_dir / rel).read_bytes()
        if len(raw) != int(layer["raw_bytes"]):
            raise RuntimeError(f"persisted Anneessens {name} byte count drift")
        if sha256_bytes(raw) != layer["raw_forensic_sha256"]:
            raise RuntimeError(f"persisted Anneessens {name} SHA drift")
        payload = json.loads(raw)
        features = payload.get("features")
        if not isinstance(features, list) or len(features) != expected_count:
            raise RuntimeError(f"persisted Anneessens {name} feature count drift")
    return measurement
...
def run(repo_root: Path, output_path: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "persisted_source": {
            "measurement_path": "data/provenance/anneessens_urbis_source_cell.measurement.json",
        ...
        "canonical_registration_authorized": False,
        "municipality_assignment_authorized": False,
    ...
    print(
        "ANNEESSENS_MUNICIPALITY_PREFLIGHT_OK "
        f"cell={CELL_ID} status={result['status']} municipalities={len(coverage['intersections'])} "
        f"coverage={coverage['intersection_coverage_sum']:.12f} semantic={result['semantic_sha256']}"
    )
    return result
