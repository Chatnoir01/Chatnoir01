...
def validate_evidence(gate_path: Path | str, manifest_path: Path | str, artifact_root: Path | str | None = None) -> dict[str, Any]:
    ...
    return {"artifact_kind":manifest["artifact_kind"],"base_sha":base_sha,"head_sha":head_sha,"resolution":list(expected_dims),"view_count":len(validated_files),"view_ids":view_ids,"files":validated_files,"source_surface_facing":source_surface_facing,"human_review_status":"pending","visual_approval_claimed":False}
...
def main() -> int:
    ...
    print("GRAND_PLACE_FACADE_EVIDENCE_STRUCTURALLY_VALID " f"views={result['view_count']} resolution={result['resolution'][0]}x{result['resolution'][1]} " f"maison_du_roi_front_wall_ratio={facing['front_facing_wall_area_ratio']:.6f} " f"maison_du_roi_front_roof_ratio={facing['front_facing_roof_area_ratio']:.6f} " "human_review=pending visual_approval_claimed=false")
    return 0
