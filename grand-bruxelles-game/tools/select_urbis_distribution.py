...
def direct_score(link: dict[str, str]) -> tuple[int, int]:
    ...
    # Prefer a packaged distribution so all GPKG companion metadata is retained.
    suffix_score = 2 if href.endswith(".zip") else 1
    rel_score = 1 if link.get("rel", "").lower() in {"enclosure", "alternate"} else 0
...
def resolve_links(initial_links: list[dict[str, str]], max_depth: int = 3) -> dict[str, Any]:
    ...
    while queue:
        ...
        try:
            root = fetch_xml(url)
        except Exception as exc:  # noqa: BLE001 - diagnostic metadata is useful here.
...
def main() -> int:
    ...
    # Prefer the most specific / shortest matching title if duplicate translated entries occur.
    ...
    print("Selected:", output["selected"]["href"])
    return 0
