...
def strip_animations(data: bytes) -> tuple[bytes, int]:
    ...
    # Preserve every non-animation JSON structure and every binary/extension chunk byte-for-byte.
    # JSON is re-serialized canonically so the output is deterministic across runs/platforms.
    ...
    # Fail closed: verify that output reparses and no animation payload survived.
