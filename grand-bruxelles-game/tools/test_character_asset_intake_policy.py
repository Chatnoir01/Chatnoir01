...
H0 = hashlib.sha256(b"license-snapshot-fixture").hexdigest()
H1 = hashlib.sha256(b"archive-fixture").hexdigest()
...
def adopt_with_payloads(payloads):
    ...
    doc["adopted"] = [UAL]
    c.update({
        "license_snapshot_sha256": H0,
        "acquired_archive_sha256": H1,
...
for field in ("license_snapshot_sha256", "acquired_archive_sha256"):
    ...
    assert any("non-placeholder" in e and field in e for e in validate(doc)), field
for sentinel in ("0" * 64, "a" * 64, "f" * 64):
...
for unsafe_name in (".", "../walk.glb", "/tmp/walk.glb", "clips/../../walk.glb", "clips\\walk.glb", "",
                    ...
                    "clips/" + "a" * 256,
                    "clips/" + "é" * 128):
    errors = validate(adopt_with_payloads({unsafe_name: H2}))
