...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        blocked_status = {"production_authorized":False,"activation_ready":False,"source_package_present":False,"runtime_package_present":False,"blocker":"source_not_ready"}
        ...
        case_variant_registry={"schema":"grand-bruxelles-civ1-roster-registry-v1","entries":[{"asset_path":"GRAND-BRUXELLES-GAME/ASSETS/CHARACTERS/CIVILIANS/CIV1/CIV1.GLB"}]}
        assert blocking_entries(case_variant_registry,root)==["GRAND-BRUXELLES-GAME/ASSETS/CHARACTERS/CIVILIANS/CIV1/CIV1.GLB"],"CIV-1 readiness must fail closed for case-variant roster paths on case-insensitive deployment filesystems"
        nested_civ1_path="GRAND-BRUXELLES-GAME/ASSETS/CHARACTERS/CIVILIANS/CIV1/LOD/CIV1_LOD1.GLB"
        nested_civ1_registry={"schema":"grand-bruxelles-civ1-roster-registry-v1","entries":[{"asset_path":nested_civ1_path}]}
        assert blocking_entries(nested_civ1_registry,root)==[nested_civ1_path],"CIV-1 readiness must cover nested LOD/source-derived assets beneath the candidate namespace"
        civ1_root_path="GRAND-BRUXELLES-GAME/ASSETS/CHARACTERS/CIVILIANS/CIV1"
        civ1_root_registry={"schema":"grand-bruxelles-civ1-roster-registry-v1","entries":[{"asset_path":civ1_root_path}]}
        assert blocking_entries(civ1_root_registry,root)==[],"the CIV-1 namespace directory itself must not be misclassified as a roster asset"
        for sibling in ("CIV10", "CIV1-OLD", "CIV1_BACKUP"):
            sibling_path=f"GRAND-BRUXELLES-GAME/ASSETS/CHARACTERS/CIVILIANS/{sibling}/{sibling}.GLB"
            sibling_registry={"schema":"grand-bruxelles-civ1-roster-registry-v1","entries":[{"asset_path":sibling_path}]}
            assert blocking_entries(sibling_registry,root)==[],f"CIV-1 readiness boundary must not capture sibling identity {sibling}"
        ...
        ready_status={"candidate_id":"CIV-1","production_authorized":True,"activation_ready":True,"source_package_present":True,"runtime_package_present":True,"blocker":None,"character_source":{"repository":"https://github.com/ibrews/VitruvianGodot","commit":"bdecdcd537b4031fdd0fb299b7e4f93f084fffa0","license_claim":"MIXED_BY_COMPONENT","license_evidence":{"unresolved_components":[]}},"source_paths":[source_path],"source_manifest":{source_path:{"upstream_path":"godot_project/body.glb","license_scope_verified":True,"license":"CC0-1.0","git_blob_sha1":"3914b89458e542b73f0168b0bf80c8e356e78f9c","sha256":"097b2c590e51dcb5d4ac5e75f4efed4c494a7ced9007349c3e3dfbf565cab48b","size_bytes":16}}}
        ...
        for field,bad in (("repository","https://github.com/example/substitute"),("commit","0"*40),("license_claim","CC0-1.0")):
            substituted=json.loads(json.dumps(ready_status)); substituted["character_source"][field]=bad; status.write_text(json.dumps(substituted),encoding="utf-8"); assert source_ready(root) is False,f"readiness must reject substituted upstream {field}"
        runtime_missing=json.loads(json.dumps(ready_status)); runtime_missing["runtime_package_present"]=False; status.write_text(json.dumps(runtime_missing),encoding="utf-8"); assert source_ready(root) is False,"source readiness must fail closed when the runtime package is absent"
        status.write_text(json.dumps(ready_status),encoding="utf-8")
        ...
        source_file.write_bytes(b"tampered-civ1-source"); status.write_text(json.dumps(ready_status),encoding="utf-8"); assert source_ready(root) is False,"readiness must bind each source file to declared payload identities and size"
        source_file.write_bytes(b"civ1-source-body")
        ...
        missing_sha256=json.loads(json.dumps(ready_status)); del missing_sha256["source_manifest"][source_path]["sha256"]; status.write_text(json.dumps(missing_sha256),encoding="utf-8"); assert source_ready(root) is False,"every ready source record must carry SHA-256 payload identity"
        wrong_sha256=json.loads(json.dumps(ready_status)); wrong_sha256["source_manifest"][source_path]["sha256"]="0"*64; status.write_text(json.dumps(wrong_sha256),encoding="utf-8"); assert source_ready(root) is False,"declared SHA-256 must match source payload bytes"
