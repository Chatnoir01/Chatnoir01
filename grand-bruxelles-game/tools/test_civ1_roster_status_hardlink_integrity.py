    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if not readiness.source_ready(root):
                raise AssertionError("ordinary single-link status control must remain eligible")

            external = root / "external-status-alias.json"
            os.link(status, external)
            if status.stat().st_nlink < 2:
                raise AssertionError("hardlink witness was not created")
            if readiness.source_ready(root):
                raise AssertionError("hardlinked source_status.json must not establish CIV-1 READY authority")
    finally:
        readiness._status_consistent = original

    print("CIV1_ROSTER_STATUS_HARDLINK_INTEGRITY_GREEN")
    return 0
