...
EXPECTED_ORIGIN = {
    ...
    "n": 172250.0,
    "altitude": 0.0,
    "strategy": "acquisition_bbox_center",
...
def main() -> int:
    ...
    assert target["game_origin"] == EXPECTED_ORIGIN
    ...
    assert guards["game_origin_outside_acquisition_bbox_forbidden"] is True
    ...
    assert "NORD_ORIGIN_E" in fetch_text and "NORD_ORIGIN_N" in fetch_text
    assert "Nord game origin must stay inside acquisition bbox" in fetch_text
    assert "NORD_CITY_MACHINE_DATA_OK" in validator_text
    assert "REQUIRED_SLUGS" in validator_text
    assert "EXPECTED_ORIGIN" in validator_text
    print(
        "NORD_SOURCE_CONTRACT_OK bbox=1000x500m origin=149500,172250 "
        "layers=5 synthetic_geometry=false promotion=source_only"
    )
    return 0
