...
ROAD_WIDTHS = {
    ...
    "tertiary": 7.0, "unclassified": 6.0, "residential": 5.6,
    "living_street": 5.0, "service": 4.0, "pedestrian": 4.0,
    "track": 3.2, "cycleway": 2.2, "footway": 1.8, "path": 1.6,
}
...
def numeric_tag(tags: dict[str, Any], key: str) -> float | None:
    ...
    if raw is None:
        return None
    try:
        return float(str(raw).strip().lower().replace("m", "").strip())
    except ValueError:
        return None
...
def truthy_osm_tag(tags: dict[str, Any], key: str) -> bool:
    raw = tags.get(key)
    return raw is not None and str(raw).strip().lower() not in {"", "0", "false", "no", "none"}
...
def railway_vertical_metadata(tags: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"tunnel": tunnel, "covered": covered, "layer": layer, "surface_visible": not tunnel and not covered and layer >= 0.0}
...
def building_height(tags: dict[str, Any]) -> float:
    ...
    return {"house": 8.5, "apartments": 14.0, "commercial": 12.0, "retail": 9.5, "office": 15.0, "church": 18.0, "cathedral": 28.0, "train_station": 15.0, "garage": 3.5, "garages": 3.5, "shed": 3.0}.get(str(tags.get("building", "yes")), 10.5)
...
def convert(data: dict[str, Any], origin: tuple[float, float]) -> dict[str, Any]:
    roads, buildings, railways, environment_points = [], [], [], []
    for element in data.get("elements", []):
        tags = element.get("tags", {}) or {}
        if element.get("type") == "node":
            kind = environment_point_kind(tags)
            if kind and "lat" in element and "lon" in element:
                environment_points.append({"osm_id": element.get("id"), "kind": kind, "position": metric_point(float(element["lat"]), float(element["lon"]), *origin)})
            continue
        ...
        if highway:
            ...
            if lanes and lanes >= 2: width = max(width, lanes * 3.0)
            roads.append({"osm_id": element.get("id"), "name": tags.get("name", ""), "class": highway, "width": round(width, 2), "drivable": highway in DRIVABLE, "points": points})
        railway = tags.get("railway")
        if railway:
            railways.append({"osm_id": element.get("id"), "name": tags.get("name", ""), "class": railway, **railway_vertical_metadata(tags), "points": points})
        if "building" in tags and len(points) >= 4:
            if points[0] == points[-1]: points = points[:-1]
            area = polygon_area(points)
            if len(points) >= 3 and 8.0 <= area <= 60_000.0:
                buildings.append({"osm_id": element.get("id"), "name": tags.get("name", ""), "kind": tags.get("building", "yes"), "height": building_height(tags), "area": round(area, 2), "footprint": points})
    all_points = [p for road in roads for p in road["points"]] + [p for b in buildings for p in b["footprint"]] + [p["position"] for p in environment_points]
    bounds = [0.0, 0.0, 0.0, 0.0]
    if all_points:
        xs, zs = [p[0] for p in all_points], [p[1] for p in all_points]
    ...
    return {"format":"grand-bruxelles-osm-v1","source":"OpenStreetMap contributors via Overpass API","license":"ODbL-1.0","origin":{"lat":origin[0],"lon":origin[1]},"bounds_m":bounds,"stats":{"roads":len(roads),"drivable_roads":sum(1 for road in roads if road["drivable"]),"buildings":len(buildings),"railways":len(railways),"environment_points":len(environment_points)},"roads":roads,"buildings":buildings,"railways":railways,"environment_points":environment_points}


def parse_origin(raw: str) -> tuple[float, float]:
    parts = [float(part.strip()) for part in raw.split(",")]
    if len(parts) != 2: raise argparse.ArgumentTypeError("origin must be lat,lon")
    return parts[0], parts[1]


def main() -> int:
    parser = argparse.ArgumentParser(description="Convert Overpass JSON into Grand Bruxelles game data")
    parser.add_argument("--input", type=Path, required=True); parser.add_argument("--output", type=Path, required=True); parser.add_argument("--origin", type=parse_origin, default=DEFAULT_ORIGIN)
    args = parser.parse_args(); raw = json.loads(args.input.read_text(encoding="utf-8")); converted = convert(raw, args.origin)
    args.output.parent.mkdir(parents=True, exist_ok=True); args.output.write_text(json.dumps(converted, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    stats = converted["stats"]; print(f"converted {stats['roads']} roads / {stats['buildings']} buildings / {stats['railways']} railways / {stats['environment_points']} environment points -> {args.output}")
    return 0
