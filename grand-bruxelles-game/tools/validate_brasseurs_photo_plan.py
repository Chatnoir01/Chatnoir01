...
def main() -> None:
    ...
    stored_norm = [float(v) for v in p["normalized_column_x"]]
    for i, (a,b) in enumerate(zip(computed_norm, stored_norm)):
    ...
    stored_offsets = [float(v) for v in derived["column_center_offsets_from_left_m"]]
    for i, (a,b) in enumerate(zip(offsets, stored_offsets)):
        if not math.isclose(a,b,rel_tol=0,abs_tol=0.001): fail(f"world offset {i}")
    spacings = [offsets[i+1]-offsets[i] for i in range(3)]
    stored_spacings = [float(v) for v in derived["bay_center_spacing_m"]]
    for i, (a,b) in enumerate(zip(spacings, stored_spacings)):
