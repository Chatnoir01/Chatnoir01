...
EXPECTED = {
    "candidate_id": "CIV-1", "owner_verdict": "GARDER", "approved_review_pr": 1006,
...
EXPECTED_UNRESOLVED = [
    "godot_project/vitruvian_body.glb",
    "godot_project/hairtool_cards.glb",
    "godot_project/vitruvian_hair.glb",
]
EXPECTED_SOURCE_MANIFEST = {
    "assets/characters/civilians/civ1/source/vitruvian_body.glb": {"upstream_path":"godot_project/vitruvian_body.glb","git_blob_sha1":"09bcade1092e5a89b474e91e6013209d4c68c127","size_bytes":6879364,"license_scope_verified":False},
    "assets/characters/civilians/civ1/source/vitruvian_head.glb": {"upstream_path":"godot_project/vitruvian_head.glb","git_blob_sha1":"0c810e209f09fc079086746f0813de9531d0f7fb","size_bytes":10189832,"license_scope_verified":True,"license":"CC0-1.0"},
    "assets/characters/civilians/civ1/source/hairtool_cards.glb": {"upstream_path":"godot_project/hairtool_cards.glb","git_blob_sha1":"04799adc868ba72e0fa1c1ab60c0442e12d5987e","size_bytes":14839096,"license_scope_verified":False},
    "assets/characters/civilians/civ1/source/vitruvian_hair.glb": {"upstream_path":"godot_project/vitruvian_hair.glb","git_blob_sha1":"b6f63553ce9d5f2f83920b445f21831caf9017f1","size_bytes":21189248,"license_scope_verified":False},
    "assets/characters/civilians/civ1/source/shoes03.obj": {"upstream_path":"base/clothes/shoes03/shoes03.obj","git_blob_sha1":"2cd09f0af9c5bd13604d57d8af19e9205933ee85","size_bytes":None,"license_scope_verified":True,"license":"CC0-1.0"},
}
...
def validate(root: Path, require_ready: bool = False) -> list[str]:
    ...
    for key in ("candidate_id", "owner_verdict", "approved_review_pr"):
        if status.get(key) != EXPECTED[key]: errors.append(f"{key} must be {EXPECTED[key]!r}")
    ...
    if character.get("license_claim") != "MIXED": errors.append("character source license claim must remain MIXED")
    evidence = character.get("license_evidence")
    if not isinstance(evidence, dict):
        errors.append("character source license evidence missing")
        evidence = {}
    if evidence.get("license_path") != "LICENSE" or evidence.get("notice_path") != "NOTICE.md": errors.append("character source license evidence path drift")
    if evidence.get("tool_code_license") != "MIT": errors.append("character tool code license drift")
    explicit = evidence.get("explicit_cc0_assets")
    if not isinstance(explicit, list) or "godot_project/vitruvian_head.glb" not in explicit: errors.append("explicit CC0 head evidence missing")
    if evidence.get("unresolved_assets") != EXPECTED_UNRESOLVED: errors.append("unresolved character asset license scope drift")
    ...
    for label, value in (("source_paths", source_paths), ("runtime_files", runtime_files), ("forbidden_runtime_paths", forbidden)):
        if not isinstance(value, list) or not value: errors.append(f"{label} must be a non-empty list")
    if not isinstance(source_manifest, dict): errors.append("source_manifest must be an object"); source_manifest = {}
    if not isinstance(runtime_hashes, dict): errors.append("runtime_sha256 must be an object"); runtime_hashes = {}
    ...
    if isinstance(source_paths, list):
        ...
        for rel, expected_pin in EXPECTED_SOURCE_MANIFEST.items():
            ...
            if not isinstance(pin, dict): errors.append(f"source manifest entry missing: {rel}"); continue
            for key, expected_value in expected_pin.items():
                if pin.get(key) != expected_value: errors.append(f"source manifest pin drift: {rel} {key}")
            if set(pin) != set(expected_pin): errors.append(f"source manifest metadata drift: {rel}")
    ...
    if source_present:
        ...
        if evidence.get("unresolved_assets"): errors.append("source materialization forbidden while character license evidence remains unresolved")
        if isinstance(source_paths, list):
            for rel in source_paths:
                target = root / rel
                if not target.is_file(): errors.append(f"declared source file missing: {rel}"); continue
                expected_pin = EXPECTED_SOURCE_MANIFEST.get(rel)
                if expected_pin is None: errors.append(f"unrecognized source file: {rel}"); continue
    ...
    if runtime_present and isinstance(runtime_files, list):
        for rel in runtime_files:
            target = root / rel
            if not target.is_file(): errors.append(f"declared runtime file missing: {rel}"); continue
            expected_hash = runtime_hashes.get(rel)
            if not isinstance(expected_hash, str) or len(expected_hash) != 64: errors.append(f"missing sha256 for runtime file: {rel}"); continue
            if _sha256(target) != expected_hash.lower(): errors.append(f"sha256 mismatch for runtime file: {rel}")
